 ...
 *       implements the soot.EquivTo interface, and if so then use the soot.EquivTo.equivTo method
 *       for comparison.<br>
 *       soot.Value extends soot.EquivTo, so it only makes sense to take advantage of that instead
 *       of using the more naiive equals method inherited from Object (only comparing references).
 *   <li>All fields and methods declared as private, changed to protected.
 * </ol>
 *
...
public class ValueArraySparseSet extends AbstractFlowSet {
    ...
    public void intersection(FlowSet otherFlow, FlowSet destFlow) {
        if (sameType(otherFlow) && sameType(destFlow)) {
            ...
            if (dest == other || dest == this) workingSet = new ValueArraySparseSet();
            else {
                workingSet = dest;
                workingSet.clear();
            }
    ...
    public boolean contains(Object obj) {
        for (int i = 0; i < numElements; i++)
            if (elements[i] instanceof EquivTo && ((EquivTo) elements[i]).equivTo(obj)) return true;
            else if (elements[i].equals(obj)) return true;
