...
public class VeryBusyExpsTagger extends BodyTransformer {
    ...
    /**
     * Adds <code>StringTag</code>s and <code>ColorTag</code>s to the body in the interest of using
     * Eclipse to relay information to the user.<br>
     * Every unit that has a busy expression flowing out of it, gets a <code>StringTag</code>
     * describing that fact (per expression). If an expression that is busy out of that unit is also
     * used within it, then we add a <code>ColorTag</code> to that expression.
     *
        ...
        for (Unit u : b.getUnits()) {
            for (Value v : vbe.getBusyExpressionsAfter(u)) {
                ...
                for (ValueBox use : u.getUseBoxes()) {
                    if (use.getValue().equivTo(v)) use.addTag(new ColorTag(ColorTag.RED, TAG_TYPE));
                }
            }
        }
    }
