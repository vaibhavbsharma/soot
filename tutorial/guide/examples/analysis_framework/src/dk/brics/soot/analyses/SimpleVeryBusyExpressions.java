...
public class SimpleVeryBusyExpressions implements VeryBusyExpressions {
    ...
    public SimpleVeryBusyExpressions(DirectedGraph<Unit> graph) {
        ...
        unitToExpressionsAfter =
                new HashMap<Unit, List<AbstractBinopExpr>>(graph.size() * 2 + 1, 0.7f);
        unitToExpressionsBefore =
                new HashMap<Unit, List<AbstractBinopExpr>>(graph.size() * 2 + 1, 0.7f);
...
class SimpleVeryBusyAnalysis extends BackwardFlowAnalysis<Unit, FlowSet> {
    ...
    /**
     * This method performs the actual joining of successor nodes (i.e. doAnalysis calls this method
     * for each successor, if there are more than one, of the current node). Since very busy
     * expressions is a <u>must</u> analysis we join by intersecting.
     *
    ...
    /**
     * Used to initialize the in and out sets for each node. In our case we want to build up the
     * sets as we go, so we initialize with the empty set.
     *
     * <p>Note: If we had information about all the possible values the sets could contain, we could
     * initialize with that and then remove values during the analysis.
     *
     * @return an empty set
     */
    @Override
    ...
    /**
     * Returns a flow set representing the initial set of the entry node. In our case the entry node
     * is the last node and it should contain the empty set.
     *
     * @return an empty set
     */
    @Override
    ...
    /**
     * Adds to the out set the values that flow through the node d from the in set.
     *
     * <p>This method has two phases, a kill phase and a gen phase. The kill phase performs the
     * following:<br>
     * out = (in - expressions containing a reference to any local defined in the node) union out.
     * <br>
     * The gen phase performs the following:<br>
     * out = out union binary operator expressions used in the node.
     *
     ...
     * The kill set is generated by iterating over the def-boxes of the unit. For each local defined
     * in the unit we iterate over the binopExps in the inSet, and check whether they use that
     * local. If so, it is added to the kill set.
     *
                ...
                while (inIt.hasNext()) {
                    ...
                    while (eIt.hasNext()) {
                        ...
                        if (useBox.getValue() instanceof Local
                                && useBox.getValue().equivTo(defBox.getValue())) kills.add(e);
                    }
                }
            }
        }
    ...
    /**
     * Performs gens by iterating over the units use-boxes. If the value of a use-box is a binopExp
     * then we add it to the outSet.
     *
     * @param outSet the set flowing out of the unit
     * @param u the unit being flown through
     */
    private void gen(FlowSet outSet, Unit u) {
        for (ValueBox useBox : u.getUseBoxes()) {
            ...
            if (useBox.getValue() instanceof BinopExpr) outSet.add(useBox.getValue());
        }
    }
}
