 ...
 *
 * <p>Instruments the given class to print out the number of Jimple goto statements executed.
 *
 * <p>To enable this class, enable the given PackAdjuster by compiling it separately, into the soot
 * package.
 */
class GotoInstrumenter extends BodyTransformer {
    ...
    private void addStmtsToBefore(
        ...
        units.insertBefore(
                Jimple.v()
                        .newAssignStmt(
                                tmpLong, Jimple.v().newStaticFieldRef(gotoCounter.makeRef())),
                s);
    ...
    protected void internalTransform(Body body, String phaseName, Map options) {
        ...
        // gotoCounter (this only works in simple cases, because you may have multiple returns or
        // System.exit()'s )
        synchronized (this) {
        ...
        // Add code to increase goto counter each time a goto is encountered
        {
            boolean isMainMethod =
                    body.getMethod().getSubSignature().equals("void main(java.lang.String[])");
