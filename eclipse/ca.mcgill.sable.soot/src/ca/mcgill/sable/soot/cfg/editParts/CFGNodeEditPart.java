...
public class CFGNodeEditPart extends AbstractGraphicalEditPart
    ...
    public void applyGraphResults(DirectedGraph graph, HashMap map) {
        ...
        ((CFGNodeFigure) getFigure())
                .setBounds(
                        new Rectangle(
                                node.x,
                                node.y,
                                node.width,
                                node.height)); // getFigure().getBounds().height));//getFigure().getBounds().height));
        List outgoing = getSourceConnections();
        for (int i = 0; i < outgoing.size(); i++) {
            CFGEdgeEditPart edge = (CFGEdgeEditPart) outgoing.get(i);
