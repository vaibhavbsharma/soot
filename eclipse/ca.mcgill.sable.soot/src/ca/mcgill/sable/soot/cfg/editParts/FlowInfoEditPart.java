...
public class FlowInfoEditPart extends AbstractGraphicalEditPart implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getPropertyName().equals(CFGElement.FLOW_INFO)) {
            ...
            ((Label) getFigure())
                    .setForegroundColor(
                            SootPlugin.getDefault().getColorManager().getColor(new RGB(0, 153, 0)));
            ((Label) getFigure())
                    .setSize(
                            evt.getNewValue().toString().length() * 7,
                            getFigure().getBounds().height);
            ((PartialFlowDataEditPart) getParent())
                    .updateSize(evt.getNewValue().toString().length() * 7 + 10);
        }
    }
    ...
    public void resetColors() {
        ((Label) getFigure())
                .setForegroundColor(
                        SootPlugin.getDefault().getColorManager().getColor(new RGB(0, 0, 0)));
    }
