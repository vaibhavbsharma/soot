...
/**
 * This adds a classpath variable "SOOTCLASSES" to the eclipse environment. The variable points to
 * lib/sootclasses.jar in the plugin ca.mcgill.sable.soot.lib.
 *
 * @author Eric Bodden
 */
public class SootClasspathVariableInitializer extends ClasspathVariableInitializer {
    ...
    public void initialize(String variable) {
        if (variable.equals(VARIABLE_NAME_CLASSES)) { // $NON-NLS-1$
            ...
            try {
                JavaCore.setClasspathVariable(
                        VARIABLE_NAME_CLASSES, // $NON-NLS-1$
                        new Path(jarPath),
        ...
        if (variable.equals(VARIABLE_NAME_SOURCE)) { // $NON-NLS-1$
            ...
            try {
                JavaCore.setClasspathVariable(
                        VARIABLE_NAME_SOURCE, // $NON-NLS-1$
                        new Path(jarPath),
                        new NullProgressMonitor());
            } catch (JavaModelException e) {
                e.printStackTrace();
    ...
    private static String getSootFilePath(String relativePath) {
        ...
        Bundle ajdeBundle = Platform.getBundle(LIBRARY_PLUGIN_NAME);
        ...
        if (new File(cpath.toString()).exists()) {
            ...
            IPath rootPath = SootPlugin.getWorkspace().getRoot().getLocation();
            ...
            if (new File(cpath.toString()).exists()) sootJarPath = cpath.toString();
        }
