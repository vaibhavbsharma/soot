                    ...
                    if ((flags & IResourceDelta.CONTENT) != 0) {
                        if (delta.getResource() instanceof IFile) {
                            SootPlugin.getDefault()
                                    .getManager()
                                    .updateFileChangedFlag((IFile) delta.getResource());
                            String fileExtension =
                                    delta.getResource().getFullPath().getFileExtension();
                            if (fileExtension != null
                                    && fileExtension.equals(SootResourceManager.JIMPLE_FILE_EXT)) {
    ...
    private void updateJimpleOutline(IFile file) {
        ...
        for (int i = 0; i < refs.length; i++) {
            ...
            if (refs[i].getName().equals(file.getName())) {
                JimpleEditor ed =
                        (JimpleEditor) refs[i].getEditor(true).getAdapter(JimpleEditor.class);
                if (ed != null) {
                    if (ed.getPage() != null) {
