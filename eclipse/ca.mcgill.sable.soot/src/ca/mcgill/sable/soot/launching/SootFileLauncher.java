...
/** Soot Launcher for files. Determines file type, project classpath, src-precedence */
public class SootFileLauncher extends SootLauncher {
    ...
    public void handleFiles(Object toProcess) {
        ...
        if (toProcess instanceof IClassFile) {
            ...
            IPackageFragmentRoot pfr =
                    (IPackageFragmentRoot) cf.getAncestor(IJavaElement.PACKAGE_FRAGMENT_ROOT);
            ...
            if (pf.isDefaultPackage()) {
                ...
                getToProcessList()
                        .add(pf.getElementName() + "." + removeFileExt(cf.getElementName()));
            }
            ...
            if (fileExtension != null) {
                if (fileExtension.compareTo("jimple") == 0) {
                    setClasspathAppend(
                            platform_location + file.getParent().getFullPath().toOSString());
                    addJars();
                    setIsSrcPrec(true);
    ...
    private void handleSourceFile(ICompilationUnit cu) {
        IPackageFragmentRoot pfr =
                (IPackageFragmentRoot) cu.getAncestor(IJavaElement.PACKAGE_FRAGMENT_ROOT);
        ...
        if (isSrcPrec() && getSrcPrec().equals("java")) {
            ...
            try {
                ...
                IFolder output =
                        proj.getFolder(cu.getJavaProject().getOutputLocation().lastSegment());
                IPackageFragment pkf =
                        (IPackageFragment) cu.getAncestor(IJavaElement.PACKAGE_FRAGMENT);
                IFile exists = null;
                ...
                if (!exists.exists()) {
                    ...
                    MessageDialog noClassFound =
                            new MessageDialog(
                                    window.getShell(),
                                    "Soot Information",
                                    null,
                                    "No underlying class file was found, maybe build project.",
                                    0,
                                    new String[] {"OK"},
                                    0);
                    noClassFound.open();
                ...
                setClasspathAppend(
                        platform_location + cu.getJavaProject().getOutputLocation().toOSString());
    ...
    private void handleClassFile(IFile file) {
        ...
        if (!cf.loadClassFile(fis)) {
            ...
            MessageDialog noClassFound =
                    new MessageDialog(
                            ...
                            "Soot Information",
                            null,
                            "Could not determine package for class file will not continue.",
                            0,
                            new String[] {"OK"},
                            0);
            noClassFound.open();
        ...
        setClasspathAppend(
                file.getLocation()
                        .toOSString()
                        .substring(0, file.getLocation().toOSString().indexOf(cf.toString())));
        addJars();
    }
    ...
    public void setClasspathAppend(String ca) {
        ...
        if ((this.classpathAppend == null) || (this.classpathAppend.equals(""))) {
            ...
            if (this.classpathAppend.indexOf(ca) == -1) {
                this.classpathAppend =
                        this.classpathAppend + getSootClasspath().getSeparator() + ca;
            }
        }
    }
