...
/**
 * After Soot runs handles any new dava files by either copying them to an already existing "Dava
 * Project" or creating a "Dava Project" and copying the files there
 *
...
public class DavaHandler {
    ...
    private void createBeforeList() {
        try {
            IResource[] elems =
                    getSootOutputFolder()
                            .getFolder(Messages.getString("DavaHandler.dava"))
                            .getFolder(Messages.getString("DavaHandler.src"))
                            .members();
    ...
    public void handleAfter() {
            ...
            for (int i = 0; i < elems.length; i++) {
                if (getBeforeList() == null) {
                    ...
                    if (elems[i] instanceof IFile) {
                        ...
                        if (SootPlugin.getDefault()
                                .getManager()
                                .isFileMarkersRemove((IFile) elems[i])) {
                            newMembers.add(elems[i]);
                            // this sets changed bit to 0 - so file doesn't stay on list
                            // indefinitely
        ...
        if (!newMembers.isEmpty()) {
            ...
            if (davaProjectExists()) {
                setDavaProj(
                        JavaCore.create(
                                SootPlugin.getWorkspace().getRoot().getProject(getDavaProjName())));
    ...
    private boolean createSpecialDavaProject(IPath jreLibPath) {
        ...
        MessageDialog create =
                new MessageDialog(
                        window.getShell(),
                        Messages.getString("DavaHandler.Soot_Question"),
                        null,
                        Messages.getString(
                                "DavaHandler.Would_you_like_to_create_a_new_Dava_Project_with_generated_Dava_src_files"),
                        0,
                        new String[] {
                            Messages.getString("DavaHandler.OK"),
                            Messages.getString("DavaHandler.Cancel")
                        },
                        0);
        create.open();
        if (create.getReturnCode() == Dialog.OK) {
            ...
            if (!proj.exists()) {
                try {
                    ...
                    String[] natures =
                            new String[] {Messages.getString("org.eclipse.jdt.core.javanature")};
                    ...
                    IFolder folder =
                            proj.getFolder(Messages.getString("DavaHandler.src")); // $NON-NLS-1$
                    if (!folder.exists()) {
                        folder.create(false, true, null);
                    }
                    setSrcFolder(folder);
                    IFolder out =
                            proj.getFolder(Messages.getString("DavaHandler.bin")); // $NON-NLS-1$
                    if (!folder.exists()) {
                        folder.create(false, true, null);
                    }
    ...
    private boolean shouldCopyFiles() {
        ...
        MessageDialog copy =
                new MessageDialog(
                        ...
                        null,
                        Messages.getString(
                                "DavaHandler.Would_you_like_to_copy_Dava_src_files_to_the_Dava_Project"),
                        0,
                        new String[] {
                            Messages.getString("DavaHandler.OK"),
                            Messages.getString("DavaHandler.Cancel")
