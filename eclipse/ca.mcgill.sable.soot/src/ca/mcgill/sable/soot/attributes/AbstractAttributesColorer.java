...
public abstract class AbstractAttributesColorer {
            ...
            getDisplay()
                    .syncExec(
                            new Runnable() {
                                public void run() {
                                    IRegion reg =
                                            ((AbstractTextEditor) getEditorPart())
                                                    .getHighlightRange();
                                    ((AbstractTextEditor) getEditorPart())
                                            .setInput(getEditorPart().getEditorInput());
                                    if (reg != null) {
                                        ((AbstractTextEditor) getEditorPart())
                                                .setHighlightRange(
                                                        reg.getOffset(), reg.getLength(), true);
                                    }
                                };
                            });
        }
                            ...
                            public void run() {
                                for (int i = 0; i < srs.length; i++) {
                                    try {
                                        ...
                                        System.out.println(
                                                "Seting Style Range Ex: " + e.getMessage());
                                    }
                                }
                            };
                        });
    ...
    protected void setAttributeTextColor(
            int sline, int eline, int spos, int epos, RGB colorKey, boolean fg) {
        int sLineOffset = 0;
        int eLineOffset = 0;
