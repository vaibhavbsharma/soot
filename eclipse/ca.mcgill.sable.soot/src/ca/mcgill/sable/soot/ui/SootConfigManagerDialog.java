...
public class SootConfigManagerDialog extends TitleAreaDialog implements ISelectionChangedListener {
    ...
    protected void configureShell(Shell shell) {
        ...
        shell.setText(
                Messages.getString("SootConfigManagerDialog.Manage_Configurations")); // $NON-NLS-1$
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        setTitle(
                Messages.getString(
                        "SootConfigManagerDialog.Soot_Configurations_Manager")); // $NON-NLS-1$
    ...
    private SootConfiguration getInitialInput() {
        ...
        try {
            numConfig =
                    settings.getInt(
                            Messages.getString(
                                    "SootConfigManagerDialog.config_count")); // $NON-NLS-1$
        ...
        if (numConfig != 0) {
            ...
            for (int i = 0; i < numConfig; i++) {
                configNames[i] =
                        settings.get(
                                Messages.getString("SootConfigManagerDialog.soot_run_config")
                                        + (i + 1)); // $NON-NLS-1$
    ...
    protected Control createSpecialButtonBar(Composite parent) {
        ...
        GridData data =
                new GridData(GridData.VERTICAL_ALIGN_END | GridData.HORIZONTAL_ALIGN_CENTER);
    ...
    protected Button createSpecialButton(
            Composite parent, int id, String label, boolean defaultButton, boolean enabled) {
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        ...
        Button runButton =
                createButton(
                        parent,
                        5,
                        Messages.getString("SootConfigManagerDialog.Run"),
                        false); // $NON-NLS-1$
        ...
        createButton(
                parent,
                6,
                Messages.getString("SootConfigManagerDialog.Close"),
                true); // $NON-NLS-1$
    }
    ...
    protected void createSpecialButtonsForButtonBar(Composite parent) {
        createSpecialButton(
                parent,
                0,
                Messages.getString("SootConfigManagerDialog.New"),
                false,
                true); // $NON-NLS-1$
        createSpecialButton(
                parent,
                1,
                Messages.getString("SootConfigManagerDialog.Edit"),
                false,
                false); // $NON-NLS-1$
        createSpecialButton(
                parent,
                2,
                Messages.getString("SootConfigManagerDialog.Delete"),
                false,
                false); // $NON-NLS-1$
        createSpecialButton(
                parent,
                3,
                Messages.getString("SootConfigManagerDialog.Rename"),
                false,
                false); // $NON-NLS-1$
        createSpecialButton(
                parent,
                4,
                Messages.getString("SootConfigManagerDialog.Clone"),
                false,
                false); // $NON-NLS-1$
    }
    ...
    private void newPressed() {
        ...
        InputDialog nameDialog =
                new InputDialog(
                        this.getShell(),
                        Messages.getString("SootConfigManagerDialog.Saving_Configuration_Name"),
                        Messages.getString(
                                "SootConfigManagerDialog.Enter_name_to_save_configuration_with"),
                        "",
                        validator); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        nameDialog.open();
    ...
    private void deletePressed() {
        ...
        MessageDialog msgDialog =
                new MessageDialog(
                        this.getShell(),
                        Messages.getString(
                                "SootConfigManagerDialog.Soot_Configuration_Remove_Message"),
                        null,
                        Messages.getString(
                                "SootConfigManagerDialog.Are_you_sure_you_want_to_remove_this_configuration"),
                        0,
                        new String[] {
                            Messages.getString("SootConfigManagerDialog.Yes"),
                            Messages.getString("SootConfigManagerDialog.No")
                        },
    ...
    private void renamePressed() {
        ...
        InputDialog nameDialog =
                new InputDialog(
                        this.getShell(),
                        Messages.getString("SootConfigManagerDialog.Rename_Saved_Configuration"),
                        Messages.getString("SootConfigManagerDialog.Enter_new_name"),
                        "",
                        validator); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        nameDialog.open();
        if (nameDialog.getReturnCode() == Dialog.OK) {
            settings.put(
                    Messages.getString("SootConfigManagerDialog.soot_run_config") + oldNameCount,
                    nameDialog.getValue()); // $NON-NLS-1$
    ...
    private void clonePressed() {
        ...
        InputDialog nameDialog =
                new InputDialog(
                        this.getShell(),
                        Messages.getString("SootConfigManagerDialog.Clone_Saved_Configuration"),
                        Messages.getString("SootConfigManagerDialog.Enter_new_name"),
                        result,
                        validator); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        nameDialog.open();
        if (nameDialog.getReturnCode() == Dialog.OK) {
            config_count++;
            settings.put(
                    Messages.getString("SootConfigManagerDialog.soot_run_config") + config_count,
                    nameDialog.getValue()); // $NON-NLS-1$
            settings.put(nameDialog.getValue(), settings.getArray(result));
            settings.put(
                    Messages.getString("SootConfigManagerDialog.config_count"),
                    config_count); // $NON-NLS-1$
            getTreeRoot().addChild(new SootConfiguration(nameDialog.getValue()));
            saveMainClass(nameDialog.getValue(), settings.get(result + "_mainClass"));
        }
        refreshTree();
