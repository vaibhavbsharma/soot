...
public abstract class AbstractOptionsDialog extends TitleAreaDialog
        ...
        if (getRadioGroups() != null) {
            ...
            while (it.hasNext()) {
                ...
                while (itButtons.hasNext()) {
                    if (((BooleanOptionWidget) itButtons.next())
                            .getButton()
                            .equals(e.getSource())) {
    ...
    private EnableGroup findEnableGroup(String phaseAlias) {
        ...
        while (it.hasNext()) {
            ...
            if (next.getPhaseAlias().equals(phaseAlias) && (next.getSubPhaseAlias() == null))
                return next;
        }
        return null;
    }

    protected void addToEnableGroup(
            String phaseAlias, String subPhaseAlias, ISootOptionWidget widget, String alias) {
        ...
        while (it.hasNext()) {
            ...
            if (eGroup.isPhaseOptType()) {
                ...
                if (eGroup.getLeader().getButton().getSelection()
                        && eGroup.getLeader().getButton().isEnabled()) {
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        if (isCanRun()) {
            createButton(
                    parent,
                    1,
                    Messages.getString("AbstractOptionsDialog.Run"),
                    true); // $NON-NLS-1$
            createButton(
                    parent,
                    2,
                    Messages.getString("AbstractOptionsDialog.Close"),
                    false); // $NON-NLS-1$
        } else {
            createButton(
                    parent,
                    0,
                    Messages.getString("AbstractOptionsDialog.Save"),
                    true); // $NON-NLS-1$
            createButton(
                    parent,
                    2,
    ...
    private Composite sootMainClassCreate(Composite parent) {
...

        {
            ...
            setSootMainClassWidget(
                    new StringOptionWidget(
                            editGroupSootMainClass,
                            SWT.NONE,
                            new OptionData(
                                    "Soot Main Class",
                                    "",
            ...
            setSootMainProjectWidget(
                    new StringOptionWidget(
                            ...
                            new OptionData(
                                    ...
                                    "",
                                    "sootMainProject",
                                    "\nThe Java project holding the main class.",
                                    defaultString)));
        }
