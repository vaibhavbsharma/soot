 ...
 * classes GrimpBody, JimpleBody and BafBody all extend this class. This class provides methods that
 * are common to any IR, such as methods to get the body's units (statements), traps, and locals.
 *
...
public abstract class Body extends AbstractHost implements Serializable {
  ...
  public SootMethod getMethod() {
    if (method == null) throw new RuntimeException("no method associated w/ body");
    return method;
  }
  ...
  public Local getParameterLocal(int i) {
    for (Unit s : getUnits()) {
      if (s instanceof IdentityStmt && ((IdentityStmt) s).getRightOp() instanceof ParameterRef) {
        ...
        if (pr.getIndex() == i) return (Local) is.getLeftOp();
      }
    }
  ...
  /**
   * Returns the list of parameter references used in this body. The list is as long as the number
   * of parameters declared in the associated method's signature. The list may have <code>null
   * </code> entries for parameters not referenced in the body. The returned list is of fixed size.
   */
  ...
  /**
   * Returns the Chain of Units that make up this body. The units are returned as a PatchingChain.
   * The client can then manipulate the chain, adding and removing units, and the changes will be
   * reflected in the body. Since a PatchingChain is returned the client need <i>not</i> worry about
  ...
  /**
   * Returns the result of iterating through all Units in this body and querying them for their
  ...
  public List<UnitBox> getAllUnitBoxes() {
...

    {
      ...
      while (it.hasNext()) {
        Tag t = it.next();
        if (t instanceof CodeAttribute) unitBoxList.addAll(((CodeAttribute) t).getUnitBoxes());
      }
    }
  ...
  /**
   * If branchTarget is true, returns the result of iterating through all branching Units in this
  ...
  /**
   * Returns the result of iterating through all Units in this body and querying them for ValueBoxes
   * used. All of the ValueBoxes found are then returned as a List.
   *
  ...
  /**
   * Returns the result of iterating through all Units in this body and querying them for ValueBoxes
   * defined. All of the ValueBoxes found are then returned as a List.
   *
  ...
  /**
   * Returns a list of boxes corresponding to Values either used or defined in any unit of this
   * Body.
   *
  ...
  public long getModificationCount() {
    return localChain.getModificationCount()
        + unitChain.getModificationCount()
