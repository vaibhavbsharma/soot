    ...
    while (it.hasNext()) {
      ...
      while (mIt.hasNext()) {
        ...
        if (subSig.equals("void main(java.lang.String[])") && m.isPublic() && m.isStatic()) {
          ...
          Iterator<SootClass> cIt =
              hierarchy.getSuperclassesOfIncluding(cOrigSuperclass).iterator();
          while (cIt.hasNext()) {
        ...
        while (keys.hasMoreElements()) {
          ...
          { // build new junk method to call original method
            ...
            newM =
                Scene.v()
                    .makeSootMethod(
                        newMName, paramTypes, rType, oldM.getModifiers(), oldM.getExceptions());
            iC.addMethod(newM);
          ...
          { // build copy of old method
            ...
            if (rType instanceof VoidType) {
              ...
              units.add(
                  Jimple.v()
                      .newAssignStmt(
                          loc, Jimple.v().newVirtualInvokeExpr(ths, newM.makeRef(), args)));
              ...
              if (v instanceof SpecialInvokeExpr) {
                ...
                if (sie.getBase().equivTo(thisLocal)
                  ...
                  if (!iC.declaresMethod("<init>", smr.parameterTypes())) {
                    ...
                    List<Local> args =
                        BodyBuilder.buildParameterLocals(initUnits, locals, paramTypes);

                    initUnits.add(
                        Jimple.v().newInvokeStmt(Jimple.v().newSpecialInvokeExpr(ths, smr, args)));
