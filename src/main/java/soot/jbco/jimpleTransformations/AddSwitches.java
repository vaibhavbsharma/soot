...
public class AddSwitches extends BodyTransformer implements IJbcoTransform {
  ...
  protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
    ...
    if (ops[0].getType() instanceof PrimType) {
      units.insertBefore(
          Jimple.v().newAssignStmt(b1, Jimple.v().newStaticFieldRef(ops[0].makeRef())), u);
    } else {
      ...
      locals.add(B);
      units.insertBefore(
          Jimple.v().newAssignStmt(B, Jimple.v().newStaticFieldRef(ops[0].makeRef())), u);
      units.insertBefore(
          Jimple.v()
              .newAssignStmt(
                  b1, Jimple.v().newVirtualInvokeExpr(B, m.makeRef(), Collections.emptyList())),
          u);
    }
    if (ops[1].getType() instanceof PrimType) {
      units.insertBefore(
          Jimple.v().newAssignStmt(b2, Jimple.v().newStaticFieldRef(ops[1].makeRef())), u);
    } else {
      ...
      locals.add(B);
      units.insertBefore(
          Jimple.v().newAssignStmt(B, Jimple.v().newStaticFieldRef(ops[1].makeRef())), u);
      units.insertBefore(
          Jimple.v()
              .newAssignStmt(
                  b2, Jimple.v().newVirtualInvokeExpr(B, m.makeRef(), Collections.emptyList())),
          u);
    }
    ...
    while (tit.hasNext()) {
      ...
      if (Rand.getInt(5) < 4) {
        units.insertBefore(
            Jimple.v()
                .newAssignStmt(l, Jimple.v().newAddExpr(l, IntConstant.v(Rand.getInt(3) + 1))),
            nxt);
      }
    }
