            ...
            if (max != 0) {
              ...
              if (shft_rem[0] != null
                ...
                if (shft_rem[1] != null) { // if there is an additive floating
                  // component
                  Local tmp2 = null,
                      tmp1 = Jimple.v().newLocal("__tmp_shft_lcl" + localCount++, opType);
                  locals.add(tmp1);

                  // shift the integral portion
                  Unit newU =
                      Jimple.v()
                          .newAssignStmt(tmp1, Jimple.v().newShlExpr(op, IntConstant.v(shift)));
                  unitsBuilt.add(newU);
                  ...
                  if (rem != 1) {
                    ...
                    if (nc instanceof DoubleConstant && !(opType instanceof DoubleType)) {
                      ...
                      newU =
                          Jimple.v()
                              .newAssignStmt(tmp2, Jimple.v().newCastExpr(op, DoubleType.v()));
                      unitsBuilt.add(newU);
                      units.insertBefore(newU, u);
                  ...
                  if (tmp2 == null) {
                    ...
                    Local tmp3 =
                        Jimple.v().newLocal("__tmp_shft_lcl" + localCount++, tmp2.getType());
                    locals.add(tmp3);

                    newU =
                        Jimple.v()
                            .newAssignStmt(tmp3, Jimple.v().newCastExpr(tmp1, tmp2.getType()));
                    unitsBuilt.add(newU);
                    units.insertBefore(newU, u);
                ...
                if (neg) {
                  Unit newU =
                      Jimple.v()
                          .newAssignStmt(as.getLeftOp(), Jimple.v().newNegExpr(as.getLeftOp()));
                  unitsBuilt.add(newU);
                  units.insertAfter(newU, u);
                }
