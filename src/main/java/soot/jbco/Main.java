    ...
    if (!metrics) {
      ...
      for (int i = 0; i < transformsToAdd.size(); i++) {
        if (transformsToAdd.get(i).startsWith("bb")) {
          ...
          bb.insertBefore(
              new Transform(
                  "bb.jbco_j2bl", newTransform((Transformer) getTransform("bb.jbco_j2bl"))),
              "bb.lso");
          bb.insertBefore(
              new Transform("bb.jbco_ful", newTransform((Transformer) getTransform("bb.jbco_ful"))),
              "bb.lso");
          bb.add(
              new Transform(
                  "bb.jbco_rrps", newTransform((Transformer) getTransform("bb.jbco_rrps"))));
      ...
      if (jbcoSummary) {
        for (int i = 0; i < 3; i++) {
          Iterator<Transform> phases =
              i == 0 ? wjtp.iterator() : i == 1 ? jtp.iterator() : bb.iterator();
          G.v()
              .out
              .println(
                  i == 0
    ...
    if (htmp != null) {
      ...
      while (keys.hasNext()) {
        ...
        if (o instanceof java.util.regex.Pattern) {
          if (((java.util.regex.Pattern) o).matcher(method).matches()) intg = (Integer) htmp.get(o);
