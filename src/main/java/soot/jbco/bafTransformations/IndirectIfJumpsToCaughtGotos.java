...
public class IndirectIfJumpsToCaughtGotos extends BodyTransformer implements IJbcoTransform {
  ...
  protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
    ...
    if (nonTrap == null) {
      ...
      for (Iterator<Unit> it = units.iterator(); it.hasNext(); ) {
        ...
        if (u instanceof IdentityInst && ((IdentityInst) u).getLeftOp() instanceof Local) {
          ...
          if (last != null) units.insertAfter(nonTrap, last);
          else units.addFirst(nonTrap);
          break;
        }
      }
    }
    ...
    while (stack.size() > 0) toinserttry.add(Baf.v().newPopInst(stack.pop()));
    ...
    b.getTraps()
        .add(Baf.v().newTrap(ThrowSet.getRandomThrowable(), addedUnits.get(0), nop, handler));
    ...
    if (addedUnits.size() > 0 && debug) {
      ...
      // StackTypeHeightCalculator.calculateStackHeights(b), false);
    }
  }
    ...
    if (untrapped.size() > 0) {
      ...
      while (result == null && count < 10) {
        ...
        if (!result.fallsThrough()
            || units.getSuccOf(result) == null
            || units.getSuccOf(result) instanceof ThrowInst) result = null;
      }
    }
