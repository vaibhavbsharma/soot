...
public class TryCatchCombiner extends BodyTransformer implements IJbcoTransform {
  ...
  protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
    ...
    while (traps.hasNext()) {
      ...
      if (s.size() > 0) {
        for (int i = 0; i < s.size(); i++) {
          ...
          units.insertBeforeNoRedirect(
              FixUndefinedLocals.getPushInitializer(varLocal, type), first);
      ...
      if (Rand.getInt() % 2 == 0) {
        units.insertBeforeNoRedirect(
            Baf.v().newPushInst(IntConstant.v(Rand.getInt(3) + 1)), begUnit);
        units.insertBeforeNoRedirect(Baf.v().newStoreInst(IntType.v(), controlLocal), begUnit);
      } else {
        units.insertBeforeNoRedirect(
            Baf.v().newIncInst(controlLocal, IntConstant.v(Rand.getInt(3) + 1)), begUnit);
      }
  ...
  private void loadBooleanValue(PatchingChain<Unit> units, SootField f, Unit insert) {
    ...
    if (f.getType() instanceof RefType) {
      SootMethod boolInit =
          ((RefType) f.getType()).getSootClass().getMethod("boolean booleanValue()");
