...
public interface Chain<E> extends Collection<E>, Serializable {
  ...
  /**
   * Inserts <code>toInsert</code> in the Chain before <code>point</code>. (It would probably be
   * better to make Chain implement List)
   */
  void insertBefore(Chain<E> toInsert, E point);

  /**
   * Inserts <code>toInsert</code> in the Chain after <code>point</code>. (It would probably be
   * better to make Chain implement List)
   */
   ...
   */
  void insertOnEdge(E toInsert, E point_src, E point_tgt);
   ...
   * @param point_tgt the target point of an edge
   */
  void insertOnEdge(List<E> toInsert, E point_src, E point_tgt);
   ...
   * @param point_tgt the target point of an edge
   */
  void insertOnEdge(Chain<E> toInsert, E point_src, E point_tgt);
  ...
  /**
   * Returns true if object <code>someObject</code> follows object <code>someReferenceObject
   * </code> in the Chain.
   */
  ...
  /**
   * Returns an iterator over a copy of this chain. This avoids ConcurrentModificationExceptions
   * from being thrown if the underlying Chain is modified during iteration. Do not use this to
   * remove elements which have not yet been iterated over!
   */
  ...
  /**
   * Gets all elements in the chain. There is no guarantee on sorting. On the other hand, the
   * collection returned by this method is thread-safe. You can iterate over it even in the case of
   * concurrent modifications to the underlying chain.
   *
