...
public class CFGOptionMatcher {
  ...
  /**
   * Searches the options in this <code>CFGOptionMatcher</code> looking for one whose name begins
   * with the passed string (ignoring the case of letters in the string).
   *
      ...
      if (uncasedName.startsWith(uncasedQuarry)) {
        if (match == -1) {
          ...
          throw new CompilationDeathException(
              CompilationDeathException.COMPILATION_ABORTED, "Option parse error");
        }
      }
    }
    if (match == -1) {
  ...
  /**
   * Returns a string containing the names of all the options in this <code>CFGOptionMatcher
