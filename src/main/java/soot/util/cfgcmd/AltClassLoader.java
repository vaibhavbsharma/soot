...
public class AltClassLoader extends ClassLoader {
  ...
  /**
   * Constructs an <code>AltClassLoader</code> for inclusion in Soot's global variable manager,
   * {@link G}.
   *
   ...
   * classpath set by the most recent call to its {@link #setAltClassPath}.
   *
  ...
  /**
   * Mangles a classname so that it will not be found on the system classpath by our parent class
   * loader, even if there is a class with the original name there. We use a crude heuristic to do
   * this that happens to work with the names we have needed to mangle to date. The heuristic
   * requires that <code>origName</code> include at least two dots (i.e., the class must be in a
   * package, where the package name has at least two components). More sophisticated possibilities
   * certainly exist, but they would require more thorough parsing of the class file.
   *
  ...
  /**
   * Loads a class, from the alternate classpath if the class's name has been included in the list
   * of alternate classes with {@link #setAltClasses(String[]) setAltClasses()}, from the regular
   * system classpath otherwise. When a alternate class is loaded, its references to other alternate
   * classes are also resolved to the alternate classpath.
   *
  ...
  public Class<?> loadClass(String name) throws ClassNotFoundException {
    ...
    if (DEBUG) {
      G.v().out.println("AltClassLoader.loadClass asking parent for " + nameForParent);
    }
    return super.loadClass(nameForParent, false);
  }

  /**
   * Replaces any occurrences in <code>classBytes</code> of classnames to be loaded from the
   * alternate class path with the corresponding mangled names. Of course we should really parse the
   * class pool properly, since the simple-minded, brute force replacment done here could produce
   * problems with some combinations of classnames and class contents. But we've got away with this
   * so far!
   */
