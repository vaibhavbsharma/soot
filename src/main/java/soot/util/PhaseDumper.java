 ...
 * the bodies being manipulated by the phase is dumped before and after the phase is applied. If a
 * phase is on the <code>cfgDumpingPhases</code> list, then whenever a CFG is constructed during the
...
public class PhaseDumper {
  ...
  /**
   * Tells the <code>PhaseDumper</code> that a {@link Body} transforming phase has started, so that
  ...
  public void dumpAfter(Body b, String phaseName) {
    ...
    if (poppedPhaseName != phaseName) {
      throw new IllegalArgumentException(
          "dumpAfter(" + phaseName + ") when poppedPhaseName == " + poppedPhaseName);
    }
    if (isBodyDumpingPhase(phaseName)) {
  ...
  /**
   * Tells the <code>PhaseDumper</code> that a {@link Scene} transforming phase has started, so that
   * it can dump the phases's &ldquo;before&rdquo; files. If the phase is to be dumped, <code>
   * dumpBefore</code> deletes any old graph files dumped during previous runs of the phase.
   *
   * @param phaseName the name of the phase that has just started.
   */
  ...
  /**
   * Tells the <code>PhaseDumper</code> that a {@link Scene} transforming phase has ended, so that
   * it can dump the phases's &ldquo;after&rdquo; files.
   *
   * @param phaseName the name of the phase that has just ended.
   * @throws IllegalArgumentException if <code>phaseName</code> does not match the <code>
   *     PhaseDumper</code>'s record of the current phase.
  ...
  /**
   * Asks the <code>PhaseDumper</code> to dump the passed {@link ExceptionalGraph} if the current
   * phase is being dumped.
   *
   * @param g the graph to dump.
   */
  public void dumpGraph(ExceptionalGraph g) {
    ...
    try {
      ...
      if (isCFGDumpingPhase(phaseName)) {
        try {
          String outputFile =
              nextGraphFileName(g.getBody(), phaseName + "-" + getClassIdent(g) + "-");
  ...
  /**
   * Prints the current stack trace, as a brute force tool for program understanding. This method
   * appeared in response to the many times dumpGraph() was being called while the phase stack was
   * empty. Turned out that the Printer needs to build a BriefUnitGraph in order to print a graph.
   * Doh!
   */
