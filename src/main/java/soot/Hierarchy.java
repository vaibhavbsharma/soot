...
public class Hierarchy {
  ...
  /**
   * Returns true if child is a subclass of possibleParent. If one of the known parent classes is
   * phantom, we conservatively assume that the current class might be a child.
   */
  public boolean isClassSubclassOf(SootClass child, SootClass possibleParent) {
    ...
    if (parentClasses.contains(possibleParent)) return true;

    for (SootClass sc : parentClasses) if (sc.isPhantom()) return true;
   ...
   * classes is phantom, we conservatively assume that the current class might be a child.
   */
  ...
  /**
   * Checks whether check is a visible class in view of the from class. It assumes that protected
   * and private classes do not exit. If they exist and check is either protected or private, the
   * check will return false.
   */
  public boolean isVisible(SootClass from, SootClass check) {
    if (check.isPublic()) return true;

    if (check.isProtected() || check.isPrivate()) return false;
  ...
  public boolean isVisible(SootClass from, ClassMember m) {
    ...
    if (!isVisible(from, m.getDeclaringClass())) return false;
  ...
  public List<SootMethod> resolveAbstractDispatch(SootClass c, SootMethod m) {
    ...
    if (c.isInterface()) {
      ...
      classesIt = classes;
    } else classesIt = getSubclassesOfIncluding(c);
  ...
  public SootMethod resolveSpecialDispatch(SpecialInvokeExpr ie, SootMethod container) {
     ...
     * invokespecial.
     */
    if ("<init>".equals(target.getName()) || target.isPrivate()) return target;
