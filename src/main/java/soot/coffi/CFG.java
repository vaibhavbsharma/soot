...
public class CFG {
  ...
  private void buildBBCFG() {
    ...
    for (int i = 0; i < ca.exception_table_length; i++) {
      ...
      if (bb == null) {
        G.v()
            .out
            .println("Warning: No basic block found for" + " start of exception handler code.");
      } else {
  ...
  private Instruction findMatchingRet(
      ...
      // Initialize the 'this' variable
      {
        if (!isStatic) {
          ...
          units.add(
              Jimple.v()
                  .newIdentityStmt(
                      local, Jimple.v().newThisRef(jmethod.getDeclaringClass().getType())));
        }
      }
        ...
        while (!changedInstructions.isEmpty()) {
          ...
          for (Instruction s : successors) {
            if (!visitedInstructions.contains(s)) {
              ...
              if (handlerInstructions.contains(s)) {
                // The type stack for an instruction handler should always be that
                // of
        ...
        for (; ; ) {
          ...
          if (reachableInstructions.contains(ins))
            generateJimple(
                ...
                instructionToPostTypeStack.get(ins),
                constant_pool,
                statementsForIns,
                b);
      ...
      for (int i = 0; i < codeAttribute.exception_table_length; i++) {
        ...
        // Insert assignment of exception
        {
          ...
          if (targetToHandler.containsKey(firstTargetStmt))
            ...
            // changed to account for catch blocks which are also part of normal control
            // flow
            ...
            if (catchStart != null) {
              ...
              if (catchStart != units.getPredOf(afterEndStmt)) {
                throw new IllegalStateException(
                    "Assertion failure: catchStart != pred of afterEndStmt");
              }
  ...
  OutFlow processFlow(Instruction ins, TypeStack typeStack, cp_info[] constant_pool) {
    ...
    switch (x) {
      ...
      case ByteCode.LDC2:
      case ByteCode.LDC2W:
        return processCPEntry(
            constant_pool, ((Instruction_intindex) ins).arg_i, typeStack, jmethod);
  ...
  private Type jimpleTypeOfFieldInFieldRef(Scene cm, cp_info[] constant_pool, int index) {
    ...
    CONSTANT_NameAndType_info nat =
        (CONSTANT_NameAndType_info) (constant_pool[fr.name_and_type_index]);
  ...
  private Type jimpleReturnTypeOfNameAndType(Scene cm, cp_info[] constant_pool, int index) {
    ...
    String methodDescriptor =
        ((CONSTANT_Utf8_info) (constant_pool[nat.descriptor_index])).convert();
  ...
  private Type jimpleReturnTypeOfMethodRef(Scene cm, cp_info[] constant_pool, int index) {
    ...
    CONSTANT_NameAndType_info nat =
        (CONSTANT_NameAndType_info) (constant_pool[mr.name_and_type_index]);
  ...
  private OutFlow processCPEntry(
  ...
  /**
   * Runs through the given bbq contents performing the target fix-up pass; Requires all reachable
   * blocks to have their done flags set to true, and this resets them all back to false;
   *
  ...
  private void generateJimpleForCPEntry(
      cp_info constant_pool[],
      int i,
      TypeStack typeStack,
      TypeStack postTypeStack,
      SootMethod jmethod,
    ...
    if (c instanceof CONSTANT_Integer_info) {
      ...
      String className =
          ((CONSTANT_Utf8_info) (constant_pool[((CONSTANT_Class_info) c).name_index])).convert();
  ...
  void generateJimple(
    ...
    switch (x) {
      ...
      case ByteCode.LDC1:
        generateJimpleForCPEntry(
            constant_pool,
            ((Instruction_Ldc1) ins).arg_b,
            typeStack,
            postTypeStack,
            jmethod,
            statements);
      ...
      case ByteCode.LDC2W:
        generateJimpleForCPEntry(
            constant_pool,
            ((Instruction_intindex) ins).arg_i,
            typeStack,
            postTypeStack,
            jmethod,
            statements);
      ...
      case ByteCode.SASTORE:
        ...
        stmt =
            Jimple.v()
                .newAssignStmt(
                    a, Util.v().getLocalForStackOp(listBody, typeStack, typeStack.topIndex()));
        break;
      ...
      case ByteCode.DUP_X1:
        ...
        stmt =
            Jimple.v()
                .newAssignStmt(
                    Util.v()
                        .getLocalForStackOp(listBody, postTypeStack, postTypeStack.topIndex() - 1),
                    l2);
        ...
        stmt =
            Jimple.v()
                .newAssignStmt(
                    Util.v()
                        .getLocalForStackOp(listBody, postTypeStack, postTypeStack.topIndex() - 2),
                    Util.v().getLocalForStackOp(listBody, postTypeStack, postTypeStack.topIndex()));
          ...
          stmt =
              Jimple.v()
                  .newAssignStmt(
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 2),
                      l3);
          ...
          stmt =
              Jimple.v()
                  .newAssignStmt(
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 3),
                      l1);
          ...
          stmt =
              Jimple.v()
                  .newAssignStmt(
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 4),
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 1));
          ...
          stmt =
              Jimple.v()
                  .newAssignStmt(
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 5),
                      Util.v()
                          .getLocalForStackOp(
                              listBody, postTypeStack, postTypeStack.topIndex() - 1));
      ...
      case ByteCode.DNEG:
        rhs =
            Jimple.v()
                .newNegExpr(Util.v().getLocalForStackOp(listBody, typeStack, typeStack.topIndex()));
        stmt =
            Jimple.v()
                .newAssignStmt(
                    Util.v().getLocalForStackOp(listBody, postTypeStack, postTypeStack.topIndex()),
  ...
  private SootMethodRef createMethodRef(
      cp_info[] constant_pool, ICONSTANT_Methodref_info methodInfo, boolean isStatic) {
