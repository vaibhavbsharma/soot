...
abstract class Instruction implements Cloneable {
  ...
  /**
   * Assuming the actual bytecode for this instruction has been extracted already, and index is the
  ...
  /**
   * Changes offset values in this instruction to Instruction references; default behaviour is to do
   * nothing.
   *
   * @param bc complete array of bytecode.
   * @see ByteCode#build
  ...
  /**
   * Returns an array of the instructions to which this instruction might branch (only valid if
   * branches==<i>true</i>; default action is to return <i>null</i>).
   *
  ...
  /**
   * Marks the appropriate spot if that constant_pool entry is used by this instr. For every
   * constant pool entry used (referenced) by this instruction, the corresponding boolean in the
   * given array is set to <i>true</i>.
   *
