...
public class Util {
   ...
   * using getLocalForIndex(...) and getLocalForStackOp(...) each time a different current method
   * body is considered.
   */
  public void bodySetup(
      LocalVariableTable_attribute la, LocalVariableTypeTable_attribute lt, cp_info[] ca) {
  ...
  public void resolveFromClassFile(
    ...
    // Load up class file, and retrieve bclass from class manager.
    {
      ...
      if (!name.equals(bclass.getName())) {
        throw new RuntimeException(
            "Error: class "
                + name
                + " read in from a classfile in which "
                + bclass.getName()
                + " was expected.");
      }
    }
    ...
    // Set superclass
    {
      if (coffiClass.super_class != 0) {
        ...
        CONSTANT_Class_info c =
            (CONSTANT_Class_info) coffiClass.constant_pool[coffiClass.super_class];
    ...
    // Add interfaces to the bclass
    {
      for (int i = 0; i < coffiClass.interfaces_count; i++) {
        CONSTANT_Class_info c =
            (CONSTANT_Class_info) coffiClass.constant_pool[coffiClass.interfaces[i]];
    ...
    for (int i = 0; i < coffiClass.fields_count; i++) {
      ...
      String fieldName =
          ((CONSTANT_Utf8_info) (coffiClass.constant_pool[fieldInfo.name_index])).convert();

      String fieldDescriptor =
          ((CONSTANT_Utf8_info) (coffiClass.constant_pool[fieldInfo.descriptor_index])).convert();
      ...
      for (int j = 0; j < fieldInfo.attributes_count; j++) {
        ...
        // add signature tag
          ...
          String name =
              ((CONSTANT_Utf8_info) (coffiClass.constant_pool[attr.attribute_name])).convert();
    ...
    for (int i = 0; i < coffiClass.methods_count; i++) {
      ...
      String methodName =
          ((CONSTANT_Utf8_info) (coffiClass.constant_pool[methodInfo.name_index])).convert();
      String methodDescriptor =
          ((CONSTANT_Utf8_info) (coffiClass.constant_pool[methodInfo.descriptor_index])).convert();
        ...
        for (int j = 0; j < methodInfo.attributes_count; j++) {
          if (methodInfo.attributes[j] instanceof Exception_attribute) {
            ...
            for (int k = 0; k < exceptions.number_of_exceptions; k++) {
              CONSTANT_Class_info c =
                  (CONSTANT_Class_info)
                      coffiClass.constant_pool[exceptions.exception_index_table[k]];
  ...
  Local getLocalCreatingIfNecessary(JimpleBody listBody, String name, Type type) {
    ...
    if (l != null) {
      if (!l.getType().equals(type)) {
        throw new RuntimeException(
            "The body already declares this local name with a different type.");
      }
    } else {
  ...
  Local getLocalForIndex(JimpleBody listBody, int index, Instruction context) {
    return getLocalForIndex(
        ...
        context.nextOffset(context.originalIndex),
        ByteCode.isLocalStore(context.code));
  }

  private Local getLocalForIndex(
      JimpleBody listBody, int index, int bcIndex, int nextBcIndex, boolean isLocalStore) {
    String name = null;
    Local local;
  ...
  private void addAnnotationVisibilityAttribute(
      Host host, attribute_info attribute, ClassFile coffiClass, Collection<Type> references) {
  ...
  private void addAnnotationVisibilityParameterAttribute(
    ...
    if (attribute instanceof RuntimeVisibleParameterAnnotations_attribute) {
      RuntimeVisibleParameterAnnotations_attribute attr =
          (RuntimeVisibleParameterAnnotations_attribute) attribute;
      tag =
          new VisibilityParameterAnnotationTag(
              attr.num_parameters, AnnotationConstants.RUNTIME_VISIBLE);
      ...
      RuntimeInvisibleParameterAnnotations_attribute attr =
          (RuntimeInvisibleParameterAnnotations_attribute) attribute;
      tag =
          new VisibilityParameterAnnotationTag(
              attr.num_parameters, AnnotationConstants.RUNTIME_INVISIBLE);
      for (int i = 0; i < attr.num_parameters; i++) {
        parameter_annotation pAnnot = attr.parameter_annotations[i];
        VisibilityAnnotationTag vTag =
  ...
  private void addAnnotations(
    ...
    for (int i = 0; i < numAnnots; i++) {
      ...
      String annotType =
          ((CONSTANT_Utf8_info) coffiClass.constant_pool[annot.type_index]).convert();
      String ref = annotType.substring(1, annotType.length() - 1);
      ref = ref.replace('/', '.');
      references.add(RefType.v(ref));
      AnnotationTag annotTag =
          new AnnotationTag(
              annotType,
              createElementTags(
                  annot.num_element_value_pairs, coffiClass, annot.element_value_pairs));
