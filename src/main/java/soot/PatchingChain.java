...
/**
 * An implementation of a Chain which can contain only Units, and handles patching to deal with
 * element insertions and removals. This is done by calling Unit.redirectJumpsToThisTo at strategic
 * times.
 */
...
public class PatchingChain<E extends Unit> extends AbstractCollection<E> implements Chain<E> {
  ...
  /**
   * Returns the inner chain used by the PatchingChain. In general, this should not be used.
   * However, direct access to the inner chain may be necessary if you wish to perform certain
   * operations (such as control-flow manipulations) without interference from the patching
   * algorithms.
   */
  ...
  protected class PatchingIterator implements Iterator<E> {
    ...
    public void remove() {
      ...
      if ((successor = getSuccOf(lastObject)) == null) successor = getPredOf(lastObject);
      // Note that redirecting to the last unit in the method
      // like this is probably incorrect when dealing with a Trap.
      // I.e., let's say that the final unit in the method used to
      // be U10, preceded by U9, and that there was a Trap which
