...
public class XMLPrinter {
  ...
  private void printStatementsInBody(Body body, java.io.PrintWriter out) {
    ...
    */
    ...
    XMLNode parametersNode =
        methodNode.addChild("parameters", new String[] {"method"}, new String[] {cleanMethodName});
    ...
    while (unitIt.hasNext()) {
      ...
      if (stmtToName.containsKey(currentStmt)) {
        ...
        // if( index != -1 )
        //	xmlLabels = xmlLabels.substring( 0, index ) + ( labelID ) + xmlLabels.substring(
        // index + 2 );
        // index = xmlLabels.indexOf( "%d" );
        // if( index != -1 )
        //	xmlLabels = xmlLabels.substring( 0, index ) + new Float( ( new Float( labelID
        // ).floatValue() / new Float( units.size() ).intValue() ) * 100.0 ).intValue() +
        // xmlLabels.substring( index + 2 );
    ...
    */
    ...
    XMLNode typesNode =
        localsNode.addChild("types", new String[] {"count"}, new String[] {localTypes.size() + ""});
    ...
    // add count attribute to labels node, and stmtcount, and stmtpercentage attributes to each
    // label node
    ...
    if (trapIt.hasNext()) {
      while (trapIt.hasNext()) {
        ...
        catchNode.addChild(
            "end",
            new String[] {"label"},
            new String[] {stmtToName.get(trap.getEndUnit()).toString()});
        catchNode.addChild(
            "handler",
            new String[] {"label"},
  ...
  private String cleanMethod(String str) {
    // method names can be filtered here, for example replacing < and > with _ to comply with
    // XML name tokens
    return str.trim().replace('<', '_').replace('>', '_');
  }

  private String cleanLocal(String str) {
    // local names can be filtered here, for example replacing $ with _ to comply with XML name
    // tokens
  ...
  private void printXMLTo(SootClass cl, PrintWriter out) {
    ...
    // Print interfaces
    {
      xmlTempNode =
          xmlClassNode.addChild(
              "interfaces", "", new String[] {"count"}, new String[] {cl.getInterfaceCount() + ""});
    ...
    // Print fields
    {
      xmlTempNode =
          xmlClassNode.addChild(
              "fields", "", new String[] {"count"}, new String[] {cl.getFieldCount() + ""});
    ...
    // Print methods
    {
      ...
      setXMLNode(
          xmlClassNode.addChild(
              "methods", new String[] {"count"}, new String[] {cl.getMethodCount() + ""}));
