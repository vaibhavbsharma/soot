 ...
 * Body instance; as a consequence, changes made on a Block will be automatically reflected in its
 * enclosing method body. Blocks also exist in the context of a BlockGraph, a CFG for a method where
 * Block instances are the nodes of the graph. Hence, a Block can be queried for its successors and
...
public class Block implements Iterable<Unit> {
  ...
  /**
   * Sets the list of Blocks that are successors of this block in it's enclosing BlockGraph
   * instance.
   *
