 ...
 * references start new blocks. Exceptional control flow is ignored, so the graph will be a forest
 * where each exception handler constitutes a disjoint subgraph.
 */
public class ArrayRefBlockGraph extends BlockGraph {
  ...
  /**
   * Constructs an <tt>ArrayRefBlockGraph</tt> corresponding to the <tt>Unit</tt>-level control flow
   * represented by the passed {@link BriefUnitGraph}.
   *
   * @param unitGraph The <tt>BriefUnitGraph</tt> for which to build an <tt>ArrayRefBlockGraph</tt>.
   */
