 ...
 * account when finding the <tt>Block</tt>s for the provided Body. Any {@link Unit} which is the
 * first <tt>Unit</tt> to be convered by some exception handler will start a new Block, and any
 * <ttUnit</tt> which is the last <tt>Unit</tt> to be covered a some exception handler, will end the
 * block it is part of. These ``zones'', however, are not split up to indicate the possibility that
 * an exception will lead to control exiting the zone before it is completed.
 */
public class ZonedBlockGraph extends BlockGraph {
   ...
   * <tt>BriefUnitGraph</tt> as a parameter, as a minor optimization.
   *
   * @param body The <tt>Body</tt> for which to produce a <tt>ZonedBlockGraph</tt>.
   */
  ...
  /**
   * Constructs a <tt>ZonedBlockGraph</tt> corresponding to the <tt>Unit</tt>-level control flow
   * represented by the passed {@link BriefUnitGraph}.
   *
   * @param unitGraph The <tt>BriefUnitGraph</tt> for which to produce a <tt>ZonedBlockGraph</tt>.
   */
  ...
  protected Set<Unit> computeLeaders(UnitGraph unitGraph) {
    ...
    if (body != mBody) {
      throw new RuntimeException(
          "ZonedBlockGraph.computeLeaders() called with a UnitGraph that doesn't match its mBody.");
    }
