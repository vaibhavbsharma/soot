 ...
 * multi-headed and potentially multi-tailed which makes the results of RegionAnalysis and PDG
 * construction unreliable (It's not clear if it would be useful anyway); Also, ExceptionalGraph's
 ...
 * make the graph single entry single exit.
 *
...
public class HashMutablePDG extends HashMutableEdgeLabelledDirectedGraph<PDGNode, String>
        ...
        for (Iterator<Block> bItr = bs.iterator(); bItr.hasNext(); ) {
          ...
          if (b.equals(a))
            throw new RuntimeException(
                "PDG construction: A and B are not supposed to be the same node!");
          ...
          while (dode != aParentDode) {
            ...
            // This occurs if the CFG is multi-tailed and therefore the pdom is a
            // forest.
            if (dode.getParent() == null)
              // throw new RuntimeException("parent dode in pdom is null: dode is " +
              // aDode);
              break;
            dode = dode.getParent();
          }
          ...
          // first make A's pdg node be a conditional (predicate) pdgnode, if it's not
          // already.
          ...
          // now remove b and all the nodes in the same weak region from the list of
          // dependents
          ...
          while (!copyOfDependents.isEmpty()) {
            ...
            if (depBPDGNode == null) {
              ...
              // now remove all the nodes in the same weak region from the list of
              // dependents
            ...
            /**
             * If all the nodes in the weak region of depB are dependent on A, then add an edge from
             * the region of B to the region of depB.
             *
             * <p>else, a new region has to be created to contain the dependences of depB, if not
             * already created.
             */
            if (dependents.containsAll(rdepB.getBlocks())) {
              ...
              // set the region hierarchy
              // Do not set this because the region was created before so must have
              // the
              ...
              if (predPDGofdepB == pdgnodeOfdepBRegion) {
                ...
                Region newRegion =
                    new Region(
                        this.m_strongRegionStartID++,
                        topLevelRegion.getSootMethod(),
                        topLevelRegion.getSootClass(),
                        this.m_cfg);
                newRegion.add(depB);
                ...
                if (depB == a) {
                  ...
                  // This is needed to correctly adjust the prev/next pointers for
                  // the new loop node. We should not need
                  // to adjust the old loop header node because the prev/next
                  // should not have been set previously for it.
                  ...
                  // DEtermine which dependent of the loop header is actually the
                  // loop body region
  ...
  public PDGNode getPDGNode(Object cfgNode) {
    if (cfgNode != null && cfgNode instanceof Block)
      if (this.m_obj2pdgNode.containsKey(cfgNode)) return this.m_obj2pdgNode.get(cfgNode);
  ...
  private void replaceInGraph(PDGNode newnode, PDGNode oldnode) {
    ...
    HashMutableEdgeLabelledDirectedGraph graph =
        (HashMutableEdgeLabelledDirectedGraph) this.clone();
  ...
  public void removeAllEdges(PDGNode from, PDGNode to) {
    if (!containsAnyEdge(from, to)) return;
