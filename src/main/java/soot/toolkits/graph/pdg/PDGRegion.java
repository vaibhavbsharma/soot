 ...
 * graph. A PDGRegion is slightly different than a weak or strong region; the loops and conditional
 * relations between regions are explicitly represented in the PDGRegion.
 *
...
public class PDGRegion implements IRegion, Iterable<PDGNode> {
  ...
  public PDGRegion(
      int id, List<PDGNode> nodes, SootMethod m, SootClass c, UnitGraph ug, PDGNode node) {
    ...
    if (Options.v().verbose()) G.v().out.println("New pdg region create: " + id);
  }
  ...
  public Object clone() {
    PDGRegion r =
        new PDGRegion(
            this.m_id, this.m_method, this.m_class, this.m_unitGraph, m_corrspondingPDGNode);
  ...
  class ChildPDGFlowIterator implements Iterator<PDGNode> {
    ...
    public boolean hasNext() {
      if (beginning) if (m_list.size() > 0) return true;
  ...
  /**
   * return an iterator that know how to follow the control flow in a region. This actually returns
   * a ChildPDGFlowIterator that only iterates through the dependent nodes that contribute to the
   * units that belong to a region as defined by a weak region.
   */
    ...
    if (this.m_units == null) {
      ...
      for (Iterator<PDGNode> itr = this.iterator(); itr.hasNext(); ) {
        ...
        if (node.getType() == PDGNode.Type.REGION) {
          // Actually, we should only get here if a loop header region is in this region's
          // children list.
          // Or if the PDG is based on an ExceptionalUnitGraph, then this could be the
          ...
          throw new RuntimeException(
              "Exception in PDGRegion.getUnits: PDGNode's type is undefined!");
      }
    }
    return this.m_units;
  }
  ...
  public PDGNode unit2PDGNode(Unit u) {
    if (this.m_unit2pdgnode.containsKey(u)) return this.m_unit2pdgnode.get(u);
    else return null;
  }
  ...
  public String toString() {
    ...
    if (this.m_parent != null) str += "Parent is: " + this.m_parent.getID() + "----\n";
