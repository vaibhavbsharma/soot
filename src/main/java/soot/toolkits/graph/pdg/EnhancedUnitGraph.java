 ...
 * BriefUnitGraph when there are no exception handling construct in the method; at the presence of
 * such constructs, the CFG is constructed from a brief graph by adding a concise representation of
 * the exceptional flow as well as START/STOP auxiliary nodes. In a nutshell, the exceptional flow
...
public class EnhancedUnitGraph extends UnitGraph {
  ...
  /**
   * This method removes all the heads in the CFG except the one that corresponds to the first unit
   * in the method.
   */
  ...
  protected void handleExplicitThrowEdges() {
    ...
    for (Iterator<Unit> itr = tails.iterator(); itr.hasNext(); ) {
      ...
      if (x2mergePoint.containsKey(xgode)) mergePoint = x2mergePoint.get(xgode);
        ...
        // immediately
        /*
         * if(mergePoint == null) { //throw new
         * RuntimeException("No child post-dominates x."); mergePoint =
         * potentialMergePoint;
         *
         * }
         */
  ...
  /**
   * Add an exceptional flow edge for each handler from the corresponding auxiliary nop node to the
  ...
  protected void addAuxiliaryExceptionalEdges() {
    ...
    for (Iterator<Trap> trapIt = body.getTraps().iterator(); trapIt.hasNext(); ) {
       ...
       *
       * <p>The problem is that we don't have a complete DOM tree at this transient state.
       *
       * <p>The work-around is to not process a trap that has already an edge pointing to it.
       */
...
/**
 * This class represents a block of code guarded by a trap. Currently, this is not used but it might
 * well be put to use in later updates.
 *
 * @author Hossein Sadat-Mohtasham
 */
class GuardedBlock {
