...
public class RegionAnalysis {
  ...
  private void findWeakRegions() {
    ...
    try {
      ...
      if (this.m_blockCFG.getHeads().size() == 1) {
        ...
        // throw new RuntimeException("RegionAnalysis: cannot properly deal with
        // multi-headed and tailed CFG!");
      }
   ...
   * is recursively called with w and the same region id as v. If the comparison fails, then a new
   * region is created and weakRegionDFS is called recursively with w but this time with the newly
   * created region id.
   *
   * @param v
   * @param r
  ...
  private void weakRegionDFS2(Block v, int r) {
    // regions keep an implicit order of the contained blocks so it matters where blocks are
    // added
    // below.
