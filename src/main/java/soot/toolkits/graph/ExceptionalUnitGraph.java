 ...
 * (since any of those predecessors may have been the last <code>Unit</code> to complete execution
 * before the handler starts execution). If the excepting <code>Unit</code> might have the side
 * effect of changing some field, then there will definitely be an edge from the excepting <code>
 * Unit</code> itself to its handlers, since the side effect might occur before the exception is
 * raised. If the excepting <code>Unit</code> has no side effects, then parameters passed to the
 * <code>ExceptionalUnitGraph</code> constructor determine whether or not there is an edge from the
 * excepting <code>Unit</code> itself to the handler <code>Unit</code>.
 */
public class ExceptionalUnitGraph extends UnitGraph implements ExceptionalGraph<Unit> {
  ...
  /**
   * Constructs the graph from a given Body instance using the passed {@link ThrowAnalysis} and a
  ...
  /**
   * Constructs the graph from a given Body instance, using the {@link Scene} 's default {@link
   * ThrowAnalysis} to estimate the set of exceptions that each {@link Unit} might throw and a
   * default value, provided by the {@link Options} class, for the <code>omitExceptingUnitEdges
   * </code> parameter.
   *
   * @param body the <code>Body</code> from which to build a graph.
  ...
  /**
   * Allocates an <code>ExceptionalUnitGraph</code> object without initializing it. This
   * &ldquo;partial constructor&rdquo; is provided for the benefit of subclasses whose constructors
   * need to perform some subclass-specific processing before actually creating the graph edges
   * (because, for example, the subclass overrides a utility method like {@link
   * #buildExceptionDests(ThrowAnalysis)} or {@link #buildExceptionalEdges(ThrowAnalysis, Map, Map,
   * Map, boolean)} with a replacement method that depends on additional parameters passed to the
  ...
  /**
   * Performs the real work of constructing an <code>ExceptionalUnitGraph</code>, factored out of
  ...
  /**
   * Utility method used in the construction of {@link soot.toolkits.graph.UnitGraph UnitGraph}
   * variants which include exceptional control flow. It determines which {@link Unit}s may throw
  ...
  protected Map<Unit, Collection<ExceptionDest>> buildExceptionDests(ThrowAnalysis throwAnalysis) {
    ...
    Map<Unit, ThrowableSet> unitToUncaughtThrowables =
        new LinkedHashMap<Unit, ThrowableSet>(units.size());
  ...
  /**
   * A utility method for recording the exceptions that a <code>Unit</code> throws to a particular
      ...
      for (ExceptionDest dest : dests) {
        if (dest.getTrap() != null) {
          ...
          if (alwaysAddSelfEdges
              || (selfThrowables != null && selfThrowables.catchableAs(trapsType))) {
      ...
      CFGEdge(Unit head, Unit tail) {
        if (tail == null)
          throw new RuntimeException(
              "invalid CFGEdge(" + (head == null ? "null" : head.toString()) + ',' + "null" + ')');
        this.head = head;
        this.tail = tail;
      }
  ...
  /**
   * Utility method, to be called only after the unitToPreds and unitToSuccs maps have been built.
   * It defines the graph's set of heads to include the first {@link Unit} in the graph's body,
   * together with all the <code>Unit</code>s in <code>additionalHeads</code>. It defines the
   * graph's set of tails to include all <code>Unit</code>s which represent some sort of return
   * bytecode or an <code>athrow</code> bytecode which may escape the method.
   */
  ...
  /**
   * Returns a collection of {@link ExceptionalUnitGraph.ExceptionDest ExceptionDest} objects which
  ...
  public Collection<ExceptionDest> getExceptionDests(final Unit u) {
    ...
    if (result == null) {
      ExceptionDest e =
          new ExceptionDest(null, null) {
            private ThrowableSet throwables;

            @Override
            public ThrowableSet getThrowables() {
              if (null == throwables) throwables = throwAnalysis.mightThrow(u);
              return throwables;
            }
          };
