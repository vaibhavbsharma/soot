 ...
 * Control flow associated with exceptions is taken into account: when a {@link Unit} may throw an
 * exception that is caught by a {@link Trap} within the <code>Body</code>, the excepting <code>Unit
 * </code> starts a new basic block (<code>Unit</code>s do not start a new block when all the
 * exceptions they might throw would escape the method without being caught).
 */
public class ExceptionalBlockGraph extends BlockGraph implements ExceptionalGraph<Block> {
   ...
   * ExceptionalUnitGraph</code> as a parameter, as a minor optimization.
   *
  ...
  /**
   * Constructs a graph for the blocks found by partitioning the the {@link Unit}s in an {@link
    ...
    if (unitGraph.getBody().getTraps().size() == 0) {
      ...
      if (throwAnalysis == null) {
        throw new IllegalStateException(
            "ExceptionalUnitGraph lacked a cached ThrowAnalysis for a Body with no Traps.");
      }
  ...
  /**
   * Utility method which, given a {@link List} of objects and a {@link Map} where those objects
  ...
  private Map<Block, Collection<ExceptionDest>> buildExceptionDests(
      ExceptionalUnitGraph unitGraph, Map<Unit, Block> unitToBlock) {
  ...
  /**
   * Utility method which, given a {@link Block} and the {@link ExceptionalUnitGraph} from which it
  ...
  public Collection<ExceptionDest> getExceptionDests(final Block b) {
    if (blockToExceptionDests == null) {
      ExceptionDest e =
          new ExceptionDest(null, null, null) {
