 ...
 * Provides an interface to add/delete nodes and edges.
 *
 * @param <N>
 * @param <L>
 */
public interface MutableEdgeLabelledDirectedGraph<N, L> extends EdgeLabelledDirectedGraph<N, L> {
  ...
  /**
   * Removes all edges with the given label in the graph. If no edges are present, no change is
   * made.
   *
   * @param label label for the edge to remove.
   */
   ...
   * consequence it is considered both a head and tail for the graph.
   *
