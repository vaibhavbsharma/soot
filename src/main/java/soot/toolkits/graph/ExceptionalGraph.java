...
public interface ExceptionalGraph<N> extends DirectedGraph<N> {
   ...
   * executes (perhaps only partially, if it throws an exception after producing a side effect),
   * execution may proceed to node <code>b</code>. An ExceptionDest from <code>a</code> to <code>b
   * </code>, on the other hand, says that when <code>a</code> fails to execute, execution may
   * proceed to <code>b</code> instead.
   */
  interface ExceptionDest<N> {
    ...
    /**
     * Returns the CFG node corresponding to the beginning of the exception handler that catches the
