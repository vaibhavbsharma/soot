...
public class ProcessData {
  ...
  public static void main(String[] args) {
    ...
    if (args[0].equals("--help")) {
      ...
      if (aggregationMechanism == ProcessData.BENCHMARK) {
        ...
        System.out.println(
            "Only one tex file with the name" + metricListFileName + ".tex will be created");
    ...
    if (args[startIndex].equals("*")) {
      ...
      for (int i = startIndex; i < args.length; i++) {
        ...
        if (!temp.endsWith(".xml")) {
          // System.out.println("Argument number "+(startIndex+1) + ": '" + temp+"' is not
          // a valid xml file");
          // System.exit(1);
        } else {
  ...
  public static void printHelp() {
    ...
    System.out.println("\n\n The -metricList mode");
    System.out.println(
        "The argument at location 1 should be name of a file where the list of metrics will be stored");
    ...
    System.out.println("\n\n The -tables mode");
    System.out.println(
        "The argument at location 1 should be name of a file where the list of metrics are stored");
    ...
    System.out.println(
        "Each xml file is considered to be a benchmark with a bunch of classes in it");
  ...
  private static void readStar() {
    ...
    if (children == null) {
      ...
      if (children != null) {
        for (String element : children) xmlFileList.add(element);
      }
    }
  }
    ...
    if (aggregationMechanism == ProcessData.BENCHMARK) {
      ...
      while (it.hasNext()) {
        ...
        if (CSV) {
          ...
          String xmlfileColumnType =
              fileName.substring(fileName.lastIndexOf('-') + 1, fileName.lastIndexOf(".xml"));
          System.out.println("XML FILE COLUMN TYPE" + xmlfileColumnType);
          if (xmlfileColumnType.equals("Jad")
              || xmlfileColumnType.equals("original")
              || xmlfileColumnType.equals("SourceAgain")
              || xmlfileColumnType.equals("disabled")
      ...
      while (keys.hasNext()) {
        ...
        if (!CSV) printTexTableHeader(bench, key, columns);
         ...
         */
        if (decompiler) {
          // coming from decompiler ordering is: original, Jad, SourceAgain,
          // Dava(enabled), Dava(disabled)
          ...
          while (tempIt.hasNext()) {
            ...
            } else throw new RuntimeException("property xml not correct");
          }
          ...
          // coming from obfuscator ordering is: original,  jbco enabled, jbco disabled,
          // klassmaster enabled,klassmaster disabled,
        ...
        while (fileIt.hasNext()) {
          ...
          try {
            ...
            while (tempIt.hasNext()) {
              ...
              // System.out.println("NEXT TEMP IT
              // ISSSSSSSSSSSSSSSSSSSSSS"+nexttempit);
              if (temp instanceof Integer) {
                    ...
                    case 4:
                      if (nexttempit.equals("Total-Abrupt")) {
                        ...
                        if (totalConditional != 0) {
                          // average by dividing total-cond-complexity for
                          // sum of if+ifelse+loops
                        ...
                        if (astCount != 0) {
                          // average by dividing D-W-Complexity by node
                          // count
                        ...
                        if (exprCount != 0) {
                          // average by dividing expr-complexity for
                          // exprCount
                        ...
                        if (nameCount != 0) {
                          // average by dividing name-complexity for
                          // nameCount
                ...
                } else bench.print("&" + val);
              } else throw new RuntimeException("Unknown type of object stored!!!");
              if (CSV) {
                if (tempIt.hasNext()) {
        ...
        if (CSV) bench.println("");
        else printTexTableFooter(bench, "");
  ...
  private static void getClassMetrics(String fileName, Document doc, Vector<String> columns) {
    ...
    System.out.println(
        "Creating tex file" + newClassName + " from metrics info in file" + fileName);
      ...
      if (classNode.getNodeType() == Node.ELEMENT_NODE) {
        ...
        for (int s = 0; s < metrics.getLength() && columnIndex < columns.size(); s++) {
          ...
          if (metricNode.getNodeType() == Node.ELEMENT_NODE) {
            ...
            // We get to this point only if the metric name is the same as the column
            // name
  ...
  private static void printCSVHeader(PrintWriter out) {
    if (decompiler) out.println(",Original,Jad,SourceAgain,Dava(enabled),Dava(disabled)");
    else
  ...
  private static void printTexTableHeader(
      PrintWriter out, String rowHeading, Vector<String> columns) {
    // out.println("\\begin{figure}[hbtp]");
    out.println("\\begin{table}[hbtp]");
    out.print("\\begin{tabular}{");

    for (int i = 0; i <= columns.size(); i++) out.print("|l");
