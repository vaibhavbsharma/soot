 ...
 *
 * <p><code>ThrowableSet</code> does not implement the {@link java.util.Set} interface, so perhaps
 * it is misnamed. Instead, it provides only the operations that we require for determining whether
 * a given statement might throw an exception that would be caught by a given handler.
 *
 * <p>There is a limitation on the combinations of operations permitted on a <code>ThrowableSet
 * </code>. The <code>ThrowableSet</code>s returned by {@link #whichCatchableAs(RefType)} cannot be
 * involved in subsequent <code>add()</code> or <code>whichCatchableAs()</code> operations. That is,
 * given
 *
 * <blockquote>
 *
 ...
 * </blockquote>
 *
 * for any <code>ThrowableSet</code> <code>s</code> and {@link soot.RefType RefType} <code>r</code>,
 * and
 *
 * <blockquote>
 *
 * <code>t == p.getUncaught()</code> or <code>t == p.getCaught()</code>
 *
 * </blockquote>
 *
 * then calls to <code>t.add(r)</code>, <code>t.add(a)</code>, and <code>s.add(t)</code>, will throw
 * an {@link ThrowableSet.AlreadyHasExclusionsException}, for any <code>RefType</code> <code>r
 * </code>, {@link AnySubType} <code>a</code>, and <code>ThrowableSet</code> <code>t</code>.
 *
 * <p>Actually the restrictions implemented are not quite so strict (there are some combinations of
 * <code>whichCatchableAs()</code> followed by <code>add()</code> which will not raise an
 * exception), but a more accurate description would require reference to the internals of the
 * current implementation. The restrictions should not be too onerous for <code>ThrowableSet</code>
 * 's anticipated uses: we expect <code>ThrowableSet</code>s to grow by accumulating all the
 * exception types that a given {@link Unit} may throw, then, shrink as the types caught by
 * different exception handlers are removed to yield the sets representing exceptions which escape
 * those handlers.
 *
 * <p>The <code>ThrowableSet</code> class is intended to be immutable (hence the <code>final</code>
 * modifier on its declaration). It does not take the step of guaranteeing immutability by cloning
 * the <code>RefLikeType</code> objects it contains, though, because we trust {@link Scene} to
 * enforce the existence of only one <code>RefLikeType</code> instance with a given name.
 */
public final class ThrowableSet {
  ...
  /**
   * Singleton class for fields and initializers common to all ThrowableSet objects (i.e., these
   * would be static fields and initializers, in the absence of soot's {@link G} and {@link
   * Singletons} classes).
   */
  public static class Manager {
    ...
    /**
     * <code>ThrowableSet</code> containing all the asynchronous and virtual machine errors, which
     * may be thrown by any bytecode instruction at any point in the computation.
     */
    final ThrowableSet VM_ERRORS;

    /**
     * <code>ThrowableSet</code> containing all the exceptions that may be thrown in the course of
     * resolving a reference to another class, including the process of loading, preparing, and
     * verifying the referenced class.
     */
    final ThrowableSet RESOLVE_CLASS_ERRORS;

    /**
     * <code>ThrowableSet</code> containing all the exceptions that may be thrown in the course of
     * resolving a reference to a field.
     */
    final ThrowableSet RESOLVE_FIELD_ERRORS;

    /**
     * <code>ThrowableSet</code> containing all the exceptions that may be thrown in the course of
     * resolving a reference to a non-static method.
     */
    final ThrowableSet RESOLVE_METHOD_ERRORS;

    /**
     * <code>ThrowableSet</code> containing all the exceptions which may be thrown by instructions
     * that have the potential to cause a new class to be loaded and initialized (including
     * UnsatisfiedLinkError, which is raised at runtime rather than linking type).
     */
  ...
  /**
   * Set of exception types which, though members of exceptionsIncluded, are to be excluded from the
   * types represented by this <code>ThrowableSet</code>. To simplify the implementation, once a
   * <code>ThrowableSet</code> has any excluded types, the various <code>add()</code> methods of
   * this class must bar additions of subtypes of those excluded types.
   */
  private final Set<AnySubType> exceptionsExcluded;

  /**
   * A map from ({@link RefLikeType} \\union <code>ThrowableSet</code>) to <code>ThrowableSet
   * </code>. If the mapping (k,v) is in <code>memoizedAdds</code> and k is a <code>ThrowableSet
   * </code>, then v is the set that results from adding all elements in k to <code>this</code>. If
   * (k,v) is in <code>memoizedAdds</code> and k is a {@link RefLikeType}, then v is the set that
   * results from adding k to <code>this</code>.
   */
  ...
  private static <T> Set<T> getImmutable(Set<T> in) {
    if ((null == in) || in.isEmpty()) return Collections.emptySet();
    if (1 == in.size()) return Collections.singleton(in.iterator().next());
  ...
  public ThrowableSet add(AnySubType e) throws ThrowableSet.AlreadyHasExclusionsException {
    ...
    for (AnySubType excludedType : exceptionsExcluded) {
      ...
      boolean isExcluded =
          exclusionBase.getSootClass().isPhantom() && exclusionBase.equals(newBase);
      isExcluded |=
          !exclusionBase.getSootClass().isPhantom()
    ...
    for (RefLikeType incumbent : this.exceptionsIncluded) {
      if (incumbent instanceof RefType) {
        ...
        if (newBase.getSootClass().isPhantom()) {
          if (!incumbentBase.equals(newBase)) resultSet.add(incumbent);
        }
   ...
   *     possible to represent the addition of <code>s</code> to this <code>ThrowableSet
   *     </code>.
   */
  ...
  /**
   * Returns a <code>ThrowableSet</code> which contains all the exceptions in <code>
   * addedExceptions</code> in addition to those in this <code>ThrowableSet</code>.
   *
  ...
  /**
   * Returns a <code>ThrowableSet</code> which contains all the exceptions in this <code>
   * ThrowableSet</code> except for those in <code>removedExceptions</code>.
   *
   * @param removedExceptions a set of {@link RefLikeType} and {@link AnySubType} objects to be
   *     added to the types included in this <code>ThrowableSet</code>.
   * @return a set containing all the <code>addedExceptions</code> as well as the exceptions in this
   *     set.
   */
  private ThrowableSet remove(Set<RefLikeType> removedExceptions) {
    // Is there anything to remove?
    if (removedExceptions.isEmpty()) return this;

    int changes = 0;
    Set<RefLikeType> resultSet = new HashSet<RefLikeType>(this.exceptionsIncluded);
    for (RefLikeType tp : removedExceptions) {
      if (tp instanceof RefType) if (resultSet.remove(tp)) changes++;
    }

    ThrowableSet result = null;
    if (changes > 0) result = Manager.v().registerSetIfNew(resultSet, this.exceptionsExcluded);
    else result = this;
    return result;
  }

  /**
   * Returns a <code>ThrowableSet</code> which contains all the exceptions from the current set
   * except for those in the given <code>ThrowableSet</code>.
   *
   * @param s The set containing the exceptions to exclude from the new set
    ...
    if (exceptionsExcluded.size() > 0) {
      ...
      for (AnySubType exclusion : exceptionsExcluded) {
        if (catcher.getSootClass().isPhantom()) {
          if (exclusion.getBase().equals(catcher)) return false;
        ...
        if (thrownType instanceof RefType) {
          ...
          if (catcher.getSootClass().isPhantom()) {
            if (thrownBase.equals(catcher)
                || thrownBase.getClassName().equals("java.lang.Throwable")) return true;
          }
    ...
    for (RefLikeType inclusion : exceptionsIncluded) {
      if (inclusion instanceof RefType) {
        ...
        if (catcher.getSootClass().isPhantom()) {
          ...
          else uncaughtIncluded = addExceptionToSet(inclusion, uncaughtIncluded);
        ...
        // only if it is in the inclusion list and ignore any hierarchy.
        if (catcher.getSootClass().isPhantom()) {
          if (base.equals(catcher)) caughtIncluded = addExceptionToSet(inclusion, caughtIncluded);
          else {
  ...
  /**
   * The return type for {@link ThrowableSet#whichCatchableAs(RefType)}, consisting of a pair of
   * ThrowableSets.
   */
  public static class Pair {
     ...
     *
     * @param caught The set of exceptions to be returned when {@link #getCaught()} is called on the
     *     constructed <code>ThrowableSet.Pair</code>.
     * @param uncaught The set of exceptions to be returned when {@link #getUncaught()} is called on
     *     the constructed <code>ThrowableSet.Pair</code>.
     */
    ...
    /**
     * Indicates whether two {@link Object}s are <code>ThrowableSet.Pair</code>s representing the
     * same set of caught and uncaught exception types.
     *
     * @param o the <code>Object</code> to compare to this <code>ThrowableSet.Pair</code>.
     * @return <code>true</code> if <code>o</code> is a <code>ThrowableSet.Pair</code> representing
     *     the same set of caught and uncaught types as this <code>
     *     ThrowableSet.Pair</code>.
     */
    @Override
   ...
   * @param set The <code>Set</code> to which to add the types, or <code>null</code> if no <code>
   *     Set</code> has yet been allocated.
  ...
  /**
   * Returns a cryptic identifier for this <code>ThrowableSet</code>, used to identify a set when it
  ...
  /**
   * Produce an abbreviated representation of this <code>ThrowableSet</code>, suitable for human
   * consumption. The abbreviations include:
   *
   * <ul>
   *   <li>The strings &ldquo;<code>java.lang.</code>&rdquo; is stripped from the beginning of
   *       exception names.
   *   <li>The string &ldquo;<code>Exception</code>&rdquo; is stripped from the ends of exception
   *       names.
   *   <li>Instances of <code>AnySubType</code> are indicated by surrounding the base type name with
   *       parentheses, rather than with the string &ldquo; <code>Any_subtype_of_</code> &rdquo;
   *   <li>If this <code>ThrowableSet</code> includes all the elements of {@link
   *       ThrowableSet.Manager#VM_ERRORS VM_ERRORS}, they are abbreviated as &ldquo;<code>
   *       vmErrors</code>&rdquo; rather than listed individually.
   *
  ...
  private String toAbbreviatedString(Set<? extends RefLikeType> s, char connector) {
    ...
    Collection<RefLikeType> vmErrorThrowables =
        ThrowableSet.Manager.v().VM_ERRORS.exceptionsIncluded;
    ...
    for (Iterator<? extends RefLikeType> it = sortedThrowableIterator(s); it.hasNext(); ) {
      ...
      } else throw new RuntimeException("Unsupported type " + reflikeType.getClass().getName());
  ...
  /**
   * A package-private method to provide unit tests with access to the {@link RefLikeType} objects
   * which represent the <code>Throwable</code> types included in this set.
   *
  ...
  /**
   * A package-private method to provide unit tests with access to the {@link RefLikeType} objects
   * which represent the <code>Throwable</code> types excluded from this set.
   *
   * @return an unmodifiable collection view of the <code>Throwable</code> types excluded from this
   *     set.
   */
