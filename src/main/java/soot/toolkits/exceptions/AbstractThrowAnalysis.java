...
/**
 * Abstract class implementing parts of the {@link ThrowAnalysis} interface which may be common to
 * multiple concrete <code>ThrowAnalysis</code> classes. <code>AbstractThrowAnalysis</code> provides
 * straightforward implementations of {@link mightThrowExplicitly(ThrowInst)} and {@link
 * mightThrowExplicitly(ThrowStmt)}, since concrete implementations of <code>ThrowAnalysis</code>
 * seem likely to differ mainly in their treatment of implicit exceptions.
 */
  ...
  public ThrowableSet mightThrowExplicitly(ThrowStmt t) {
    ...
    if (thrownType == null || thrownType instanceof UnknownType) {
      ...
      throw new IllegalStateException(
          "UnitThrowAnalysis StmtSwitch: type of throw argument is not a RefType!");
    } else {
      ThrowableSet result = ThrowableSet.Manager.v().EMPTY;
