 ...
 * analyses are expensive to compute. A {@link SmartLocalDefs} instance requires a {@link UnitGraph}
 * (usually a {@link ExceptionalUnitGraph}), and creating these repeatedly, and applying the {@link
 * SmartLocalDefs} analysis repeatedly costs time. Therefore in this class we pool these instances
 * in cases in which the respective body is still the same.
 *
 * @author Eric Bodden
 */
public class SmartLocalDefsPool {
   ...
   * created {@link ExceptionalUnitGraph} for b, with standard parameters. If the body b's
   * modification count has not changed since the last time such an analysis was requested for b,
   * then the previously created analysis is returned instead.
   *
