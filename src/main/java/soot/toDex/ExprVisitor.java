 ...
 * <br>
 * Use {@link Switchable#apply(soot.util.Switch)} with this visitor to add statements. These are
 * added to the instructions in the {@link StmtVisitor}.<br>
 * If the expression is part of an assignment or an if statement, use {@link
 * #setDestinationReg(Register)} and {@link #setTargetForOffset(Stmt)}, respectively.
 *
 * @see StmtVisitor
 */
class ExprVisitor implements ExprSwitch {
  ...
  private Insn buildInvokeInsn(
      String invokeOpcode, MethodReference method, List<Register> argumentRegs) {
    Insn invokeInsn;
    ...
    if (regCountForArguments <= 5) {
      ...
      throw new Error(
          "too many parameter registers for invoke-* (> 255): "
              + regCountForArguments
  ...
  private void buildCalculatingBinaryInsn(
      final String binaryOperation, Value firstOperand, Value secondOperand, Expr originalExpr) {
    constantV.setOrigStmt(origStmt);
    ...
    if (destinationReg.getType() instanceof IntType
      ...
      if (SootToDexUtils.fitsSigned8(secondOpConstant)) {
        stmtV.addInsn(
            buildLit8BinaryInsn(binaryOperation, firstOpReg, (byte) secondOpConstant), origStmt);
        return;
      }
      if (SootToDexUtils.fitsSigned16(secondOpConstant)) {
        if (!binaryOperation.equals("SHL")
            && !binaryOperation.equals("SHR")
            && !binaryOperation.equals("USHR")) {
          // no shift opc available for /lit16
          stmtV.addInsn(
              buildLit16BinaryInsn(binaryOperation, firstOpReg, (short) secondOpConstant),
              origStmt);
          return;
        }
      }
    ...
    if (isSmallerThan(destRegType, PrimitiveType.INT)) {
      ...
      if (isBiggerThan(PrimitiveType.getByName(secondOpReg.getType().toString()), destRegType)) {
        destinationReg = regAlloc.asTmpReg(secondOpReg.getType());
          ...
          PrimitiveType.getByName(firstOpReg.getType().toString()), destRegType)) {
  ...
  private Insn buildNormalBinaryInsn(
      String binaryOperation, Register firstOpReg, Register secondOpReg) {
  ...
  private Insn buildLit16BinaryInsn(
      String binaryOperation, Register firstOpReg, short secondOpLieteral) {
    Opcode opc = Opcode.valueOf(binaryOperation + "_INT_LIT16");
    return new Insn22s(opc, destinationReg, firstOpReg, secondOpLieteral);
  }

  private Insn buildLit8BinaryInsn(
      String binaryOperation, Register firstOpReg, byte secondOpLiteral) {
  ...
  private Insn buildComparingBinaryInsn(
      String binaryOperation, Value firstOperand, Value secondOperand) {
    constantV.setOrigStmt(origStmt);
  ...
  private Insn buildCmpInsn(String opcodePrefix, Value firstOperand, Value secondOperand) {
    ...
    if (opcodePrefix.equals("CMP_LONG")) {
      ...
      throw new RuntimeException(
          "unsupported type of operands for cmp* opcode: " + firstOperand.getType());
    }
  ...
  public void caseInstanceOfExpr(InstanceOfExpr ioe) {
    ...
    stmtV.addInsn(
        new Insn22c(Opcode.INSTANCE_OF, destinationReg, referenceToCheckReg, checkType), origStmt);
  }
  ...
  private boolean needsCastThroughInt(PrimitiveType sourceType, PrimitiveType castType) {
    ...
    return isEqualOrBigger(sourceType, PrimitiveType.LONG)
        && !isEqualOrBigger(castType, PrimitiveType.INT);
  }

  private boolean isMoveCompatible(PrimitiveType sourceType, PrimitiveType castType) {
    if (sourceType == castType) {
      // at this point, the types are "bigger" or equal to int, so no "should cast from int"
      // is needed
      return true;
    }
  ...
  private Opcode getCastOpc(PrimitiveType sourceType, PrimitiveType castType) {
    Opcode opc =
        Opcode.valueOf(
            sourceType.getName().toUpperCase() + "_TO_" + castType.getName().toUpperCase());
    if (opc == null) {
  ...
  public void caseNewMultiArrayExpr(NewMultiArrayExpr nmae) {
    ...
    if (nmae.getSizeCount() > 255) {
      throw new RuntimeException(
          "number of dimensions is too high (> 255) for the filled-new-array* opcodes: "
              + nmae.getSizeCount());
    }
    ...
    if (dimensions <= 5) {
      ...
      stmtV.addInsn(
          new Insn3rc(Opcode.FILLED_NEW_ARRAY_RANGE, dimensionSizeRegs, dimensions, arrayTypeItem),
          null);
