 ...
 * calling convention (that is, the last registers are for the parameters).<br>
 * <br>
...
class RegisterAssigner {
  ...
  public List<Insn> finishRegs(
    ...
    InstructionIterator insnIter =
        new InstructionIterator(insns, insnsStmtMap, instructionRegisterMap);
  ...
  private void renumParamRegsToHigh(
      List<Insn> insns, List<LocalRegisterAssignmentInformation> parameterInstructionsList) {
  ...
  /**
   * Reserves low registers in case we later find an instruction that has short operands. We can
   * then move the real operands into the reserved low ones and use those instead.
   *
   * @param insns
   * @param insnsStmtMap
  ...
  /**
   * Gets the maximum number of registers needed by a single instruction in the given list of
   * instructions.
   *
  ...
  private int getRegsNeeded(
      int regsAlreadyReserved, List<Insn> insns, Map<Insn, Stmt> insnsStmtMap) {
  ...
  private void addMoveForIncompatResultReg(
      InstructionIterator insns, Register destReg, Register origResultReg, Insn curInsn) {
  ...
  /**
   * Adds move instructions to put values into lower registers before using them in an instruction.
   * This assumes that enough registers have been reserved at 0...n.
   *
   * @param curInsn
   * @param insns
   * @param regs
  ...
  private void addMovesForIncompatRegs(
      Insn curInsn, InstructionIterator insns, List<Register> regs, BitSet incompatRegs) {
