...
public class MultiDexBuilder {
  ...
  public void internClass(final ClassDef clz) {
    ...
    if (hasOverflowed()) {
      ...
      // NOTE: We do not want to call internClass recursively here, this might end in an
      // endless loop
  ...
  private boolean hasOverflowed() {
    ...
    // together in
    // the same dex file.
    // For Dalvik (pre 22), it is important that at least the main dex file (classes.dex)
    // contains all needed
    // dependencies of the Main activity, which means that one would have to determine necessary
    // dependencies and
