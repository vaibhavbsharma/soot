 ...
 * we have a trap (1-3) with handler A and a trap (2) with handler B, we transform them into three
 * new traps: (1) and (3) with A, (2) with A+B.
 *
 * @author Steven Arzt
 */
    ...
    while ((to = getNextOverlap(b)) != null) {
      ...
      if (to.t1.getBeginUnit() != to.t2Start) {
        ...
        Trap newTrap =
            Jimple.v()
                .newTrap(
                    to.t1.getException(), to.t1.getBeginUnit(), to.t2Start, to.t1.getHandlerUnit());
      ...
      // (t1start, t2start) ... t1end ... t2end
        ...
        if (firstEndUnit == to.t1.getEndUnit()) {
          if (to.t1.getException() != to.t2.getException()) {
            ...
            // Virtual Machine searches the exception handlers of the current method in
            // the order
            // that they appear in the corresponding exception handler table in the
            // class file,
            // starting from the beginning of that table. Note that the Java Virtual
            // Machine does
            // not enforce nesting of or any ordering of the exception table entries of
            // a method.
            ...
            // The Java
            // Virtual Machine does not enforce nesting of or any ordering of the
            // exception table
            // entries (2.10). However, because try-catch constructs are structured, a
            // compiler
            // can always order the entries of the exception handler table such that,
            // for any thrown
            // exception and any program counter value in that method, the first
            // exception handler
            // that matches the thrown exception corresponds to the innermost matching
            // catch clause."
            //
