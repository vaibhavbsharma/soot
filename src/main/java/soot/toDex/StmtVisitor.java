 ...
 * <br>
 * Use {@link Switchable#apply(soot.util.Switch)} with this visitor to add statements and {@link
 * #getFinalInsns()} to get the final dexlib instructions.<br>
 * <br>
...
class StmtVisitor implements StmtSwitch {
  ...
  /**
   * Reduces the instruction list by removing unnecessary instruction pairs such as move v0 v1; move
   * v1 v0;
   */
  ...
  public void caseAssignStmt(AssignStmt stmt) {
    ...
    this.insnRegisterMap.put(
        insns.get(insns.size() - 1), LocalRegisterAssignmentInformation.v(lhsReg, lhsLocal));
  }
    ...
    if (rhs instanceof CaughtExceptionRef) {
      ...
      parameterInstructionsList.add(
          LocalRegisterAssignmentInformation.v(
              regAlloc.asLocal(localForThis).clone(), localForThis));
    } else {
  ...
  private Insn buildSwitchInsn(
      Opcode opc, Value key, Stmt defaultTarget, SwitchPayload payload, Stmt stmt) {
   ...
   *
   * @param monitorConsts The set of monitor constants fow which to assign fixed registers
   */
