...
public class DalvikTyper implements IDalvikTyper {
  ...
  public void setType(ValueBox vb, Type t, boolean isUse) {
    ...
    if (vb.getValue() instanceof Local) {
      ...
      // Debug.printDbg(IDalvikTyper.DEBUG, "not instance of local: vb: ", vb, " value: ",
      // vb.getValue(), " class: ", vb.getValue().getClass());
    }
  }
  ...
  public void assignType(final Body b) {
          ...
          new StmtSwitch() {
            ...
            public void caseAssignStmt(AssignStmt stmt) {
              ...
              if (l instanceof Local) { // r NOT local
                ...
                if (r instanceof UntypedConstant) return;
                for (Tag t : stmt.getTags()) {

                  if (r instanceof CastExpr) {
                    // do not check tag, since the tag is for the operand of the
                    // cast
                    break;
                  }
                ...
                if (r instanceof ArrayRef && rightType instanceof UnknownType) {
                  ...
                  if (castType instanceof PrimType) {
                    ...
                    for (Tag t : stmt.getTags()) {
                      // Debug.printDbg("assign primitive type from stmt tag:
                      // ", stmt, t);
                      if (t instanceof IntOpTag) {
    ...
    if (!todoUnits.isEmpty()) {
      ...
      for (Unit u : b.getUnits()) {
        if (u instanceof DefinitionStmt) {
          ...
          if (r instanceof UntypedConstant) continue;
                ...
                if (use instanceof AssignStmt) {
                  ...
                  if (r2 instanceof Local) {
                    ...
                    if (ar.getIndex() == currentUnit.getLeftOp()) {
                      // Debug.printDbg("skipping since local is used as
                      // index...");
                      continue;
                    }
                    ...
                    if (baseT.toString().equals(("java.lang.Object"))) {
                      // look for an array type, because an TTT[] is also an
                      // Object...
                      ...
                      if (aTypeOtherThanObject == null) {
                        throw new RuntimeException(
                            "error: did not found array type for base "
                                + arBase
                                + " "
                                + local2Obj.get(arBase)
                                + " \n "
                                + b);
                      }
      ...
      while (!todoUnits.isEmpty()) {
        ...
        if (baseT.toString().equals(("java.lang.Object"))) {
          ...
          if (aTypeOtherThanObject == null) {
            throw new RuntimeException(
                "did not found array type for base "
                    + baselocal
                    + " "
                    + local2Obj.get(baselocal)
                    + " \n "
                    + b);
          }
          baseT = aTypeOtherThanObject;
        ...
        // Debug.printDbg("v: ", ar, " base:", ar.getBase(), " base type: ", basetype, "
        // type: ", ar.getType());
    ...
    for (Constraint c : constraints) {
      if (!vbList.contains(c.l)) {
        // Debug.printDbg(IDalvikTyper.DEBUG, "warning: ", c.l, " not in locals!
        // removing...");
        toRemove.add(c);
        continue;
      }
      if (!vbList.contains(c.r)) {
        // Debug.printDbg(IDalvikTyper.DEBUG, "warning: ", c.r, " not in locals!
        // removing...");
        toRemove.add(c);
        continue;
      }
    ...
    for (LocalObj lo : localObjList) {
      if (!vbList.contains(lo.vb)) {
        // Debug.printDbg(IDalvikTyper.DEBUG, "  -- removing vb: ", lo.vb, " with type ",
        // lo.t);
        continue;
      }
      Local l = lo.getLocal();
      ...
      if (localTemp.contains(l) && lo.isUse) {
        // Debug.printDbg(IDalvikTyper.DEBUG, "  /!\\ def already added for local ", l, "!
        // for vb: ", lo.vb);
      } else {
        // Debug.printDbg(IDalvikTyper.DEBUG, "  * add type ", t, " to local ", l, " for vb:
        // ", lo.vb);
        localTemp.add(l);
        typed.put(lo.vb, t);
      }
    ...
    while (!constraints.isEmpty()) {
      ...
      for (Constraint c : constraints) {
        ...
        if (l instanceof Local && r instanceof Constant) {
          ...
          if (!localTyped.containsKey(l)) continue;
          Type lt = localTyped.get(l);
          // Debug.printDbg(IDalvikTyper.DEBUG, "would like to set type ", lt, " to
          // constant: ", c);
          Value newValue = null;
          if (lt instanceof IntType
            ...
            if (cst instanceof UntypedIntOrFloatConstant
                && ((UntypedIntOrFloatConstant) cst).value == 0) {
              newValue = NullConstant.v();
              // Debug.printDbg("new null constant for constraint ", c, " with l type:
              // ", localTyped.get(l));
            } else {
          ...
          if (localTyped.containsKey(leftLocal)) {
            ...
            if (!localTyped.containsKey(rightLocal)) {
              // Debug.printDbg(IDalvikTyper.DEBUG, "set type ", leftLocalType, " to
              // local ", rightLocal);
            ...
            if (!localTyped.containsKey(leftLocal)) {
              // Debug.printDbg(IDalvikTyper.DEBUG, "set type ", rightLocalType, " to
              // local ", leftLocal);
          ...
          if (localTyped.containsKey(base)) {
            ...
            // Debug.printDbg(IDalvikTyper.DEBUG, "type of local1: ", t, " ",
            // t.getClass());
            ...
            if (!localTyped.containsKey(rightLocal)) {
              // Debug.printDbg(IDalvikTyper.DEBUG, "set type ", elementType, " to
              // local ", r);
          ...
          if (localTyped.containsKey(base)) {
            ...
            // Debug.printDbg(IDalvikTyper.DEBUG, "type of local2: ", t, " ",
            // t.getClass());
            Type elementType = null;
            if (t instanceof ArrayType) {
              ArrayType at = (ArrayType) t;
    ...
    for (Unit u : b.getUnits()) {
      ...
      AssignStmt ass = (AssignStmt) u;
      if (!(ass.getLeftOp() instanceof Local)) continue;
      if (!(ass.getRightOp() instanceof UntypedConstant)) continue;
    ...
    for (Constraint c : constraints) {
      ...
      if (l instanceof Local && r instanceof Constant) {
        if (r instanceof UntypedIntOrFloatConstant) {
          ...
          Value newValue = null;
          if (cst.value != 0) {
            // Debug.printDbg(IDalvikTyper.DEBUG, "[untyped constaints] set type int to
            // non zero constant: ", c, " = ", cst.value);
          ...
          // Debug.printDbg(IDalvikTyper.DEBUG, "[untyped constaints] set type long to
          // constant: ", c);
          ...
          new StmtSwitch() {
            ...
            public void caseAssignStmt(AssignStmt stmt) {
              ...
              if (stmt.getLeftOp() instanceof ArrayRef
                  && stmt.getRightOp() instanceof UntypedConstant) {
                UntypedConstant uc = (UntypedConstant) stmt.getRightOp();
              ...
              if (c instanceof BinopExpr) {
                ...
                if (op1 instanceof UntypedConstant || op2 instanceof UntypedConstant) {
                  ...
                  if (op1 instanceof Local) {
                    ...
                    if (op1 instanceof UntypedIntOrFloatConstant
                        && op2 instanceof UntypedIntOrFloatConstant) {
                      UntypedIntOrFloatConstant uc1 = (UntypedIntOrFloatConstant) op1;
                      UntypedIntOrFloatConstant uc2 = (UntypedIntOrFloatConstant) op2;
                      be.setOp1(uc1.toIntConstant()); // to int or float,
                      // it does not matter
                      be.setOp2(uc2.toIntConstant());
                    } else if (op1 instanceof UntypedLongOrDoubleConstant
                        && op2 instanceof UntypedLongOrDoubleConstant) {
                      ...
                      be.setOp1(uc1.toLongConstant()); // to long or
                      // double, it does
                      // not matter
                      be.setOp2(uc2.toLongConstant());
                    } else {
                      throw new RuntimeException(
                          "error: expected same type of untyped constants. Got " + stmt);
                    }
                  } else if (op1 instanceof UntypedConstant || op2 instanceof UntypedConstant) {
                    ...
                    throw new RuntimeException(
                        "error: expected local/untyped untyped/local or untyped/untyped. Got "
                            + stmt);
                  }
                }
  ...
  protected void checkExpr(Value v, Type t) {
    ...
    for (ValueBox vb : v.getUseBoxes()) {
      ...
      if (value instanceof Local) {
        ...
        if ((v instanceof ShrExpr || v instanceof ShlExpr || v instanceof UshrExpr)
            && ((BinopExpr) v).getOp2() == value) {
          // Debug.printDbg("setting type of operand two of shift expression to integer",
          // value);
  ...
  protected void setInvokeType(InvokeExpr invokeExpr) {
    ...
    if (invokeExpr instanceof StaticInvokeExpr) {
      ...
      throw new RuntimeException(
          "error: unhandled invoke expression: " + invokeExpr + " " + invokeExpr.getClass());
    }
  }
