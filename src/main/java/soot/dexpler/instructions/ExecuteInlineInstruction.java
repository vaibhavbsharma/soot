...
public class ExecuteInlineInstruction extends MethodInvocationInstruction
  ...
  public void deOdex(DexFile parentFile, Method method, ClassPath cp) {
    ...
    InlineMethodResolver inlineMethodResolver =
        InlineMethodResolver.createInlineMethodResolver(odexFile.getOdexVersion());
    MethodAnalyzer analyzer = new MethodAnalyzer(cp, method, inlineMethodResolver, false);
    targetMethod =
        inlineMethodResolver.resolveExecuteInline(
            new AnalyzedInstruction(analyzer, instruction, -1, -1));
  }
