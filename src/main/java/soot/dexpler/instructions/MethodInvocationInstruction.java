...
public abstract class MethodInvocationInstruction extends DexlibAbstractInstruction
  ...
  public void finalize(DexBody body, DexlibAbstractInstruction successor) {
    ...
    if (successor instanceof MoveResultInstruction) {
      ...
      body.add(assign);
      unit = assign;
      // this is a invoke statement (the MoveResult had to be the direct successor for an
      // expression)
    } else {
  ...
  private SootMethodRef getSootMethodRef(InvocationType invType) {
    ...
    methodRef =
        Scene.v()
            .makeMethodRef(
                sc, methodName, parameterTypes, returnType, invType == InvocationType.Static);
  ...
  protected void jimplifyVirtual(DexBody body) {
    ...
    invocation =
        Jimple.v()
            .newVirtualInvokeExpr(parameters.get(0), ref, parameters.subList(1, parameters.size()));
    ...
    invocation =
        Jimple.v()
            .newSpecialInvokeExpr(
                parameters.get(0), getSootMethodRef(), parameters.subList(1, parameters.size()));
    body.setDanglingInstruction(this);
  }
