 ...
 * given to rightValue. This default type is "int" for registers whose size is less or equal to
 * 32bits and "long" to registers whose size is 64bits. The problem is that 32bits registers could
 * be either "int" or "float" and 64bits registers "long" or "double". If the analysis concludes
 * that an "int" has to be changed to a "float", rightValue has to change from
 * IntConstant.v(literal) to Float.intBitsToFloat((int) literal). If the analysis concludes that an
 * "long" has to be changed to a "double, rightValue has to change from LongConstant.v(literal) to
  ...
  protected void internalTransform(final Body body, String phaseName, Map<String, String> options) {
    ...
    for (Local loc : getNumCandidates(body)) {
      ...
      for (Unit u : defs) {
        ...
        final Local l =
            u instanceof DefinitionStmt ? (Local) ((DefinitionStmt) u).getLeftOp() : null;
        // check defs
        u.apply(
          ...
          use.apply(
              new AbstractStmtSwitch() {
                ...
                public void caseReturnStmt(ReturnStmt stmt) {
                  usedAsFloatingPoint =
                      stmt.getOp() == l && isFloatingPointLike(body.getMethod().getReturnType());
                  doBreak = true;
                  return;
                }
              });
