...
public class DexAnnotation {
                ...
                if (ae instanceof AnnotationAnnotationElem) {
                  ...
                  for (SootMethod sm : clazz.getMethods()) {
                    ...
                    if (defaults.containsKey(methodName)) {
                      ...
                      // Okay, the name is the same, but is it actually the
                      // same type?
                      Type annotationType = getSootType(e);
                      boolean isCorrectType = false;
                      if (annotationType == null) {
                        // we do not know the type of the annotation, so we
                        // guess it's the correct type.
                        isCorrectType = true;
                      } else {
                      ...
                      if (isCorrectType && sm.getParameterCount() == 0) {
                        ...
                        // In case there is more than one matching method,
                  ...
                  for (Entry<String, AnnotationElem> leftOverEntry : defaults.entrySet()) {
                    // We were not able to find a matching method for the tag,
                    // because the return signature
                    // does not match
    ...
    for (Annotation a : annotations) {
      ...
      if (atypes.equals("dalvik.annotation.AnnotationDefault")) {
        ...
        if (clazz.hasOuterClass()) {
          ...
          if (Util.isByteCodeClassName(classType)) outerClass += ";";
        } else if (classType.contains("$")) {
          // remove everything after the last '$' including the last '$'
        ...
        for (AnnotationElem ae : e.getValues()) {
          ...
          int accessFlags = 0; // seems like this information is lost during the .class -- dx -->
          // .dex process.
          Tag innerTag =
              new InnerClassTag(
  ...
  private ArrayList<AnnotationElem> getElements(Set<? extends AnnotationElement> set) {
    ...
    for (AnnotationElement ae : set) {
      ...
      // Debug.printDbg("element: ", ae.getName() ," ", ae.getValue() ," type: ",
      // ae.getClass());
