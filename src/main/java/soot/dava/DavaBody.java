...
public class DavaBody extends Body {
  ...
  DavaBody(Body body) {
    ...
    AugmentedStmtGraph asg =
        new AugmentedStmtGraph(new BriefUnitGraph(this), new TrapUnitGraph(this));
  ...
  private void applyASTAnalyses(ASTNode AST) {
    ...
    */
    ...
    if (flag) {
      ...
      do {
        ...
        */
        ...
        debug(
            "applyASTAnalyses", "after OraggregatorTwo ifElseFlipped is" + G.v().ASTIfElseFlipped);
        ...
        debug(
            "applyASTAnalyses",
            "after NewStringBufferSimplification" + G.v().ASTTransformations_modified);
        ...
        debug(
            "applyASTAnalyses",
            "after UselessLabeledBlockRemover" + G.v().ASTTransformations_modified);
        ...
        if (!G.v().ASTTransformations_modified) {
          ...
          debug(
              "applyASTAnalyses",
              "after UselessAbruptStmtRemover" + G.v().ASTTransformations_modified);
        }
        ...
        if (G.v().ASTTransformations_modified) {
          ...
          debug(
              "applyASTanalyses",
              "Transformation modified was true hence will reiterate. set flipped to false");
        } else {
          ...
          if (G.v().ASTIfElseFlipped) {
            ...
            if (countFlipping == 0) {
              ...
              // let this go on just once more in the hope of some other pattern being
              // matched
              G.v().ASTIfElseFlipped = false;
              countFlipping++;
    ...
    debug(
        "applyASTAnalyses", "after ClosestAbruptTargetFinder" + G.v().ASTTransformations_modified);
  ...
  private void copy_Body(Body body) {
...

    {
      ...
      for (Unit u : getUnits()) {
        ...
        } else if (s instanceof InvokeStmt) javafy(((InvokeStmt) s).getInvokeExprBox());
      }
    }
  ...
  private void javafy_unop_expr(ValueBox vb) {
    ...
    if (uoe instanceof LengthExpr) vb.setValue(new DLengthExpr(((LengthExpr) uoe).getOp()));
    else if (uoe instanceof NegExpr) vb.setValue(new DNegExpr(((NegExpr) uoe).getOp()));
  }
  ...
  private void javafy_invoke_expr(ValueBox vb) {
    ...
    if (ie instanceof InstanceInvokeExpr) {
      ...
      if (ie instanceof VirtualInvokeExpr) {
        ...
        vb.setValue(
            new DVirtualInvokeExpr(vie.getBase(), vie.getMethodRef(), vie.getArgs(), thisLocals));
