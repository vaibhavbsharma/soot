...
public class SETTryNode extends SETNode {
  ...
  public IterableSet get_NaturalExits() {
    ...
    while (it.hasNext()) {
      ...
      Iterator eit =
          ((SETNode) body2childChain.get(it.next()).getLast()).get_NaturalExits().iterator();
      while (eit.hasNext()) {
        Object o = eit.next();

        if (c.contains(o) == false) c.add(o);
      }
    }
  ...
  public ASTNode emit_AST() {
    ...
    while (it.hasNext()) {
      ...
      while (bit.hasNext()) {
        ...
        if (s instanceof GotoStmt) s = (Stmt) ((GotoStmt) s).getTarget();
    ...
    return new ASTTryNode(
        get_Label(),
        emit_ASTBody(body2childChain.get(en.get_TryBody())),
        catchList,
        exceptionMap,
        paramMap);
  }
      ...
      if (subBody.intersects(en.get_TryBody())) {
        ...
        while (ccit.hasNext()) {
          ...
          if ((childBody.intersects(en.get_TryBody()) == false)
              || (childBody.isSubsetOf(en.get_TryBody()))) continue;

          if (childBody.isSupersetOf(get_Body())) return true;
          ...
          while (cit.hasNext()) {
            ...
            while (bit.hasNext()) {
              ...
              if (childBody.contains(as) == false) remove_AugmentedStmt(as);
