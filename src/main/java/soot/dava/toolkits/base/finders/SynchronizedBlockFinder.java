...
public class SynchronizedBlockFinder implements FactFinder {
    ...
    while (asgit.hasNext()) {
      ...
      if (as.get_Stmt() instanceof EnterMonitorStmt) {
        ...
        if (synchSet != null) {
          ...
          /*
           * Synch bug due to copy stmts r2 = r1 enter monitor r1
           * synch body exit monitor r2 we need to realize that r1 and
           * r2 are the same implemented as a hack, the correct
           * approach is reaching copies
           */
            ...
            if (verify_CatchBody(en, synchBody, local, copiedLocal)) {
              if (SET.nest(new SETSynchronizedBlockNode(en, local))) {
                ...
                while (ssit.hasNext()) {
                  ...
                  /*
                   * Jeromes Implementation: changed because
                   * of copy stmt bug
                   *
                   * if ((sss instanceof MonitorStmt) &&
                   * (((MonitorStmt) sss).getOp() == local) &&
                   * (((Integer) ((HashMap) as2ml.get(
                   * ssas)).get( local)).equals( level)) &&
  ...
  /*
   * as2locals: contains all locals which have unknown level for this
   * augmented stmt viAugStmts: contains all augmented stmts which have some
   * unknown level local in them
   */

  private void find_VariableIncreasing(
      AugmentedStmtGraph asg,
      HashMap local2level_template,
      LinkedList<AugmentedStmt> viAugStmts,
      ...
      while (sit.hasNext()) {
        ...
        while (mlsit.hasNext()) {
          ...
          if ((local2level.get(local) == VARIABLE_INCR)
              && (slocal2level.get(local) != VARIABLE_INCR)) {
            slocal2level.put(local, VARIABLE_INCR);

            if (worklist.contains(sas) == false) worklist.addLast(sas);
          }
        }
      }
    }
    /*
     * At the end of this the worklist is empty the unknown level of locals
     * of an augmentedstmt present in the viSeed has been propagated through
     * out the stmts reachable from that stmt
     */
    ...
    while (asgit.hasNext()) {
      ...
      while (mlsit.hasNext()) {
        ...
        if (local2level.get(local) == VARIABLE_INCR) {
          ...
          /*
           * Nomair A. Naeem 08-FEB-2005 BUG IN CODE. The getLast
           * method returns a NoSuchElementException if list is empty.
           * Protect this by checking isEmpty
           */
          if (!viAugStmts.isEmpty()) {
            if (viAugStmts.getLast() != as) viAugStmts.addLast(as);
          } else {
  ...
  /*
   * Inputs: as: the seed stmt, i.e. the first statement of a strongly
   * connected component component: the strongly connected stmt being
   * processed as2rml: a hashmap which contains an augmentedstmt as key and a
   * hashmap with local to level mapping as value as2color: maps each
   * augmentedstmt of a scc to a color (white initially) viSeeds: variable
   * increasing levels, initially empty
   *
   * Purpose: makes a traversal of the entire scc and increases the levels of
   * locals when used in monitor enter decreases the levels of locals when
   * leaving monitor if a successor has never been visited invoke same method
   * on it with updated level information if a sucessor has been visited
   * before and has a lower level than a seed the level of the sucessor is
   * marked unknown and is added to the viSeeds list
   */
  private void DFS_Scc(
      AugmentedStmt as,
      IterableSet component,
      HashMap as2rml,
      HashMap as2color,
      AugmentedStmt seedStmt,
      IterableSet viSeeds) {
    ...
    while (sit.hasNext()) {
      ...
      if (component.contains(sas) == false) continue;
      ...
      if (scolor.equals(WHITE)) {
        ...
        while (mlsit.hasNext()) {
          ...
          Value local = (Value) mlsit.next();
          /*
           * update the sucessors hashtable with level values from the
           * seed stmt The loca2Level is the hashmap of local-->level
           * for the seed The sLocal2Level is the hashmap of local -->
           * level for the successor of the seed
           */
        ...
        while (mlsit.hasNext()) {
          ...
          if (slocal2level.get(local).intValue() < local2level.get(local).intValue()) {
            ...
            if (viSeeds.contains(sas) == false) viSeeds.add(sas);
          }
        }
      }
    }
  ...
  /*
   * Created a synch set for each enter monitor stmt. The synch set contains
   * all sucessors of the monitor enter stmt which is dominated by the enter
    ...
    while (mesit.hasNext()) {
      ...
      while (worklist.isEmpty() == false) {
        ...
        while (sit.hasNext()) {
          ...
          /*
           * if the sucessor is dominated by the head stmt and the
           * level is greater or equal to that of the head and is not
           * waiting to be analysed and is not in the synchSet.. then
           * add it to worklist
           */
          if (sas.get_Dominators().contains(headAs)
              && (sml >= monitorLevel)
              && (worklist.contains(sas) == false)
              && (synchSet.contains(sas) == false)) worklist.addLast(sas);
        }
      }
   ...
   * MonitorEnterSet: contains all monitor enter statements as2ml : key is
   * each augmented stmt of the augmented graph, value is a hashmap which
   * contains a local as key and the level as value
   */
  private void set_MonitorLevels(AugmentedStmtGraph asg) {
    ...
    while (asgit.hasNext()) {
      ...
      if (s instanceof MonitorStmt) {
        ...
        if (monitorLocalSet.contains(local) == false) monitorLocalSet.add(local);

        // add the monitor enter statement to the monitorEnter Set
        if (s instanceof EnterMonitorStmt) monitorEnterSet.add(as);
      }
    }
    ...
    HashMap<AugmentedStmt, LinkedList<Value>> incrAs2locals =
        new HashMap<AugmentedStmt, LinkedList<Value>>();

    // setup the variable increasing monitor levels
    find_VariableIncreasing(asg, local2level_template, viAugStmts, incrAs2locals);
    /*
     * At this time the viAugStmts contains all augments Stmt which have
     * some local that has unknown level the incrAs2locals contains a map of
     * augmented stmt to a linkedlist which contains locals with unknown
     * level
     */
      ...
      while (pit.hasNext()) {
        ...
        while (mlsit.hasNext()) {
          ...
          if (ps instanceof ExitMonitorStmt) {
            ...
            /*
             * if the predecessor stmt is a monitor exit and the
             * local it exits is the same as this local and the
             * predLevel is greater than 0 decrease the pred level
             */
          ...
          if (s instanceof EnterMonitorStmt) {
            ...
            /*
             * if the stmt is a monitor enter (which is true for the
             * initial worklist) and the local it enters is the same
             * as this local and the predLevel is greater or equal
             * to 0 increase the pred level
             */
            if ((ems.getOp() == local) && (predLevel >= 0)) predLevel++;
          }
          ...
          /*
           * if the pred level is greater than the current level
           * update current level add the sucessors of the stmt to the
           * worklist
           */
          if (predLevel > curLevel) {
            ...
            while (sit.hasNext()) {
              ...
              if (worklist.contains(so) == false) worklist.add(so);
            }
          }
        }
      }
  ...
  /*
   * If a stmt is removed from the synchBody then all other stmts which are
   * dominated by this stmt should also be removed from the synchBody
   */
  ...
  private boolean verify_CatchBody(
      ExceptionNode en, IterableSet synchBody, Value monitorVariable, Value copiedLocal) {
    // System.out.println("starting verification");
    {
      /*
       * synchBody is a likely superset of exception Node since the
       * synchBody contains a goto stmt to the stmt right after the
       * "to be created synch block" See if this is the case and remove
       * the stmt
       */
     ...
     * There is an error because the synchBody created has addition
     * statements regarding the catching of exception due to the outer try
     * block
     *
     * Since these statements are not part of the synch body they should be
     * removed
     */
    {
          ...
          while (succIt.hasNext()) {
            ...
            if (as1.get_Stmt() instanceof GotoStmt) {
              ...
              if (target instanceof DefinitionStmt) {
                ...
                /*
                 * These are the caught exceptions (can be more
                 * than once if there are multiple areas of
                 * protection Keep the one which is of type
                 * Throwable
                 */
                ...
                if (!(asnFrom instanceof CaughtExceptionRef)) {
                  // System.out.println("REMOVING:"+defStmt+" since this is not a
                  // caughtexception def");
    ...
    /*
     * There might be some unwanted stmts in the synch body. These are
     * likely to be stmts which have no predecessor in the synchbody. Remove
     * these stmts. NOTE: the entry of the synchbody is a special case since
     * its predecessor is also not going to be in the synchbody but we
     * SHOULD not remove the entry point
     */

    // find the entry point of synchbody
    /*
     * One way of doing this is going through the synch body and finiding a
     * stmt whose predecessor is an enter monitor stmt NOT present in the
     * synchBody
     */
    ...
    if (synchEnter == null) {
      throw new RuntimeException(
          "Could not find enter stmt of the synchBody: " + davaBody.getMethod().getSignature());
    }

    // System.out.println("Enter stmt of synchBody is:"+synchEnter);

    /*
     * Now that we know the exception case we can go through the synchBody
     * and remove all stmts whose predecessor does not belong to the
     * synchBody
     */
    ...
    while (!unChanged) {
      ...
      if (toRemove.size() > 0) {
        ...
        // System.out.println("Removing:"+toRemove+" since none of its preds are in the
        // synchBody");
        unChanged = false;
      }
    }
    ...
    /*
     * The two bodies match check if the exception thrown is of type
     * "throwable" and that there is only one catchlist The reason for doing
     * this is that synchronized blocks get converted to a try catch block
     * with the catch , catching the throwable exception and this is the
     * ONLY catch
     */
    if ((en.get_Exception().getName().equals(THROWABLE) == false)
    ...
    /*
     * find the entry point of the catch boody The entry point of the catch
     * body is the first stmt in the catchbody This can be found by finiding
     * the stmt whose predecessor is not part of the catch body
     */
    ...
    /*
     * To verify the catchbody one has to go through the catchbody ONE stmt
     * at a time. This is NOT a good approach as it is using PATTERN
     * MATCHING
     *
     * FIRST STEP Need to verify that the entrypoint stmt and any goto stmts
     * following it all point to the entrypoint
     */
    ...
    /*
     * The escuccs contains all the EXCEPTION SUCESSORS of the
    ...
    if ((((ExitMonitorStmt) s).getOp() != monitorVariable)) {
      ...
      if ((((ExitMonitorStmt) s).getOp() != copiedLocal)) {
        // System.out.println("exit monitor variable does not match enter monitor
        // variable");
        return false;
      }
    }
   ...
   * sucessors of as should directly or indirectly point to s
   */
  ...
  /*
   * Input: head: this is the monitor enter stmt synchSet: this contains all
   * sucessors of head which have level greater or equal to the head
   */
    ...
    while (bit.hasNext()) {
      ...
      if ((s instanceof ExitMonitorStmt)
        ...
        while (sit.hasNext()) {
          ...
          if (sas.get_Dominators().contains(head) == false) continue;

          Stmt ss = sas.get_Stmt();

          if (((ss instanceof GotoStmt) || (ss instanceof ThrowStmt))
              && (body.contains(sas) == false)) {
