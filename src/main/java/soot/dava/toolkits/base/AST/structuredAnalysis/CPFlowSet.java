...
public class CPFlowSet extends DavaFlowSet<CPTuple> {
  ...
  public CPFlowSet(CPFlowSet other) {
    ...
    implicitBreaks =
        (HashMap<Serializable, List<DavaFlowSet<CPTuple>>>) other.implicitBreaks.clone();
    implicitContinues =
        (HashMap<Serializable, List<DavaFlowSet<CPTuple>>>) other.implicitContinues.clone();
  }
      ...
      if (matchFound != null) {
        ...
        if (thisTuple.isTop()) {
          ...
          if (matchedValue.equals(thisValue)) {
            ...
            // if we get here either the types dont match or the values didnt match just
            // add top
            workingSet.add(new CPTuple(className, thisVar, true));
          }
        } else {
          throw new DecompilationException(
              "Ran out of cases in CPVariable values...report bug to developer");
        }
      } // if match was found
      else {
