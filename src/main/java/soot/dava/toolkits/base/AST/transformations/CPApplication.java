...
public class CPApplication extends DepthFirstAdapter {
  ...
  public void inASTSwitchNode(ASTSwitchNode node) {
    ...
    if (key instanceof Local) {
      ...
      if (value != null) {
        // System.out.println("switch key Local "+useLocal+"is present in before set with
        // value"+value);
        ...
        if (newValue != null) {
          // System.out.println("Substituted the switch key local use with the constant
          // value"+newValue);
          node.set_Key(newValue);
        } else {
          // System.out.println("FAILED TO Substitute the local use with the constant
          // value");
        }
      }
      ...
      Object value =
          beforeSet.contains(
              usedSootField.getDeclaringClass().getName(), usedSootField.getName().toString());
      if (value != null) {
        // System.out.println("FieldRef "+usedSootField+"is present in before set with
        // value"+value);
        // create constant value for the value and replace this local
        // use with the constant value use
        Value newValue = CPHelper.createConstant(value);
        if (newValue != null) {
          // System.out.println("Substituted the constant field ref use with the constant
          // value"+newValue);
          node.set_Key(newValue);
        } else {
          // System.out.println("FAILED TO Substitute the constant field ref use with the
          // constant value");
        }
      }
    }
  ...
  public ASTCondition changedCondition(ASTCondition cond, CPFlowSet set) {
    if (cond instanceof ASTAggregatedCondition) {
      ...
      if (val instanceof Local) {
        Object value = set.contains(className, val.toString());
        if (value != null) {
          // System.out.println("if Condition Local "+((Local)val)+"is present in before
          // set with value"+value);
          // create constant value for the value and replace this
          // local use with the constant value use
          Value newValue = CPHelper.createConstant(value);
          if (newValue != null) {
            // System.out.println("Substituted the local use with the constant
            // value"+newValue);
        ...
        Object value =
            set.contains(
                usedSootField.getDeclaringClass().getName(), usedSootField.getName().toString());
        if (value != null) {
          // System.out.println("if condition FieldRef "+usedSootField+"is present in
          // before set with value"+value);
          // create constant value for the value and replace this
          // field use with the constant value use
          Value newValue = CPHelper.createConstant(value);
          if (newValue != null) {
            // System.out.println("Substituted the constant field ref use with the
            // constant value"+newValue);
            ((ASTUnaryCondition) cond).setValue(newValue);
          } else {
            // System.out.println("FAILED TO Substitute the constant field ref use with
            // the constant value");
          }
        }
      } else {
  ...
  public void substituteUses(List useBoxes, CPFlowSet beforeSet) {
    ...
    while (useIt.hasNext()) {
      ...
      if (use instanceof Local) {
        ...
        Object value = beforeSet.contains(className, useLocal.toString());
        if (value != null) {
          // System.out.println("Local "+useLocal+"is present in before set with
          // value"+value);
          // create constant value for the value and replace this
          // local use with the constant value use
          Value newValue = CPHelper.createConstant(value);
