...
public class UnreachableCodeEliminator extends DepthFirstAdapter {
  ...
  public void inASTStatementSequenceNode(ASTStatementSequenceNode node) {
    ...
    for (AugmentedStmt as : node.getStatements()) {
      ...
      if (!codeFinder.isConstructReachable(s)) {
        toRemove.add(as);
        // if(DEBUG) System.out.println("Statement "+s.toString()+ " is NOT REACHABLE REMOVE
        // IT");
      }
    }
    ...
    while (sbit.hasNext()) {
      ...
      while (it.hasNext()) {
        ...
        if (!codeFinder.isConstructReachable(temp)) {
          // System.out.println("-------------------------A child of node of type
          // "+node.getClass()+" whose type is "+temp.getClass()+" is unreachable");
          toReturn.add(temp);
        } else {
  ...
  private void dealWithSwitchNode(ASTSwitchNode node) {
    ...
    while (it.hasNext()) { // going through all the cases of the switch statement
      ...
      while (itBody.hasNext()) {
        ASTNode temp = (ASTNode) itBody.next();
        // System.out.println("Checking whether child of type "+temp.getClass()+" is
        // reachable");
        if (!codeFinder.isConstructReachable(temp)) {
          // System.out.println(">>>>>>>>>>>>>>>>>-------------------------A child of node
          // of type "+node.getClass()+" whose type is "+temp.getClass()+" is
          // unreachable");
          toReturn.add(temp);
        } else {
