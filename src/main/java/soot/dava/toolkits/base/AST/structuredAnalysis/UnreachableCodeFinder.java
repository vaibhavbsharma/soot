...
public class UnreachableCodeFinder extends StructuredAnalysis {
  ...
  public class UnreachableCodeFlowSet extends DavaFlowSet {
    ...
    public void intersection(FlowSet otherFlow, FlowSet destFlow) {
      ...
      if (dest == other || dest == this) workingSet = new UnreachableCodeFlowSet();
      else {
        workingSet = dest;
        workingSet.clear();
      }
  ...
  public DavaFlowSet handleBreak(String label, DavaFlowSet output, ASTNode node) {
    ...
    if ((explicitSet != null && explicitSet.size() > 0)
        || (implicitSet != null && implicitSet.size() > 0)) {
      if (DEBUG)
        System.out.println(
            "\tBreak sets (implicit and explicit are nonempty hence forwarding true");
      ...
      if (DEBUG)
        System.out.println(
            "\tBreak sets (implicit and explicit are empty hence forwarding merge of false with inset");
  ...
  @Override
  public DavaFlowSet processASTStatementSequenceNode(
      ASTStatementSequenceNode node, DavaFlowSet input) {
    ...
    if (!isReachable(input)) {
      ...
      return input;
    } else return super.processASTStatementSequenceNode(node, input);
  }
  ...
  public DavaFlowSet processASTLabeledBlockNode(ASTLabeledBlockNode node, DavaFlowSet input) {
    ...
    if (!isReachable(input)) {
      // this sequence is not reachable hence simply return inset
      return input;
    } else return super.processASTLabeledBlockNode(node, input);
  }

  @Override
  public DavaFlowSet processASTSynchronizedBlockNode(
      ASTSynchronizedBlockNode node, DavaFlowSet input) {
    if (DEBUG) System.out.println("Processing synchronized block node");
    if (!isReachable(input)) {
      // this sequence is not reachable hence simply return inset
      return input;
    } else return super.processASTSynchronizedBlockNode(node, input);
  }
  ...
  public DavaFlowSet ifNotReachableReturnInputElseProcessBodyAndReturnTrue(
      ASTNode node, DavaFlowSet input) {
  ...
  @Override
  public DavaFlowSet processASTUnconditionalLoopNode(
      ASTUnconditionalLoopNode node, DavaFlowSet input) {
    return ifNotReachableReturnInputElseProcessBodyAndReturnTrue(node, input);
  }
  ...
  public DavaFlowSet processASTSwitchNode(ASTSwitchNode node, DavaFlowSet input) {
    ...
    if (out != null) { // just to make sure that there were some cases present
      if (defaultOut != null) output = merge(defaultOut, out);
      else output = merge(initialIn, out);
    }
    ...
    for (DavaFlowSet tmb : toMergeBreaks) outList.add(handleBreak(label, tmb, node));

    // merge all outList elements. since these are the outputs with breaks handled
    DavaFlowSet finalOut = output;
    for (DavaFlowSet fo : outList) finalOut = merge(finalOut, fo);
  ...
  public boolean isConstructReachable(Object construct) {
    ...
    if (temp == null) return true;
      ...
      if (DEBUG) System.out.println("Have before set");
