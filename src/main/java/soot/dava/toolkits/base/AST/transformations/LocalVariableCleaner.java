...
public class LocalVariableCleaner extends DepthFirstAdapter {
  ...
  public boolean canRemoveDef(DefinitionStmt ds) {
    ...
    if (ds.getRightOp() instanceof Local || ds.getRightOp() instanceof Constant) return true;
  ...
  public boolean removeStmt(Stmt stmt) {
    ...
    if (newSequence.size() < size) return true; // size of new node is smaller than orignal size
