...
public abstract class StructuredAnalysis<E> {
  ...
  public StructuredAnalysis() {
    ...
    if (MERGETYPE == UNDEFINED) throw new RuntimeException("MERGETYPE UNDEFINED");
  }
  ...
  /**
   * Specific stmts within AST Constructs are processed through this method. It will be invoked
   * everytime a stmt is encountered
   */
  public abstract DavaFlowSet<E> processStatement(Stmt s, DavaFlowSet<E> input);

  /**
   * To have maximum flexibility in analyzing conditions the analysis API breaks down the aggregated
   * conditions to simple unary or binary conditions user defined code can then deal with each
   * condition separately. To be able to deal with entire aggregated conditions the user should wite
   * their own implementation of the method processCondition
   */
  public abstract DavaFlowSet<E> processUnaryBinaryCondition(
      ASTUnaryBinaryCondition cond, DavaFlowSet<E> input);
  ...
  /**
   * This implementation breaks down the aggregated condition to the terminal conditions which all
   * have type ASTUnaryBinaryCondition. Once these are obtained the abstract method
   * processUnaryBinaryCondition is invoked. For aggregated conditions the merging is done in a
   * depth first order of the condition tree.
   */
  ...
  public DavaFlowSet<E> process(Object body, DavaFlowSet<E> input) {
    if (body instanceof ASTNode) {
      ...
      throw new RuntimeException(
          "Body sent to be processed by " + "StructuredAnalysis is not a valid body");
    }
  }
  ...
  /**
   * This method is called from the specialized ASTNodes. The purpose was to deal with different
   * ASTNodes with similar structure in one go. The method will deal with retrieve the body of the
   * ASTNode which are known to have only one subBody
   */
  ...
  /**
   * Whenever a statement has to be processed the first step is to invoke this method. This is to
   * remove the tedious work of adding code to deal with abrupt control flow from the programmer of
   * the analysis. The method invokes the processStatement method for all other statements
   *
   * <p>A programmer can decide to override this method if they want to do something specific
   */
  public DavaFlowSet<E> processAbruptStatements(Stmt s, DavaFlowSet<E> input) {
    if (s instanceof ReturnStmt || s instanceof RetStmt || s instanceof ReturnVoidStmt) {
      ...
      if (nodeLabel != null && nodeLabel.toString() != null) {
        ...
        if (abStmt.is_Continue()) temp.addToContinueList(nodeLabel.toString(), input);
        else if (abStmt.is_Break()) temp.addToBreakList(nodeLabel.toString(), input);
        else throw new RuntimeException("Found abruptstmt which is neither break nor continue");
      } else {
        // found implicit break/continue
        if (abStmt.is_Continue()) temp.addToImplicitContinues(abStmt, input);
        else if (abStmt.is_Break()) temp.addToImplicitBreaks(abStmt, input);
        else throw new RuntimeException("Found abruptstmt which is neither break nor continue");
      }
      return temp;
    } else {
  ...
  public DavaFlowSet<E> processASTStatementSequenceNode(
      ASTStatementSequenceNode node, DavaFlowSet<E> input) {
  ...
  public DavaFlowSet<E> processASTSynchronizedBlockNode(
      ASTSynchronizedBlockNode node, DavaFlowSet<E> input) {
  ...
  public DavaFlowSet<E> processASTWhileNode(ASTWhileNode node, DavaFlowSet<E> input) {
    ...
    if (DEBUG_WHILE) System.out.println("Going out of while: " + temp.toString());
    return temp;
  }
  ...
  public DavaFlowSet<E> processASTDoWhileNode(ASTDoWhileNode node, DavaFlowSet<E> input) {
    ...
    DavaFlowSet<E> initialInput = cloneFlowSet(input);
    String label = getLabel(node);
    if (DEBUG_WHILE) System.out.println("Going into do-while: " + initialInput.toString());
    ...
    if (DEBUG_WHILE) System.out.println("Going out of do-while: " + temp.toString());

    return temp;
  }

  public DavaFlowSet<E> processASTUnconditionalLoopNode(
      ASTUnconditionalLoopNode node, DavaFlowSet<E> input) {
    // an unconditional loop behaves almost like a conditional While loop
    DavaFlowSet<E> initialInput = cloneFlowSet(input);
    DavaFlowSet<E> lastin = null;
    if (DEBUG_WHILE) System.out.println("Going into while(true): " + initialInput.toString());
    ...
    if (DEBUG_WHILE) System.out.println("Going out of while(true): " + temp.toString());
    return temp;
  }
  ...
  public DavaFlowSet<E> processASTSwitchNode(ASTSwitchNode node, DavaFlowSet<E> input) {
    if (DEBUG) System.out.println("Going into switch: " + input.toString());
    ...
    for (DavaFlowSet<E> mset : toMergeBreaks) outList.add(handleBreak(label, mset, node));

    // merge all outList elements. since these are the outputs with breaks
    // handled
    DavaFlowSet<E> finalOut = output;
    for (DavaFlowSet<E> outIt : outList) finalOut = merge(finalOut, outIt);

    if (DEBUG) System.out.println("Going out of switch: " + finalOut.toString());

    return finalOut;
  }

  public DavaFlowSet<E> processASTTryNode(ASTTryNode node, DavaFlowSet<E> input) {
    if (DEBUG_TRY) System.out.println("TRY START is:" + input);
    ...
    if (DEBUG_TRY) System.out.println("TRY initialFLOW is:" + inputCatch);
    ...
    for (DavaFlowSet<E> co : catchOutput) {
      ...
      if (DEBUG_TRY) System.out.println("TRY handling breaks is:" + temp);
    ...
    for (DavaFlowSet<E> co : outList) out = merge(out, co);

    if (DEBUG_TRY) System.out.println("TRY after merge outList is:" + out);

    // System.out.println("After handling break"+out);

    for (DavaFlowSet<E> co : catchOutput) out = merge(out, co);

    if (DEBUG_TRY) System.out.println("TRY END RESULT is:" + out);
  ...
  public DavaFlowSet<E> merge(DavaFlowSet<E> in1, DavaFlowSet<E> in2) {
    if (MERGETYPE == 0)
      throw new RuntimeException(
          "Use the setMergeType method to set the type of merge used in the analysis");
    ...
    if (in1 == NOPATH && in2 != NOPATH) {
      ...
      else throw new RuntimeException("Merge type value" + MERGETYPE + " not recognized");
      out.copyInternalDataFrom(in1);
      out.copyInternalDataFrom(in2);
      return out;
    }
  ...
  public DavaFlowSet<E> mergeExplicitAndImplicit(
      String label,
  ...
  /**
   * Need to handleBreak stmts There can be explicit breaks (in which case label is non null) There
   * can always be implicit breaks ASTNode is non null
   */
  public DavaFlowSet<E> handleBreak(String label, DavaFlowSet<E> out, ASTNode node) {
    ...
    if (node == null) throw new RuntimeException("ASTNode sent to handleBreak was null");
  ...
  /**
   * Need to handleContinue stmts There can be explicit continues (in which case label is non null)
   * There can always be implicit continues ASTNode is non null
   */
  public DavaFlowSet<E> handleContinue(String label, DavaFlowSet<E> out, ASTNode node) {
    ...
    // getting the implicit list now
    if (node == null) throw new RuntimeException("ASTNode sent to handleContinue was null");
  ...
  /**
   * Invoked from within the UnconditionalWhile processing method Need to handle both explicit and
   * implicit breaks
   */
  private DavaFlowSet<E> getMergedBreakList(String label, DavaFlowSet<E> output, ASTNode node) {
    ...
    if (implicitSet != null) {
      ...
      if (implicitSet.size() > 0) toReturn = it.next();
  ...
  public void debug(String methodName, String debug) {
    if (DEBUG)
      System.out.println(
          "Class: StructuredAnalysis MethodName: " + methodName + "    DEBUG: " + debug);
  }

  public void debug(String debug) {
    if (DEBUG) System.out.println("Class: StructuredAnalysis DEBUG: " + debug);
  }
}
