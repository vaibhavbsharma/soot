      ...
      while (it.hasNext()) {
        ...
        if (temp instanceof ASTLabeledBlockNode) {
          ...
          if (label.toString() == null) {
            ...
            UselessLabeledBlockRemover.removeLabeledBlock(
                node, labelBlock, subBodyNumber, nodeNumber);
          ...
          // check if the next node in the subBody is also an ASTIfNode in which case
          // invoke OrAggregatorThree
          if (it.hasNext()) { // means we can get the nodeNumber+1
            ...
            if (nextNode instanceof ASTIfNode) {
              ...
              OrAggregatorThree.checkAndTransform(
                  node, (ASTIfNode) temp, (ASTIfNode) nextNode, nodeNumber, subBodyNumber);
              if (G.v().ASTTransformations_modified) {
    ...
    while (it.hasNext()) {
      ...
      if (temp instanceof ASTLabeledBlockNode) {
        ...
        if (label.toString() == null) {
          ...
          List<Object> newBody =
              UselessLabeledBlockRemover.createNewSubBody(tryBody, nodeNumber, labelBlock);
          if (newBody != null) {
        ...
        if (elseBody.size() == 0) {
          ...
          List<Object> newBody =
              EmptyElseRemover.createNewNodeBody(tryBody, nodeNumber, (ASTIfElseNode) temp);
          if (newBody != null) {
            // something did not go wrong
            node.replaceTryBody(newBody);
            G.v().ASTTransformations_modified = true;
        ...
        // check if the next node in the subBody is also an ASTIfNode in which case invoke
        // OrAggregatorThree
        if (it.hasNext()) { // means we can get the nodeNumber+1
          ...
          if (nextNode instanceof ASTIfNode) {
            // found an If followed by another if might match Patter 3.
            List<Object> newBody =
                OrAggregatorThree.createNewNodeBody(
                    tryBody, nodeNumber, (ASTIfNode) temp, (ASTIfNode) nextNode);
            if (newBody != null) {
      ...
      while (itBody.hasNext()) {
        ...
        if (temp instanceof ASTLabeledBlockNode) {
          ...
          if (label.toString() == null) {
            ...
            List<Object> newBody =
                UselessLabeledBlockRemover.createNewSubBody(body, nodeNumber, labelBlock);
            if (newBody != null) {
              // something did not go wrong
          ...
          if (elseBody.size() == 0) {
            // System.out.println("Empty else body found"+temp);
            List<Object> newBody =
                EmptyElseRemover.createNewNodeBody(body, nodeNumber, (ASTIfElseNode) temp);
            if (newBody != null) {
              // something did not go wrong
              catchBody.replaceBody(newBody);
              G.v().ASTTransformations_modified = true;
          ...
          if (itBody.hasNext()) { // means we can get the nodeNumber+1
            ...
            if (nextNode instanceof ASTIfNode) {
              ...
              List<Object> newBody =
                  OrAggregatorThree.createNewNodeBody(
                      body, nodeNumber, (ASTIfNode) temp, (ASTIfNode) nextNode);
              if (newBody != null) {
                // something did not go wrong and pattern was matched
                catchBody.replaceBody(newBody);
                G.v().ASTTransformations_modified = true;
