 ...
 * error that used to show up when recompiling decompiled code Will be throughly covered in
 * "Programmer Friendly Code" Sable Tech Report (2006)
 */
        ...
        while (!must.isMustInitialized(field)) {
          ...
          while (it.hasNext()) {
            ...
            if (ancestorSubBody.indexOf(grandParent) > -1) {
              ...
              if (index + 1 < ancestorSubBody.size()
                  && ancestorSubBody.get(index + 1) instanceof ASTStatementSequenceNode) {
                ...
                // check if problem is solved else remove the assign and change
                // parents
