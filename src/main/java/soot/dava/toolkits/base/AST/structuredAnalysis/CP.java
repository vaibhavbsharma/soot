...
public class CP extends StructuredAnalysis {
  ...
  public DavaFlowSet processUnaryBinaryCondition(ASTUnaryBinaryCondition cond, DavaFlowSet input) {
    if (!(input instanceof CPFlowSet)) {
      throw new RuntimeException(
          "processCondition is not implemented for other flowSet types" + input.toString());
    }
  ...
  public DavaFlowSet processSynchronizedLocal(Local local, DavaFlowSet input) {
    if (!(input instanceof CPFlowSet)) {
      throw new RuntimeException(
          "processSynchronized  is not implemented for other flowSet types" + input.toString());
    }
  ...
  public DavaFlowSet processStatement(Stmt s, DavaFlowSet input) {
    ...
    if (!(left instanceof Local && ((Local) left).getType() instanceof PrimType)) return inSet;
  ...
  private void addOrUpdate(CPFlowSet toReturn, Local left, Object val) {
    ...
    // System.out.println("DefinitionStmt checked right expr for
    // constants"+toReturn.toString());
  }
  ...
  private void handleMathematical(CPFlowSet toReturn, Local left, Value right, Object killedValue) {
    ...
    if (right instanceof BinopExpr) {
      ...
      if (op1Val != null && op2Val != null) {
        // System.out.println("found constant values for both operands of binary
        // expression");
        if (left.getType() instanceof IntType
            && op1Val instanceof Integer
            && op2Val instanceof Integer) {
          ...
          if (tempStr.length() > 1) {
            ...
            // System.out.println("found symbol "+symbol+" for the operands of binary
            // expression");
            int newValue = 0;
            boolean set = false;
            switch (symbol) {
        ...
        // System.out.println("atleast one value is not constant so cant simplify
        // expression");
      }
    }
    // System.out.println("DefinitionStmt checked right expr for mathematical
    // stuff"+toReturn.toString());
  }
  ...
  public DavaFlowSet processASTIfElseNode(ASTIfElseNode node, DavaFlowSet input) {
    ...
    if (tuple != null) {
      // if not null, is a belief going into the if branch simply add it
      // into the input set
      // System.out.println(">>>>>Adding tuple because of condition into if
      // branch"+tuple.toString());
    ...
    if (tuple1 != null) {
      ...
      // into the input set
      // System.out.println(">>>>>Adding tuple because of condition  into else
      // branch"+tuple1.toString());
    ...
    if (DEBUG_IF) {
      ...
      System.out.println(
          "\n\n  IF-ELSE   INPUTS TO MERGE ARE input (if):"
              + output1.toString()
              + " else:"
              + output2.toString()
              + "\n\n\n");
