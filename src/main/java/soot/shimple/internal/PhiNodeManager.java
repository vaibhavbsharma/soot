...
public class PhiNodeManager {
   ...
   * Since it is useless trying to keep the number of arguments and control flow predecessors in
   * synch, we might as well trim out all redundant arguments and eliminate a huge number of copy
  ...
  /**
   * Eliminate Phi nodes in block by naively replacing them with shimple assignment statements in
   * the control flow predecessors. Returns true if new locals were added to the body during the
