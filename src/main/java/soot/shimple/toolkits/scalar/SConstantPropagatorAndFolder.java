 ...
 * of SSA -- particularly the fact that Phi nodes represent natural merge points in the control
 * flow.
 *
...
public class SConstantPropagatorAndFolder extends BodyTransformer {
  ...
  /**
   * Propagates constants to the definition and uses of the relevant locals given a mapping. Notice
   * that we use the Shimple implementation of LocalDefs and LocalUses.
   */
      ...
      // update definition
      {
        ...
        if (defSrcBox.canContainValue(constant)) {
          ...
          G.v()
              .out
              .println(
                  "Warning: Couldn't propagate constant "
                      + constant
                      + " to box "
                      + defSrcBox.getValue()
                      + " in unit "
...
class SCPFAnalysis extends ForwardBranchedFlowAnalysis {
  ...
  /**
   * If a node has an empty in set, it is considered unreachable. Otherwise the node is examined and
  ...
  /**
   * Verifies if the given assumption "constant" changes the previous assumption about "local" and
   * merges the information into the localToConstant map. Returns true if something changed.
   */
