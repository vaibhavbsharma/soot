...
public class Timers {
  ...
  public void printProfilingInformation() {
    ...
    //                G.v().out.println("                setup: " + toTimeString(defsSetupTimer,
    // totalTime));
    //                G.v().out.println("             analysis: " +
    // toTimeString(defsAnalysisTimer, totalTime));
    //                G.v().out.println("                 post: " + toTimeString(defsPostTimer,
    // totalTime));
    G.v().out.println("  Computing LocalUses: " + toTimeString(usesTimer, totalTime));
    //                G.v().out.println("            Use phase1: " +
    // toTimeString(usePhase1Timer, totalTime));
    //                G.v().out.println("            Use phase2: " +
    // toTimeString(usePhase2Timer, totalTime));
    //                G.v().out.println("            Use phase3: " +
    // toTimeString(usePhase3Timer, totalTime));
    ...
    //                G.v().out.println("                setup: " + toTimeString(liveSetupTimer,
    // totalTime));
    //                G.v().out.println("             analysis: " +
    // toTimeString(liveAnalysisTimer, totalTime));
    //                G.v().out.println("                 post: " + toTimeString(livePostTimer,
    // totalTime));
    ...
    // Print out time stats.
    {
      ...
      G.v()
          .out
          .println("                Aggregation: " + toTimeString(aggregationTimer, totalTime));
      G.v().out.println("            Coloring locals: " + toTimeString(packTimer, totalTime));
      G.v()
          .out
          .println("     Generating jasmin code: " + toTimeString(buildJasminTimer, totalTime));
      G.v()
          .out
          .println("          .jasmin -> .class: " + toTimeString(assembleJasminTimer, totalTime));

      //                    G.v().out.println("           Cleaning up code: " +
      // toTimeString(cleanup1Timer, totalTime) +
      //                        "\t" + cleanup1LocalCount + " locals  " + cleanup1StmtCount +
      // " stmts");

      //                    G.v().out.println("               Split phase1: " +
      // toTimeString(splitPhase1Timer, totalTime));
      //                    G.v().out.println("               Split phase2: " +
      // toTimeString(splitPhase2Timer, totalTime));
      ...
      */
      ...
      // float memoryUsed = (Runtime.getRuntime().totalMemory() -
      // Runtime.getRuntime().freeMemory()) / 1000.0f;
      ...
      G.v()
          ...
          .println(
              "stmtCount: "
                  + stmtCount
                  + "("
                  + toFormattedString(stmtCount / timeInSecs)
                  + " stmt/s)");
  ...
  private String toTimeString(Timer timer, long totalTime) {
    ...
    String timeString =
        format.format(time / 1000.0); // paddedLeftOf(new Double(truncatedOf(time / 1000.0,
    // 1)).toString(), 5);
