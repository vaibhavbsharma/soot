...
public class PackManager {
  ...
  public void runPacks() {
    if (Options.v().oaat()) runPacksForOneClassAtATime();
    else {
      runPacksNormally();
    }
  }
  ...
  private void runPacksForOneClassAtATime() {
    ...
    for (String path : Options.v().process_dir()) {
      ...
      for (String cl : SourceLocator.v().getClassesUnder(path)) {
        ...
        try {
          if (source == null) throw new RuntimeException("Could not locate class source");
    ...
    if (Options.v().interactive_mode()) {
      if (InteractionHandler.v().getInteractionListener() == null) {
        G.v()
            .out
            .println(
                "Cannot run in interactive mode. No listeners available. Continuing in regular mode.");
  ...
  public void coffiMetrics() {
    ...
    if (hashVem.size() > 0) aM /= hashVem.size();

    G.v()
        .out
        .println(
            "Vertices, Edges, Avg Degree, Highest Deg:    "
                + tV
                + "  "
                + tE
                + "  "
  ...
  public void writeOutput() {
    ...
    if (!Options.v().no_writeout_body_releasing()) releaseBodies(reachableClasses());
  ...
  private void preProcessDAVA() {
    if (Options.v().output_format() == Options.output_format_dava) {
      ...
      if (!isSourceJavac) {
         ...
         */
        if (DEBUG) System.out.println("Source is not Javac hence invoking ThrowFinder");

        ThrowFinder.v().find();
      } else {
        if (DEBUG) System.out.println("Source is javac hence we dont need to invoke ThrowFinder");
      }
  ...
  private void runBodyPacks(final Iterator<SootClass> classes) {
    ...
    CountingThreadPoolExecutor executor =
        new CountingThreadPoolExecutor(
            threadNum, threadNum, 30, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    ...
    if (executor.getException() != null) {
      ...
      else throw new RuntimeException(executor.getException());
    }
  }
  ...
  private void writeOutput(Iterator<SootClass> classes) {
    ...
    int threadNum =
        Options.v().output_format() == Options.output_format_class && jarFile == null
    ...
    try {
      ...
      // Something went horribly wrong
      throw new RuntimeException(
          "Could not wait for writer threads to " + "finish: " + e.getMessage(), e);
    }
    ...
    for (SootMethod m : methodsCopy) {
      if (DEBUG) {
        if (m.getExceptions().size() != 0)
          System.out.println(
              "PackManager printing out jimple body exceptions for method "
                  + m.toString()
                  + " "
      ...
      if (produceJimple) {
        ...
        if (Options.v().xml_attributes()
            && Options.v().output_format() != Options.output_format_jimple) {
  ...
  private void writeClass(SootClass c) {
    ...
    if (Options.v().output_format() == Options.output_format_jimple)
      if (!c.isPhantom) ConstantValueToInitializerTransformer.v().transformClass(c);
  ...
  /**
   * assumption: only called when <code>
   * Options.v().output_format() == Options.output_format_jimple</code>
   */
  ...
  private void releaseBodies(SootClass cl) {
    ...
    while (methodIt.hasNext()) {
      ...
      if (m.hasActiveBody()) m.releaseActiveBody();
    }
  }
  ...
  private void retrieveAllBodies() {
    ...
    try {
      ...
      // Something went horribly wrong
      throw new RuntimeException(
          "Could not wait for loader threads to " + "finish: " + e.getMessage(), e);
    }
