...
/**
 * Class implementing builder parameters (this decides what kinds of nodes should be built for each
 * kind of Soot value).
 *
...
public class MethodNodeFactory extends AbstractShimpleValueSwitch {
  ...
  public final void handleStmt(Stmt s) {
    ...
    if (s.containsInvokeExpr()) {
      if (!pag.getCGOpts().types_for_invoke()) return;

      InvokeExpr iexpr = s.getInvokeExpr();
      if (iexpr instanceof VirtualInvokeExpr) {
        if (!isReflectionNewInstance(iexpr)) return;
      } else if (!(iexpr instanceof StaticInvokeExpr)) return;
    }
    ...
    s.apply(
        new AbstractStmtSwitch() {
          ...
          public final void caseAssignStmt(AssignStmt as) {
            ...
            assert r.getType() instanceof RefLikeType
                : "Type mismatch in assignment " + as + " in method " + method.getSignature();
              ...
              if (pag.getOpts().empties_as_allocs()) {
                if (s.declaringClass().getName().equals("java.util.Collections")) {
                  if (s.name().equals("EMPTY_SET")) {
                    src =
                        pag.makeAllocNode(
                            RefType.v("java.util.HashSet"), RefType.v("java.util.HashSet"), method);
                  } else if (s.name().equals("EMPTY_MAP")) {
                    src =
                        pag.makeAllocNode(
                            RefType.v("java.util.HashMap"), RefType.v("java.util.HashMap"), method);
          ...
          public final void caseIdentityStmt(IdentityStmt is) {
            ...
            if (libOption != CGOptions.library_disabled
                && (accessibilityOracle.isAccessible(method))) {
  ...
  public final Node caseRet() {
    VarNode ret =
        pag.makeLocalVarNode(
            Parm.v(method, PointsToAnalysis.RETURN_NODE), method.getReturnType(), method);
  ...
  public final void caseInstanceFieldRef(InstanceFieldRef ifr) {
    if (pag.getOpts().field_based() || pag.getOpts().vta()) {
      ...
      setResult(
          pag.makeLocalFieldRefNode(
              ifr.getBase(), ifr.getBase().getType(), ifr.getField(), method));
    }
  }
  ...
  public final void caseStringConstant(StringConstant sc) {
    ...
    VarNode stringConstantLocal =
        pag.makeGlobalVarNode(stringConstant, RefType.v("java.lang.String"));
  ...
  public void caseVirtualInvokeExpr(VirtualInvokeExpr v) {
    if (isReflectionNewInstance(v)) {
      NewInstanceNode newInstanceNode =
          pag.makeNewInstanceNode(v, Scene.v().getObjectType(), method);
    ...
    } else throw new RuntimeException("Unhandled case of VirtualInvokeExpr");
  }
