...
public class SparkTransformer extends SceneTransformer {
  ...
  protected void internalTransform(String phaseName, Map<String, String> options) {
    ...
    if (opts.cs_demand()) {
      ...
      PointsToAnalysis onDemandAnalysis =
          DemandCSPointsTo.makeWithBudget(opts.traversal(), opts.passes(), opts.lazy_pts());
      Date endOndemand = new Date();
      reportTime(
          "Initialized on-demand refinement-based context-sensitive analysis",
          startOnDemand,
          endOndemand);
  ...
  protected void findSetMass(PAG pag) {
    ...
    for (int i = 0; i < deRefCounts.length; i++) {
      if (deRefCounts[i] > 0) {
        G.v()
            .out
            .println("" + i + " " + deRefCounts[i] + " " + (deRefCounts[i] * 100.0 / total) + "%");
      }
    }
  }
}
