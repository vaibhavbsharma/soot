...
public class SharedHybridSet extends PointsToSetInternal {
  ...
  private void findAppropriateBitVector(
      PointsToBitVector newBitVector,
      PointsToBitVector otherBitVector,
      int otherSize,
      int szBitvector) {
    ...
    if (otherBitVector != null
      ...
      for (int overFlowSize = 0; overFlowSize < OVERFLOW_THRESHOLD; ++overFlowSize) {
        ...
        if (bitVectorCardinality < AllSharedHybridNodes.v().lookupMap.map.length
          ...
          List<PointsToBitVector> lst =
              AllSharedHybridNodes.v().lookupMap.map[bitVectorCardinality];
  ...
  private boolean nativeAddAll(SharedHybridSet other, SharedHybridSet exclude) {
    ...
    if (bitVector == null) {
      ...
      if (bitVector != null) { // Maybe both bitvectors were null; in
        ...
        if (exclude != null || mask != null) {
          ...
          if (!result.equals(bitVector)) {
            ...
            findAppropriateBitVector(
                result, other.bitVector, otherBitVectorSize, otherBitVectorSize);
      ...
      // actually added
      {
        ...
        numElements +=
            numOnes
                - originalOnes // number of new bits
                + numAdded
                - overflow.size(); // might be negative due to
        ...
        if (size() > originalSize) {
          ...
          // checkSize();
          return false; // It might happen that the bitvector being merged in adds some
          // bits
          // to the existing bitvector, but that those new bits are all elements that were
          // already
          // in the overflow list.  In that case, the set might not change, and if not we
          // return false.
          // We also leave the set the way it was by not calling findAppropriateBitvector,
          // which maximizes sharing and is fastest in the short term.  I'm not sure
          // whether it
