 ...
 * stores all the all-to-one figures, and so on.
 *
 * @author xiao
 */
public class HeapInsIntervalManager extends IFigureManager {
  ...
  public SegmentNode addNewFigure(int code, RectangleNode pnew) {
    ...
    if (code == ALL_TO_ALL) {
      ...
      if (header[ALL_TO_MANY] != null && header[ALL_TO_MANY].I2 == 0) return null;
      ...
      if (code == ONE_TO_ONE) {
        p = header[ONE_TO_ONE];
        while (p != null) {
          // We don't process the case: the input figure is a square but the tested figure
          // is a segment
