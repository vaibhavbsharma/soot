...
public class OfflineProcessor {
  ...
  /**
   * The dependence graph reverses the assignment relations. E.g., p = q => p -> q Note that, the
   * assignments that are eliminated by local variable merging should be used here. Otherwise, the
   * graph would be erroneously disconnected.
   */
  ...
  /**
   * Compute a set of pointers that required to refine the seed pointers. Prerequisite: dependence
   * graph
   */
  ...
  protected void distillConstraints() {
    ...
    for (PlainConstraint cons : geomPTA.constraints) {
      ...
      switch (cons.type) {
        ...
        case Constants.STORE_CONS:
          /**
           * Interesting point in store constraint p.f = q: For example, pts(p) = { o1, o2 }; If any
           * of the o1.f and the o2.f (e.g. o1.f) will be updated, this constraint should be kept.
  ...
  /**
   * The dependence graph will be destroyed and the impact graph will be built. p = q means q
   * impacts p. Therefore, we add en edge q -> p in impact graph.
   */
