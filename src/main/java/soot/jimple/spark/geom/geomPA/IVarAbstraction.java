 ...
 * manipulating a pointer/object.
 *
 * @author xiao
 */
public abstract class IVarAbstraction implements Numberable {
  // A shape manager that has only one all map to all member, representing the context insensitive
  // points-to info
  ...
  /**
   * This pointer/object is reachable if its enclosing method is reachable. Pleas always call this
   * method to check the status before querying points-to information.
   */
  ...
  /**
   * Test if this pointer currently has points-to result. The result can be computed in the last
   * iteration of geomPTA, although its willUpdate = false this round.
   */
  ...
  public abstract boolean add_simple_constraint_4(
      IVarAbstraction qv, long I1, long I2, long L1, long L2);
