...
public class GeomPointsTo extends PAG {
  ...
  // Because we override the points-to query interface for SPARK, we need this flag to know how to
  // answer queries
  ...
  /**
   * Data structures that only specific to geometric solver are created here. The initialized
   * container sizes are empirically chosen from the primes. We believe most of the machine today
   * can afford the memory overhead.
   */
  ...
  public void parametrize(double spark_run_time) {
    ...
    if (nodeGenerator == null)
      throw new RuntimeException(
          "The encoding " + encoding_name + " is unavailable for geometric points-to analysis.");
  ...
  /**
   * Read in the program facts generated by SPARK. We also construct our own call graph and pointer
   * variables.
   */
  private void preprocess() {
    ...
    while (edgeList.hasNext()) {
      ...
      if (edge.isThreadRunCall() || edge.kind().isExecutor() || edge.kind().isAsyncTask()) {
        ...
        // We try to refine the virtual callsites (virtual + interface) with multiple call
        // targets
  ...
  private void callGraphDFS(int s) {
    ...
    while (p != null) {
      t = p.t;
      if (vis_cg[t] == 0) callGraphDFS(t);
      if (low_cg[t] < low_cg[s]) low_cg[s] = low_cg[t];
      p = p.next;
    }
  ...
  /**
   * Build a call graph, merge the SCCs and name the contexts. Also permit clients to decide whether
   * to connect the disjoint parts in the call graph or not.
   */
  private void encodeContexts(boolean connectMissedEntries) {
    ...
    if (connectMissedEntries) {
      // The functions other than SUPER_MAIN that have zero in-degrees are missed entry
      // methods
    ...
    // The implementation is slightly different from our paper (the non-SCC edges are not moved,
    // they still use their current context mappings)
    if (getOpts().geom_blocking()) {
      ...
      for (i = 0; i < n_func; ++i) {
        ...
        while (p != null) {
          j = p.t;
          if (j != i // This is not a self-loop, and a self-loop is treated
              // specially in the initial encoding phase
              && p.scc_edge == true) {
            ...
            if (context_size[j] <= Constants.MAX_CONTEXTS - max_context_size_block[i]) {
              ...
              // We randomly pick a block for reuse (try best to avoid reusing the
              // first block)
              int iBlock = 0;
    ...
    // Print debug info
    ps.printf(
        "Reachable Methods = %d, in which #Condensed Nodes = %d, #Full Context Nodes = %d \n",
  ...
  /**
   * Remove unreachable call targets at the virtual callsites using the up-to-date points-to
   * information.
   */
  private int updateCallGraph() {
    ...
    ps.printf(
        "%d of %d virtual call edges are proved to be spurious.\n", n_obsoleted, all_virtual_edges);
    return n_obsoleted;
  }
  ...
  /**
   * 1. Update the call graph; 2. Eliminate the pointers, objects, and constraints related to the
   * unreachable code.
   */
  ...
  /**
   * For many applications, they only need the context insensitive points-to result. We provide a
   * way to transfer our result back to SPARK. After the transformation, we discard the context
   * sensitive points-to information. Therefore, if context sensitive queries are needed in future,
   * please call ddSolve() for queried pointers first.
   */
  ...
  /**
   * The starting point of the geometric points-to analysis engine. This function computes the whole
  ...
  public void solve() {
    ...
    for (rounds = 0, n_obs = 1000; rounds < Parameters.cg_refine_times && n_obs > 0; ++rounds) {
      ...
      // Offline processing:
      // substantially use the points-to result for redundancy elimination prior to the
      // analysis
      ...
      // We update the call graph and other internal data when the new points-to information
      // is ready
    ...
    // We perform a set of tests to assess the quality of the points-to results for user
    // pointers
  ...
  /**
   * The demand-driven mode for precisely computing points-to information for given pointers. Call
   * graph will not be updated in this mode.
   *
  ...
  /**
   * We thoroughly delete the geometric points-to result for space saving. Some applications such as
   * those needing the call graph only may want to clean the points-to result.
   */
  ...
  /**
   * Find our representation for the SPARK node @param v. We don't create a new node if nothing
   * found.
   */
  ...
  public PointsToSet reachingObjects(Local l) {
    ...
    if (hasTransformed || vn.getP2Set() != EmptyPointsToSet.v()) return vn.getP2Set();
  ...
  public PointsToSet reachingObjects(Context c, Local l) {
    ...
    if (hasTransformed || !(c instanceof Unit)) return reachingObjects(l);
    ...
    if (myEdge == null) return vn.getP2Set();
    ...
    for (AllocNode an : pn.get_all_points_to_objects()) {
      if (pn.pointer_interval_points_to(low, high, an)) ptset.add(an);
    }
  ...
  public PointsToSet reachingObjects(SootField f) {
    ...
    if (!f.isStatic()) throw new RuntimeException("The parameter f must be a *static* field.");
  ...
  public PointsToSet reachingObjects(AllocNode an, SootField f) {
    ...
    if (adf == null) return EmptyPointsToSet.v();

    // Not seen by geomPTA
    if (pn == null) return adf.getP2Set();

    if (hasTransformed || adf.getP2Set() != EmptyPointsToSet.v()) return adf.getP2Set();
