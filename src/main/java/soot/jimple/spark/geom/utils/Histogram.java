...
public class Histogram {
  ...
  /**
   * This function prints two histograms together for comparative reading. It requires the two
   * histograms having the same data separators.
   *
   * @param output
   * @param title
   * @param other
  ...
  public void printResult(PrintStream output, String title, Histogram other) {
    ...
    for (int i = 0; i < results.length; i++) {
      if (i == 0) {
        ...
        output.printf(
            "%d < x <= %d: %d (%d)", limits[i - 1], limits[i], results[i], other.results[i]);
      }
