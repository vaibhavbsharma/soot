...
public class GeomEvaluator {
  ...
  public void profileGeomBasicMetrics(boolean testSpark) {
    ...
    outputer.println(
        "#Core Pointers: " + n_legal_var + ", in which #AllocDot Fields: " + n_alloc_dot_fields);
    outputer.printf(
        "Total/Average Projected Points-to Tuples [core pointers]: %d (%d) / %.3f (%.3f) \n",
        evalRes.total_geom_ins_pts,
        evalRes.total_spark_pts,
        evalRes.avg_geom_ins_pts,
        evalRes.avg_spark_pts);
    outputer.println(
        "The largest points-to set size [core pointers]: "
            + evalRes.max_pts_geom
            + " ("
            + evalRes.max_pts_spark
            + ")");

    outputer.println();
    evalRes.pts_size_bar_geom.printResult(
  ...
  private void test_1cfa_call_graph(
    ...
    for (Iterator<CgEdge> it = list.iterator(); it.hasNext(); ) {
      ...
      for (AllocNode obj : set) {
        ...
        try {
          tgts.add(
              hierarchy.resolveConcreteDispatch(((RefType) t).getSootClass(), callee_signature));
        } catch (Exception e) {
  ...
  public void checkCallGraph() {
    ...
    ptsProvider.ps.printf(
        "Total virtual callsites (app code): %d (%d)\n",
    ...
    ptsProvider.ps.printf(
        "Virtual callsites additionally solved by geomPTA compared to SPARK (app code) = %d (%d)\n",
  ...
  public void checkAliasAnalysis() {
    ...
    ptsProvider.ps.printf(
        "Heap sensitive alias pairs (by Geom): %d, Percentage = %.3f%%\n",
        evalRes.n_hs_alias, (double) evalRes.n_hs_alias / evalRes.n_alias_pairs * 100);
    ptsProvider.ps.printf(
        "Heap insensitive alias pairs (by SPARK): %d, Percentage = %.3f%%\n",
  ...
  /**
   * Estimate the size of the def-use graph for the heap memory. The heap graph is estimated without
   * context information.
   */
  public void estimateHeapDefuseGraph() {
                  ...
                  new P2SetVisitor() {
                    ...
                    public void visit(Node n) {
                      IVarAbstraction padf =
                          ptsProvider.findAndInsertInstanceField((AllocNode) n, field);
    ...
    ptsProvider.ps.println(
        "The edges in the heap def-use graph is (by Geom): " + evalRes.n_geom_du_pairs);
    ptsProvider.ps.println(
        "The edges in the heap def-use graph is (by Spark): " + evalRes.n_spark_du_pairs);
    ptsProvider.ps.printf("Using time: %dms \n", end.getTime() - begin.getTime());
    ptsProvider.ps.println();
  }
}
