...
/**
 * This {@link TypeSwitch} can be used to add library behavior to the PAG. It adds allocation nodes
 * with {@link AnySubType} of the declared type to the target node.
 *
...
public class SparkLibraryHelper extends TypeSwitch {
  ...
  /**
   * A new local will be created and connected to {@link SparkLibraryHelper#node} of type {@link
   * RefType}. For this new local an allocation edge to {@link AnySubType} of its declared type will
   * be added.
   */
  @Override
  ...
  /**
   * A new local array will be created and connected to {@link SparkLibraryHelper#node} of type
   * {@link ArrayType}. For this new local an allocation edge to a new array of its declared type
   * will be added. If the {@link ArrayType#getElementType()} is still an array an allocation to a
   * new array of this element type will be made and stored until the element type is a {@link
   * RefType}. If this is the case an allocation to {@link AnySubType} of {@link ArrayType#baseType}
   * will be made.
   */
  @Override
    ...
    for (Type t = type; t instanceof ArrayType; t = ((ArrayType) t).getElementType()) {
      ...
      if (at.baseType instanceof RefType) {
        ...
        if (at.numDimensions == 1) {
          // new T()
          AllocNode alloc =
              pag.makeAllocNode(new Object(), AnySubType.v((RefType) at.baseType), method);
