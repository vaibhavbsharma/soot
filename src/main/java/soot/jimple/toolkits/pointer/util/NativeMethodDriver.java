...
/**
 * A wrapper for native method side-effect simulation. The caller passes in a native method with
 * parameters, the corresponding native simulator gets called.
 *
 * @author Feng Qian
 */
...
public class NativeMethodDriver {
  public NativeMethodDriver(NativeHelper helper) {
    ...
    cnameToSim.put(
        "java.lang.ClassLoader$NativeLibrary", new JavaLangClassLoaderNativeLibraryNative(helper));
    ...
    cnameToSim.put(
        "java.security.AccessController", new JavaSecurityAccessControllerNative(helper));
