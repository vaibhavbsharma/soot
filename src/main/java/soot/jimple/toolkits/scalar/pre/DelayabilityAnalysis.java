 ...
 * PRE (lazy code motion) and has little (no?) sense if used alone. Basically it tries to push the
 * computations we would insert in the Busy Code Motion as far down as possible, to decrease
 * life-time ranges (clearly this is not true, if the computation "uses" two variables and produces
 * only one temporary).
 */
public class DelayabilityAnalysis extends ForwardFlowAnalysis<Unit, FlowSet<EquivalentValue>> {
  ...
  /**
   * Automatically performs the Delayability-analysis on the graph <code>dg</code> and the
