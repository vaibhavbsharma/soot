 ...
 * anymore. This uses the Delayability-analysis. More precise: The delayability-analysis says us
 * already until which point we can delay a computation from the earliest computation-point. We just
 * have to search for points, where there's a computation, or, where we can't delay the computation
...
public class LatestComputation {
  ...
  public LatestComputation(
      UnitGraph unitGraph, DelayabilityAnalysis delayed, Map<Unit, EquivalentValue> equivRhsMap) {
    this(
        unitGraph,
        delayed,
        equivRhsMap,
