 ...
 * optimal), and then replacing the original computation by a reference to this new high
 * computation. This implies, that we introduce <b>many</b> new helper-variables (that can easily be
 * eliminated afterwards).<br>
 * In order to catch every redundant expression, this transformation must be done on a graph without
 * critical edges. Therefore the first thing we do, is removing them. A subsequent pass can then
 * easily remove the synthetic nodes we have introduced.<br>
 * The term "busy" refers to the fact, that we <b>always</b> move computations as high as possible.
...
public class BusyCodeMotion extends BodyTransformer {
  ...
  protected void internalTransform(Body b, String phaseName, Map<String, String> opts) {
    ...
    DownSafetyAnalysis downSafe =
        new DownSafetyAnalysis(graph, unitToNoExceptionEquivRhs, sideEffect);
    EarliestnessComputation earliest =
        new EarliestnessComputation(graph, upSafe, downSafe, sideEffect);
