 ...
 * Isolated-analysis (we just return the complement, as it's easier to calculate it). A computation
 * is isolated, if it can only be used at the current computation-point. In other words: if the
 * result of the computation will not be used later on the computation is isolated.<br>
 * The Latest-analysis helps us in finding isolated computations, as they show us points, where a
 * precedent computation can't be used anymore. In completely other words: we search the interval
 * "latest"-"computation". a computation in this interval would not be isolated.
 */
public class NotIsolatedAnalysis extends BackwardFlowAnalysis<Unit, FlowSet<EquivalentValue>> {
  ...
  /**
   * Automatically performs the Isolation-analysis on the graph <code>dg</code> using the
  ...
  public NotIsolatedAnalysis(
      DirectedGraph<Unit> dg, LatestComputation latest, Map<Unit, EquivalentValue> equivRhsMap) {
    this(
        dg,
        latest,
        equivRhsMap,
  ...
  protected void merge(
      FlowSet<EquivalentValue> inSet1,
