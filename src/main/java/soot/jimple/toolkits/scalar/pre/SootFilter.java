...
public class SootFilter {
  ...
  /**
   * only lets concrete references through. A concrete reference is either an array-ref or a
   * field-ref.<br>
  ...
  public static boolean isInvoke(Value val) {
    ...
    if (val instanceof InvokeExpr) return true;
    return false;
  }
  ...
  /**
   * returns the Value of an EquivalentValue. If there are several EquivalentValues stacked one into
   * another, gets the deepest Value.<br>
   * returns <code>null</code> if <code>val</code> is null.
   *
   * @param val the Value to inspect.
