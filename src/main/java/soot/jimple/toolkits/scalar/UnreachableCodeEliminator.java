...
public class UnreachableCodeEliminator extends BodyTransformer {
  ...
  protected void internalTransform(Body body, String phaseName, Map<String, String> options) {
    ...
    if (this.throwAnalysis == null)
      this.throwAnalysis =
          PhaseOptions.getBoolean(options, "remove-unreachable-traps", true)
    ...
    Set<Unit> reachable =
        units.isEmpty() ? Collections.<Unit>emptySet() : reachable(units.getFirst(), graph);
    ...
    for (Iterator<Trap> it = body.getTraps().iterator(); it.hasNext(); ) {
      ...
      if ((trap.getBeginUnit() == trap.getEndUnit())
          || !reachable.contains(trap.getHandlerUnit())) {
        it.remove();
      }
    }
    ...
    for (Trap t : body.getTraps())
      if (t.getEndUnit() == body.getUnits().getLast()) reachable.add(t.getEndUnit());

    Set<Unit> notReachable = new HashSet<Unit>();
    if (Options.v().verbose()) {
      for (Unit u : units) {
        if (!reachable.contains(u)) notReachable.add(u);
      }
    }
    ...
    if (Options.v().verbose()) {
      G.v()
          .out
          .println("[" + body.getMethod().getName() + "]	 Removed " + numPruned + " statements: ");
