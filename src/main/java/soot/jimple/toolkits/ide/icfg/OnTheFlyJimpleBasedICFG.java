 ...
 * calls through Class Hierarchy Analysis (CHA), as implemented through FastHierarchy. The CHA is
 ...
 * intact. Clients should call {@link #loadAllClassesOnClassPathToSignatures()} to load all required
...
public class OnTheFlyJimpleBasedICFG extends AbstractJimpleBasedICFG {
  ...
  protected final LoadingCache<Body, LocalMustNotAliasAnalysis> bodyToLMNAA =
      IDESolver.DEFAULT_CACHE_BUILDER.build(
          new CacheLoader<Body, LocalMustNotAliasAnalysis>() {
            @Override
  ...
  protected final LoadingCache<Unit, Set<SootMethod>> unitToCallees =
      IDESolver.DEFAULT_CACHE_BUILDER.build(
          new CacheLoader<Unit, Set<SootMethod>>() {
            @Override
              ...
              if (ie instanceof InstanceInvokeExpr) {
                if (ie instanceof SpecialInvokeExpr) {
                  ...
                  RefType concreteType =
                      bodyToLMNAA.getUnchecked(unitToOwner.get(u)).concreteType(base, stmt);
                  if (concreteType != null) {
                    // the base variable definitely points to a single concrete
                    // type
                    SootMethod singleTargetMethod =
                        fastHierarchy.resolveConcreteDispatch(
                            concreteType.getSootClass(), iie.getMethod());
                    ...
                    if (base.getType() instanceof RefType) {
                      ...
                      // if the base is definitely null then there is no call
                      // target
