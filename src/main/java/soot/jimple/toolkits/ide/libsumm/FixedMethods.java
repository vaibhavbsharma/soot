...
/**
 * Determines whether, according to the type hierarchy, a method call is fixed, i.e., cannot be
 * overwritten by client code.
 */
...
public class FixedMethods {
   ...
   * library code, then client code cannot possible overwrite the called method. This is trivially
   * true for InvokeStatic and InvokeSpecial, but can also hold for virtual invokes if all possible
  ...
  public static boolean isFixed(InvokeExpr ie) {
    return ie instanceof StaticInvokeExpr
        || ie instanceof SpecialInvokeExpr
        || !clientOverwriteableOverwrites(ie.getMethod());
  }

  /**
   * Returns true if this method itself is visible to the client and overwriteable or if the same
   * holds for any of the methods in the library that overwrite the argument method.
   *
  ...
  /**
   * Returns true if the given method itself is visible to the client and overwriteable. This is
   * true if neither the method nor its declaring class are final, if the method is visible and if
   * the declaring class can be instantiated.
   *
  ...
  /**
   * Returns true if clients can instantiate the given class. This holds if the given class is
   * actually an interface, or if it contains a visible constructor. If the class is an inner class,
   * then the enclosing classes must be instantiable as well.
   */
