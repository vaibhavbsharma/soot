...
public class UseFinder {
  ...
  Map<SootClass, List>
      classToExtFieldAccesses; // each field access is a Pair <containing method, stmt>
  ...
  // This is an incredibly stupid way to do this... we should just use the call graph for
  // faster/better info!
        ...
        if (method.isConcrete() && rm.contains(method)) {
          ...
          while (unitsIt.hasNext()) {
            ...
            if (s.containsInvokeExpr()) {
              ...
              if (ie.getMethodRef().resolve().getDeclaringClass()
                  == appClass) // what about sub/superclasses
              {
