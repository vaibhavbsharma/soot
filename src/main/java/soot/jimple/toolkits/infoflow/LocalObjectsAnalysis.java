...
public class LocalObjectsAnalysis {
  ...
  public boolean isObjectLocalToContext(Value localOrRef, SootMethod sm, SootMethod context) {
    ...
    if (!sm.isConcrete()) {
      ...
      throw new RuntimeException(
          "Attempted to check if a local variable in a non-concrete method is shared/local.");
    }
    ...
    if (localOrRef instanceof InstanceFieldRef) {
      ...
      if (ifr.getBase() == thisLocal) {
        boolean isLocal =
            mergedContext.isFieldLocal(InfoFlowAnalysis.getNodeForFieldRef(sm, ifr.getField()));
        if (dfa.printDebug()) {
          if (isLocal) {
            G.v()
                .out
                .println(
                    "      LOCAL  (this  .localField  from "
                        + context.getDeclaringClass().getShortName()
                        + "."
                        + context.getName()
                        + ")");
        ...
        boolean isLocal =
            SmartMethodLocalObjectsAnalysis.isObjectLocal(dfa, sm, mergedContext, ifr.getBase());
        if (isLocal) {
          ClassLocalObjectsAnalysis cloa =
              getClassLocalObjectsAnalysis(context.getDeclaringClass());
    ...
    boolean isLocal =
        SmartMethodLocalObjectsAnalysis.isObjectLocal(dfa, sm, mergedContext, localOrRef);
    if (dfa.printDebug()) {
      if (isLocal) {
        G.v()
            .out
  ...
  */
  ...
  public CallChain getNextCallChainBetween(
    ...
    if (start == end) {
      //			if(previouslyFound.contains(path)) // don't return a call chain that was already
      // returned in a previous run
      //			{
      //				G.v().out.print("P");
      //				return null;
      //			}
  ...
  */
  ...
  public boolean hasNonLocalEffects(
      SootMethod containingMethod, InvokeExpr ie, SootMethod context) {
    SootMethod target = ie.getMethodRef().resolve();
    MutableDirectedGraph dataFlowGraph =
        dfa.getMethodInfoFlowSummary(
            target); // TODO actually we want a graph that is sensitive to scalar data,
    // too
    ...
    if (ie instanceof StaticInvokeExpr) {
      ...
      // For a instance invoke on local object, check if any static fields or any shared
      // params are read/written
      ...
      // For a instance invoke on shared object, check if any fields or any shared params are
      // read/written
      else {
        Iterator graphIt = dataFlowGraph.iterator();
        while (graphIt.hasNext()) {
