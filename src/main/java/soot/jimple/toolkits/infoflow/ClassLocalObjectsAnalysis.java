...
public class ClassLocalObjectsAnalysis {
  ...
  // methods that are called at least once from outside of this class (ie need to be public,
  ...
  // methods that should be used as starting points when determining if a value in a method called
  // from this class is local or shared
  // for thread-local objects, this would contain just the run method.  For structure-local, it
  // should contain all external methods
  ...
  public ClassLocalObjectsAnalysis(
      LocalObjectsAnalysis loa, InfoFlowAnalysis dfa, UseFinder uf, SootClass sootClass) {
  ...
  public ClassLocalObjectsAnalysis(
    ...
    this.methodToMethodLocalObjectsAnalysis =
        new HashMap<SootMethod, SmartMethodLocalObjectsAnalysis>();
    ...
    if (true) {
      ...
      G.v()
          .out
          .println(
              "[local-objects]   (#analyzed/#encountered): "
  ...
  public static List<SootMethod> getAllReachableMethods(SootClass sc) {
    ...
    while (methodsIt.hasNext()) {
      ...
      if (rm.contains(method)) allMethods.add(method);
    }
    ...
    while (superclass.hasSuperclass()) // we don't want to process Object
    {
      ...
      while (scMethodsIt.hasNext()) {
        ...
        if (rm.contains(scMethod)) allMethods.add(scMethod);
      }
      superclass = superclass.getSuperclass();
    }
    return allMethods;
  ...
  private void doAnalysis() {
    ...
    while (methodsIt.hasNext()) {
      ...
      if (primitiveDfa != null) {
        ...
        if (printdfgs && method.getDeclaringClass().isApplicationClass()) {
          G.v()
              .out
              .println("Attempting to print graphs (will succeed only if ./dfg/ is a valid path)");
          DirectedGraph primitiveGraph =
              primitiveDfa.getMethodInfoFlowAnalysis(method).getMethodAbbreviatedInfoFlowGraph();
          InfoFlowAnalysis.printGraphToDotFile(
              "dfg/"
                  ...
                  + "_primitive",
              primitiveGraph,
              method.getName() + "_primitive",
              false);

          DirectedGraph nonPrimitiveGraph =
              dfa.getMethodInfoFlowAnalysis(method).getMethodAbbreviatedInfoFlowGraph();
          InfoFlowAnalysis.printGraphToDotFile(
              "dfg/" + method.getDeclaringClass().getShortName() + "_" + method.getName(),
              nonPrimitiveGraph,
              method.getName(),
              false);
        }
        ...
        if (printdfgs && method.getDeclaringClass().isApplicationClass()) {
          G.v()
              .out
              .println("Attempting to print graph (will succeed only if ./dfg/ is a valid path)");
          DirectedGraph nonPrimitiveGraph =
              dfa.getMethodInfoFlowAnalysis(method).getMethodAbbreviatedInfoFlowGraph();
          InfoFlowAnalysis.printGraphToDotFile(
              "dfg/" + method.getDeclaringClass().getShortName() + "_" + method.getName(),
      ...
      while (nodesIt.hasNext()) {
        ...
        if (node.getValue() instanceof InstanceFieldRef) {
          ...
          if (!localFields.contains(ifr.getField())
              && !sharedFields.contains(ifr.getField())
    ...
    while (changed) {
      ...
      while (methodsIt.hasNext()) {
        SootMethod method = methodsIt.next();
        // we can't learn anything from non-concrete methods, and statics can't write
        // non-static fields
        ...
        while (localFieldsIt.hasNext()) {
          ...
          while (sourcesAndSinksIt.hasNext()) {
            ...
            } else if (sourceOrSinkRef instanceof ThisRef) // or return ref
            {
              ...
              fieldBecomesShared =
                  sharedFields.contains(((FieldRef) sourceOrSinkRef).getField())
            ...
            if (fieldBecomesShared) {
              ...
              //								G.v().out.println("            Source/Sink: " +
              // sourceOrSinkRef.toString() + " is SHARED");
              localFieldsIt.remove();
              sharedFields.add(localField);
              changed = true;
              break; // other sources don't matter now... it only takes one to taint
              // the field
            } else {
              //							if(localField.getDeclaringClass().isApplicationClass())
              //								G.v().out.println("            Source: " +
              // sourceRef.toString() + " is local");
            }
          }
        }
  ...
  private void propagate() {
    ...
    methodToContext =
        new HashMap<
            SootMethod, CallLocalityContext>(); // TODO: add the ability to share a map with another
    // CLOA to save memory (be careful of
      ...
      for (SootMethod containingMethod : worklist) {
        ...
        if (dfa.printDebug())
          G.v()
              .out
              .println(
                  "      " + containingMethod.getName() + " " + containingContext.toShortString());
        ...
        for (Iterator edgesIt = Scene.v().getCallGraph().edgesOutOf(containingMethod);
          ...
          if (!methodToContext.containsKey(e.tgt())) {
            ...
            boolean causedChange =
                methodToContext
                    .get(e.tgt())
                    .merge(invokeContext); // The contexts being merged could
            // be from different DFAs.  If so,
            // primitive version might be
            // bigger.
  ...
  public CallLocalityContext getMergedContext(SootMethod method) {
    if (methodToContext.containsKey(method)) return methodToContext.get(method);

    return null;
  }

  private CallLocalityContext getContextFor(
      Edge e, SootMethod containingMethod, CallLocalityContext containingContext) {
    ...
    CallLocalityContext callingContext =
        new CallLocalityContext(
            dfa.getMethodInfoFlowSummary(callingMethod)
                .getNodes()); // just keeps a map from NODE to SHARED/LOCAL
    ...
    if (callingMethod.isConcrete()) {
            ...
            if (r instanceof InstanceFieldRef) {
              EquivalentValue newRefEqVal =
                  InfoFlowAnalysis.getNodeForFieldRef(
                      callingMethod, ((FieldRef) r).getFieldRef().resolve());
              if (callingContext.containsField(
                  newRefEqVal)) // if not, then we're probably calling a parent
                // class's method, so some fields are missing
                callingContext.setFieldLocal(newRefEqVal); // must make a new eqval for the method
              // getting called
            } else if (r instanceof ThisRef) callingContext.setThisLocal();
          }
        } else if (SmartMethodLocalObjectsAnalysis.isObjectLocal(
            dfa, containingMethod, containingContext, iie.getBase())) {
      ...
      if (ie == null) callingContext.setAllParamsShared();
        ...
        for (int param = 0; param < ie.getArgCount(); param++) {
          if (SmartMethodLocalObjectsAnalysis.isObjectLocal(
              dfa, containingMethod, containingContext, ie.getArg(param)))
            callingContext.setParamLocal(param);
          else callingContext.setParamShared(param);
        }
      }
    } else {
      // The only conservative solution for a bodyless method is to assume everything is
      // shared
      callingContext.setAllFieldsShared();
      callingContext.setThisShared();
  ...
  private CallLocalityContext getContextFor(
      SootMethod sm, boolean includePrimitiveDataFlowIfAvailable) {
  ...
  private boolean isObjectLocal(
      Value localOrRef, SootMethod sm, boolean includePrimitiveDataFlowIfAvailable) {
    ...
    SmartMethodLocalObjectsAnalysis smloa =
        getMethodLocalObjectsAnalysis(sm, includePrimitiveDataFlowIfAvailable);
  ...
  private SmartMethodLocalObjectsAnalysis getMethodLocalObjectsAnalysis(
      SootMethod sm, boolean includePrimitiveDataFlowIfAvailable) {
  ...
  protected boolean parameterIsLocal(
    ...
    if (!(param.getType() instanceof RefLikeType)
        && (!dfa.includesPrimitiveInfoFlow() || method.getName().equals("<init>"))) // TODO fix
    {
      if (dfa.printDebug() && method.getDeclaringClass().isApplicationClass())
    ...
    while (intClassCallsIt.hasNext()) {
      ...
      if (ie.getMethodRef().resolve() == method) {
        if (((ParameterRef) parameterRef.getValue()).getIndex() >= 0) {
          if (!isObjectLocal(
              ie.getArg(((ParameterRef) parameterRef.getValue()).getIndex()),
              containingMethod,
              includePrimitiveDataFlowIfAvailable)) // WORST CASE SCENARIO HERE IS
          // INFINITE RECURSION!
          {
            if (dfa.printDebug() && method.getDeclaringClass().isApplicationClass())
              G.v().out.println("          PARAM is shared (internal propagation)");
            return false; // if arg is shared for any internal call, then param is
            // shared
          }
        } else {
