...
public class SimpleMethodLocalObjectsAnalysis extends SimpleMethodInfoFlowAnalysis {
  ...
  public SimpleMethodLocalObjectsAnalysis(
      UnitGraph g, ClassLocalObjectsAnalysis cloa, InfoFlowAnalysis dfa) {
  ...
  public SimpleMethodLocalObjectsAnalysis(
      UnitGraph g, CallLocalityContext context, InfoFlowAnalysis dfa) {
    super(g, dfa, true, true); // special version doesn't run analysis yet
    ...
    Iterator<Object> sharedRefEqValIt =
        sharedRefs.iterator(); // returns a list of (correctly structured) EquivalentValue
    // wrapped refs that should be treated as shared
        ...
        G.v()
            ...
            .println(
                "      Requested value "
                    + local
                    + " is "
                    + (!sinks.contains(new CachedEquivalentValue(local)) ? "Local" : "Shared")
                    + " in "
                    + sm
                    + " ");
      }
