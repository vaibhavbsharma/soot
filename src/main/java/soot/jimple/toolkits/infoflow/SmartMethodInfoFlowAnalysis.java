...
public class SmartMethodInfoFlowAnalysis {
  ...
  public SmartMethodInfoFlowAnalysis(UnitGraph g, InfoFlowAnalysis dfa) {
    ...
    for (Iterator<SootField> it = sm.getDeclaringClass().getFields().iterator(); it.hasNext(); ) {
      ...
      if (sf.isStatic() || !sm.isStatic()) {
        ...
        if (!sm.isStatic())
          fieldRefEqVal =
              InfoFlowAnalysis.getNodeForFieldRef(sm, sf, sm.retrieveActiveBody().getThisLocal());
          ...
          if (!sm.isStatic())
            fieldRefEqVal =
                InfoFlowAnalysis.getNodeForFieldRef(
                    sm, scField, sm.retrieveActiveBody().getThisLocal());
    ...
    // Generates a data flow graph of refs and locals where "flows to data structure" is
    ...
    if (printMessages) {
      ...
      G.v()
          .out
          .println(
              "ENDING   SMART ANALYSIS FOR "
                  + g.getBody().getMethod()
                  + " ----- "
                  + (counter - counterSoFar + 1)
                  + " analyses took: "
                  + time
                  + "s");
  ...
  private List<EquivalentValue> sourcesOf(
      EquivalentValue node,
      Set<EquivalentValue> visitedSources,
    ...
    while (sinksIt.hasNext()) {
      ...
      if (!visitedSources.contains(sink)) {
        EquivalentValue flowsToSourcesOf =
            new CachedEquivalentValue(new AbstractDataSource(sink.getValue()));
  ...
  private List<EquivalentValue> sinksOf(
      EquivalentValue node,
      Set<EquivalentValue> visitedSources,
      Set<EquivalentValue> visitedSinks) {
    List<EquivalentValue> ret = new LinkedList<EquivalentValue>();
    ...
    while (succsIt.hasNext()) {
      ...
      if (succ.getValue() instanceof AbstractDataSource) {
        ...
        if (!visitedSinks.contains(v)) {
          ...
          ret.addAll(
              sourcesOf(
                  v, visitedSinks, visitedSinks)); // these nodes are really to be marked as sinks,
          // not sources
        }
      }
    }
  ...
  protected void handleFlowsToValue(Value sink, Value source) {
    ...
    if (sink instanceof InstanceFieldRef) {
      ...
      sinkEqVal =
          InfoFlowAnalysis.getNodeForFieldRef(
              sm, ifr.getField(), (Local) ifr.getBase()); // deals with inner fields
    } else sinkEqVal = new CachedEquivalentValue(sink);
    ...
    if (source instanceof InstanceFieldRef) {
      ...
      sourceEqVal =
          InfoFlowAnalysis.getNodeForFieldRef(
              sm, ifr.getField(), (Local) ifr.getBase()); // deals with inner fields
    } else sourceEqVal = new CachedEquivalentValue(source);
  ...
  protected List<Value> handleInvokeExpr(InvokeExpr ie, Stmt is) {
    ...
    HashMutableDirectedGraph<EquivalentValue> dataFlowSummary =
        dfa.getInvokeInfoFlowSummary(ie, is, sm); // must return a graph whose nodes are Refs!!!
    if (false) // DEBUG!!!
    {
      ...
      if (method.getDeclaringClass().isApplicationClass()) {
        ...
        MutableDirectedGraph<EquivalentValue> abbreviatedDataFlowGraph =
            dfa.getInvokeAbbreviatedInfoFlowGraph(ie, sm);
        InfoFlowAnalysis.printGraphToDotFile(
            "dfg/"
                ...
                + method.getName()
                + (refOnly ? "" : "_primitive"),
            abbreviatedDataFlowGraph,
            method.getName() + (refOnly ? "" : "_primitive"),
            false);
      }
    }
    //		if( ie.getMethodRef().resolve().getSubSignature().equals(new String("boolean
    // remove(java.lang.Object)")) )
    //		{
    //			G.v().out.println("*!*!*!*!*!<boolean remove(java.lang.Object)> has FLOW SENSITIVE
    // infoFlowSummary: ");
    ...
    while (nodeIt.hasNext()) {
      ...
      if (!(nodeEqVal.getValue() instanceof Ref))
        throw new RuntimeException(
            "Illegal node type in data flow summary:"
                + nodeEqVal.getValue()
      ...
      if (node instanceof ParameterRef) {
        ...
        if (iie.getBase() == thisLocal) {
          ...
          if (false) // isNonRefType(node.getType()) ) // TODO: double check this policy
          {
            ...
            if (ifr.getBase() instanceof FakeJimpleLocal) // && ((FakeJimpleLocal)
              // ifr.getBase()).getRealLocal() != null)
        ...
        if (false) // isNonRefType(node.getType()) ) // TODO: double check this policy
        {
          ...
          InstanceFieldRef ifr = (InstanceFieldRef) node;
          if (ifr.getBase()
              instanceof FakeJimpleLocal) // && ((FakeJimpleLocal) ifr.getBase()).getRealLocal()
            // != null)
            ; // sources.add(((FakeJimpleLocal) ifr.getBase()).getRealLocal());
          else sources.add(ifr.getBase());
        } else {
        ...
        throw new RuntimeException(
            "Unknown Node Type in Data Flow Graph: node " + node + " in InvokeExpr " + ie);
      }
        ...
        if (sink instanceof ParameterRef) {
          ...
          if (iie.getBase() == thisLocal) {
            ...
            for (Iterator<Value> sourcesIt = sources.iterator(); sourcesIt.hasNext(); ) {
              ...
              if (false) // isNonRefType(sink.getType()) ) // TODO: double check this
              // policy
              {
                ...
                if (ifr.getBase() instanceof FakeJimpleLocal) // && ((FakeJimpleLocal)
                  // ifr.getBase()).getRealLocal() != null)
                  ; // handleFlowsToDataStructure(((FakeJimpleLocal)
                // ifr.getBase()).getRealLocal(), source);
      ...
      if (interestingFlow) {
        if (flowsToDataStructure) {
          ...
          while (sourcesIt.hasNext()) {
            ...
            //							handleFlowsToDataStructure(((InstanceFieldRef)sink).getBase(),
            // source);
            handleFlowsToValue(sink, source);
          }
        }
        ...
        .containsInvokeExpr()) // flows data between receiver object, parameters, globals,
    // and return value
    {
