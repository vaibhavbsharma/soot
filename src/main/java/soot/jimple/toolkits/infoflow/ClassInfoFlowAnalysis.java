...
public class ClassInfoFlowAnalysis {
  ...
  public HashMutableDirectedGraph<EquivalentValue> getMethodInfoFlowSummary(
      SootMethod method, boolean doFullAnalysis) {
    if (!methodToInfoFlowSummary.containsKey(method)) {
      methodCount++;
      ...
      // Then do smart version that does follow invoke expressions, if possible
      if (method.isConcrete()
          && doFullAnalysis) // && method.getDeclaringClass().isApplicationClass())
      {
        Body b = method.retrieveActiveBody();
        UnitGraph g = new ExceptionalUnitGraph(b);
        SmartMethodInfoFlowAnalysis smdfa = new SmartMethodInfoFlowAnalysis(g, dfa);
  ...
  private HashMutableDirectedGraph<EquivalentValue> simpleConservativeInfoFlowAnalysis(
      ...
      if (s instanceof IdentityStmt) {
        ...
        if (ir instanceof ParameterRef) {
          ...
          fieldsStaticsParamsAccessed.add(
              InfoFlowAnalysis.getNodeForParameterRef(sm, pr.getIndex()));
        }
      }
    ...
    // Create an edge from each node (except the return value) to every other node (including
    // the return value)
