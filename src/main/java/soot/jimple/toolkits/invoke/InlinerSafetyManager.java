...
public class InlinerSafetyManager {
        ...
        if (ie1 instanceof SpecialInvokeExpr) {
          ...
          if (specialTarget.isPrivate()) {
            if (specialTarget.getDeclaringClass() != container.getDeclaringClass()) {
              // Do not inline a call which contains a specialinvoke call to a private
              // method outside
              // the current class.  This avoids a verifier error and we assume will
              // not have a big
  ...
  public static boolean checkAccessRestrictions(
  ...
  public static boolean ensureInlinability(
      SootMethod target, Stmt toInline, SootMethod container, String modifierOptions) {
  ...
  /**
   * Checks the safety criteria enumerated in section 3.1.4 (Safety Criteria for Method Inlining) of
   * Vijay's thesis.
   */
  private static boolean canSafelyInlineInto(
      SootMethod inlinee, Stmt toInline, SootMethod container) {
  ...
  /**
   * Returns true if any of the following cases holds: 1. inlinee is private, but
   * container.declaringClass() != inlinee.declaringClass(); or, 2. inlinee is package-visible, and
   * its package differs from that of container; or, 3. inlinee is protected, and either: a. inlinee
   * doesn't belong to container.declaringClass, or any superclass of container; b. the class of the
   * base is not a (non-strict) subclass of container's declaringClass. The base class may be null,
   * in which case 3b is omitted. (for instance, for a static method invocation.)
   */
  private static boolean invokeThrowsAccessErrorIn(
      SootClass base, SootMethod inlinee, SootMethod container) {
