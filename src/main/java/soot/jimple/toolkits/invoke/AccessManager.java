...
public class AccessManager {
   ...
   * the following cases holds: 1. target is private, but container.declaringClass() !=
   * target.declaringClass(); or, 2. target is package-visible, and its package differs from that of
   * container; or, 3. target is protected, and either: a. container doesn't belong to
   ...
   * method, assuming the statement is in method <code>container</code>
   *
   * @param container
   * @param stmt
   * @return
  ...
  /**
   * Resolves illegal accesses in the interval ]before,after[ by creating accessor methods. <code>
   * before</code> and <code>after</code> can be null to indicate beginning/end respectively.
   *
   * @param body
   * @param before
   * @param after
  ...
  /**
   * Turns a field access or method call into a call to an accessor method. Reuses existing
   * accessors based on name mangling (see createAccessorName)
   *
   * @param container
   * @param stmt
   */
  ...
  private static void createGetAccessor(SootMethod container, AssignStmt as, FieldRef ref) {
    ...
    if (accessor == null) {
      ...
      if (ref instanceof InstanceFieldRef) {
        ...
        accStmts.addFirst(
            Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newParameterRef(target.getType(), 0)));
      }
  ...
  private static void createSetAccessor(SootMethod container, AssignStmt as, FieldRef ref) {
    ...
    if (accessor == null) {
      ...
      if (ref instanceof InstanceFieldRef) {
        accStmts.add(
            Jimple.v()
                .newIdentityStmt(thisLocal, Jimple.v().newParameterRef(target.getType(), paramID)));
        parameterTypes.add(target.getType());
        paramID++;
      }
      ...
      accStmts.add(
          Jimple.v()
              .newIdentityStmt(l, Jimple.v().newParameterRef(ref.getField().getType(), paramID)));
      paramID++;
      if (ref instanceof InstanceFieldRef) {
        accStmts.add(
            Jimple.v()
                .newAssignStmt(Jimple.v().newInstanceFieldRef(thisLocal, ref.getFieldRef()), l));
      } else {
