...
public class SiteInliner {
  ...
  /**
   * Iterates over a list of sites, inlining them in order. Each site is given as a 3-element list
   * (inlinee, toInline, container).
   */
  ...
  /**
   * Inlines the method <code>inlinee</code> into the <code>container</code> at the point <code>
   * toInline</code>.
   */
  ...
  /**
   * Inlines the given site. Note that this method does not actually check if it's safe (with
   * respect to access modifiers and special invokes) for it to be inlined. That functionality is
   * handled by the InlinerSafetyManager.
   */
  public static List inlineSite(
      SootMethod inlinee, Stmt toInline, SootMethod container, Map options) {
              ...
              .insertBefore(
                  Jimple.v()
                      .newAssignStmt(
                          ...
                          Jimple.v()
                              .newCastExpr(
                                  ((InstanceInvokeExpr) ie).getBase(), parameterType.getType())),
                  toInline);
          thisToAdd = castee;
        }
      }
    ...
    // (If enabled), add a null pointer check.
    {
      if (enableNullPointerCheckInsertion && ie instanceof InstanceInvokeExpr) {
        boolean caught =
            TrapManager.isExceptionCaughtAt(
        ...
        if (caught) {
          ...
          Stmt insertee =
              Jimple.v()
                  .newIfStmt(
                      Jimple.v().newNeExpr(((InstanceInvokeExpr) ie).getBase(), NullConstant.v()),
                      toInline);
          ...
          containerB
              ...
              .insertBefore(
                  Jimple.v()
                      .newIfStmt(
                          Jimple.v()
                              .newEqExpr(((InstanceInvokeExpr) ie).getBase(), NullConstant.v()),
                          throwPoint),
                  toInline);
        }
      }
        ...
        if (s instanceof IdentityStmt) {
          ...
          if (rhs instanceof CaughtExceptionRef) continue;
            ...
            containerUnits.swapWith(
                s,
                Jimple.v()
                    .newAssignStmt(((IdentityStmt) s).getLeftOp(), ie.getArg(pref.getIndex())));
          }
