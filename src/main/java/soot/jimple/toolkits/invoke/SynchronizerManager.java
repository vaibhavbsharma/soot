...
public class SynchronizerManager {
  ...
  public Local addStmtsToFetchClassBefore(JimpleBody jb, Stmt target) {
    ...
    units.insertBefore(
        Jimple.v().newAssignStmt(l, Jimple.v().newStaticFieldRef(classCacher.makeRef())), target);

    IfStmt ifStmt;
    units.insertBefore(
        ifStmt = Jimple.v().newIfStmt(Jimple.v().newNeExpr(l, NullConstant.v()), target), target);
  ...
  public SootMethod createClassFetcherFor(SootClass c, String methodName) {
    ...
    // Create the method body
    {
      ...
      units.add(
          Jimple.v()
              .newIdentityStmt(l_r0, Jimple.v().newParameterRef(RefType.v("java.lang.String"), 0)));
      ...
      Stmt handlerStart;
      units.add(
          handlerStart = Jimple.v().newIdentityStmt(l_r3, Jimple.v().newCaughtExceptionRef()));
      ...
      units.add(
          Jimple.v()
              .newAssignStmt(
                  l_r4, Jimple.v().newNewExpr(RefType.v("java.lang.NoClassDefFoundError"))));
              ...
              .newAssignStmt(
                  ...
                  Jimple.v()
                      .newVirtualInvokeExpr(
                          l_r1,
                          Scene.v()
                              .getMethod("<java.lang.Throwable: java.lang.String getMessage()>")
                              .makeRef(),
                          new LinkedList())));
  ...
  /**
   * Wraps stmt around a monitor associated with local lock. When inlining or static method binding,
   * this is the former base of the invoke expression.
   */
