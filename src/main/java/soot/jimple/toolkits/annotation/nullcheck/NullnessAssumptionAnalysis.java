 ...
 * non-null or neither. This information could be useful in deciding whether or not to insert code
 * that accesses a potentially null object. If the original program assumes a value is non-null,
 * then adding a use of that value will not introduce any NEW nullness errors into the program. This
...
public class NullnessAssumptionAnalysis extends BackwardFlowAnalysis {
  ...
  // TOP IS MEANINGLESS FOR THIS ANALYSIS: YOU CAN'T ASSUME A VALUE IS NULL AND NON_NULL.  BOTTOM
  ...
  protected void flowThrough(Object inValue, Object unit, Object outValue)
    ...
    //		}
    // in case of a monitor statement, we know that the programmer assumes we have a non-null
    // value
  ...
  private void handleArrayRef(ArrayRef arrayRef, AnalysisInfo out) {
    ...
    // here we know that the array must point to an object, but the array value might be
    // anything
  ...
  private void handleRefTypeAssignment(
      DefinitionStmt assignStmt, AnalysisInfo rhsInfo, AnalysisInfo out) {
    ...
    // We COULD be more accurate by assigning those assumptions to the rhs prior to this
    // statement
  ...
  protected void merge(Object in1, Object in2, Object out) {
    ...
    for (Iterator keyIter = values.iterator(); keyIter.hasNext(); ) {
      ...
      if (leftAndRight.contains(
          BOTTOM)) // if on either side we know nothing... then together we know nothing
      // for sure
      {
        result = BOTTOM;
