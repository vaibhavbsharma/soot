...
class WeightedDirectedSparseGraph {
  ...
  /**
   * addMutualEdge adds bi-direct edges between two nodes. for example, i = j + 1; generates two
   * directed edges. one from j to i with weight 1, another from i to j with weight -1
   */
  ...
  */
  ...
  public void killNode(Object tokill) {
    if (!this.vertexes.contains(tokill)) return;
    ...
    while (srcIt.hasNext()) {
      ...
      if (targets.size() == 0) sources.remove(src);
    }
  ...
  public void addBoundedAll(WeightedDirectedSparseGraph another) {
    ...
    while (thisnodeIt.hasNext()) {
      ...
      if (thistargets.size() > 0) this.sources.put(src, thistargets);
    }
  }
  ...
  */
  public void addAll(WeightedDirectedSparseGraph othergraph) {
    ...
    while (othersrcIt.hasNext()) {
      ...
      Hashtable<Object, IntContainer> thistargets =
          new Hashtable<Object, IntContainer>(othersources.size());
  ...
  public boolean equals(Object other) {
    ...
    // compare each edges. It is not always true, only when shortest path graph can be
    // guaranteed.
