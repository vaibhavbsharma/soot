...
class ArrayBoundsCheckerAnalysis {
  ...
  public ArrayBoundsCheckerAnalysis(
    ...
    ailanalysis =
        new ArrayIndexLivenessAnalysis(
            new ExceptionalUnitGraph(body), fieldin, arrayin, csin, rectarray);
  ...
  private void doAnalysis() {
    ...
    if (Options.v().debug()) {
      ...
      G.v()
          .out
          .println(
              "Building PseudoTopological order finished. "
                  + "It took "
                  + mins
                  + " mins and "
                  + secs
    ...
    /* adjust livelocals set */
    {
      ...
      while (blockIt.hasNext()) {
        ...
        HashSet<IntContainer> livelocals =
            (HashSet<IntContainer>) ailanalysis.getFlowBefore(block.getHead());
        livelocals.add(zero);
      }
    }
    ...
    if (Options.v().debug()) {
      ...
      G.v()
          .out
          .println("Doing analysis finished." + " It took " + mins + " mins and " + secs + "secs.");
    }
  }
   ...
   *        false, otherwise
   */
  private boolean assertBranchStmt(
      Object in, Unit s, Block current, List succs, List<Object> changedSuccs) {
