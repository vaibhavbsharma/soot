...
public class RectangularArrayFinder extends SceneTransformer {
    ...
    */
    ...
    if (Options.v().verbose()) {
      ...
      G.v()
          .out
          .println(
              "[ra] Rectangular array finder finishes."
                  + " It took "
                  + mins
                  + " mins and "
                  + secs
  ...
  */
  private int lookforPattern(
      Chain units, Stmt startpoint, int firstdim, Local local, Type basetype, Local[] tmplocals) {
  ...
  public boolean isRectangular(Object obj) {
    if (trueSet.contains(obj)) return true;
    else return false;
  }
}
