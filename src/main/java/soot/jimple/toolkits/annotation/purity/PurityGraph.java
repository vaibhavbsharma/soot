 ...
 * Equality comparison means isomorphism (equal nodes, equal edges).
 */

/**
 * Modifications with respect to the article:
 *
 * <p>- "unanalizable call" are treated by first constructing a conservative calee graph where all
 * parameters escape globally and return points to the global node, and then applying the standard
 * analysable call construction
 *
 * <p>- unanalysable calls add a mutation on the global node; the "field" is named "outside-world"
 * and models the mutation of any static field, but also side-effects by native methods, such as
 * I/O, that make methods impure (see below).
 *
 * <p>- Whenever a method mutates the global node, it is marked as "impure" (this can be due to a
 * side-effect or static field mutation), even if the global node is not rechable from parameter
 * nodes through outside edges. It seems to me it was a defect from the article ? TODO: see if we
 * must take the global node into account also when stating whether a parameter is read-only or
 * safe.
 *
 * <p>- "simplifyXXX" functions are experimental... they may be unsound, and thus, not used now.
 *
 * <p>NOTE: A lot of precision degradation comes from sequences of the form this.field = y; z =
 * this.field in initialisers: the second statement creates a load node because, as a parameter,
 * this may have escaped and this.field may be externally modified in-between the two instructions.
 * I am not sure this can actually happend in an initialiser... in a a function called directly and
 * only by initialisers.
 *
 * <p>For the moment, summary of unanalised methods are either pure, completely impure (modify args
 * & side-effects) or partially impure (modify args but not the gloal node). We should really be
 * able to specify more precisely which arguments are r/o or safe within this methods. E.g., the
 * analysis java.lang.String: void getChars(int,int,char [],int) imprecisely finds that this is not
 * safe (because of the internal call to System.arraycopy that, in general, may introduce aliases)
 * => it pollutes many things (e.g., StringBuffer append(String), and thus, exception constructors,
 * etc.)
 */
public class PurityGraph {
   ...
   *
   * <p>Note: this gives a valid summary for all native methods, including Thread.start().
   *
   * @param withEffect add a mutated abstract field for the global node to account for side-effects
   *     in the environment (I/O, etc.).
   */
  ...
  /**
   * Special constructor for "pure" methods returning a fresh object. (or simply pure if returns
   * void or primitive).
   */
  ...
  /**
   * Call this on the merge of graphs at all return points of a method to know whether the method is
   * pure.
   */
  ...
  /**
   * We use a less restrictive notion of purity for constructors: pure constructors can mutate
   * fields of this.
   *
   * @see isPure
   */
  ...
  /**
   * Call this on the merge of graphs at all return points of a method to know whether an object
   * passed as method parameter is read only (PARAM_RO), read write (PARAM_RW), or safe
   * (PARAM_SAFE). Returns PARAM_RW for primitive-type parameters.
   */
  ...
  /**
   * Experimental simplification: remove inside nodes not reachable from escaping nodes (params,
  ...
  /**
   * Remove all local bindings (except ret). This info is indeed superfluous on summary purity
   * graphs representing the effect of a method. This saves a little memory, but also, simplify
   * summary graph drawings a lot!
   *
   * <p>DO NOT USE DURING INTRA-PROCEDURAL ANALYSIS!
   */
  void removeLocals() {
    locals = new HashMultiMap();
    backLocals = new HashMultiMap();
  ...
  void methodCall(PurityGraph g, Local right, List args, Local left) {
    ...
    while (it.hasNext()) {
      ...
      if (arg instanceof Local && arg.getType() instanceof RefLikeType) {
   ...
   *
   * @param prefix is used to prefix all dot node and edge names. Use it to avoid collision when
   *     several subgraphs are laid in the same dot file!
   * @param out is a newly created dot graph or subgraph where to put the result.
   *     <p>Note: outside edges, param and load nodes are gray dashed, while inside edges and nodes
