 ...
 *
 * <p>You must pass an {@link AbstractInterproceduralAnalysis} object so that the intraprocedural
 * part can resolve the effect of method calls.
 */
public class PurityIntraproceduralAnalysis extends ForwardFlowAnalysis<Unit, PurityGraphBox> {
  ...
  /**
   * Draw the result of the intra-procedural analysis as one big dot file, named
   * className.methodName.dot, containing one purity graph for each statement in the method.
   *
   * @param prefix
   * @param name
   */
  ...
  /**
   * Put into dst the purity graph obtained by merging all purity graphs at the method return. It is
   * a valid summary that can be used in methodCall if you do interprocedural analysis.
   *
   * @param dst
   */
