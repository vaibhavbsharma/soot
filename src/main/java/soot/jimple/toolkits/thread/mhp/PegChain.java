...
public class PegChain extends HashChain {
  ...
  private void visit(
      Unit unit, UnitGraph graph, SootMethod sm, String threadName, boolean addBeginNode) {
    /*
    ...
    if (((Stmt) unit).containsInvokeExpr()) {
      ...
      if (method.getName().equals("start")) {
        ...
        List<SootClass> superClasses =
            hierarchy.getSuperclassesOfIncluding(method.getDeclaringClass());
      ...
      if (name.equals("wait")
        ...
        if ((name.equals("start") || name.equals("run")) && find) {
          ...
          while (mayAliasIt.hasNext()) {
            ...
            */
            SootMethod meth =
                hierarchy.resolveConcreteDispatch(
                    maySootClass, method.getDeclaringClass().getMethodByName("run"));
                ...
                if (method.isConcrete() && !method.getDeclaringClass().isLibraryClass()) {
                  ...
                  if (invokeExpr instanceof StaticInvokeExpr) {
                    ...
                    if (targetList.size() > 1) {
                      ...
                      System.exit(
                          1); // What SHOULD be done is that all possible targets are inlined
  ...
  private void transformWaitNode(
      String objName, String name, String threadName, Unit unit, UnitGraph graph, SootMethod sm) {
