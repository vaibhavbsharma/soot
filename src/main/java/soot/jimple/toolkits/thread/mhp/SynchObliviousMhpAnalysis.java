 ...
 * synchronization statements (synchronized, wait, notify, etc.) are ignored. If the program has no
...
public class SynchObliviousMhpAnalysis implements MhpTester, Runnable {
  ...
  boolean optionThreaded =
      false; // DOESN'T WORK if set to true... ForwardFlowAnalysis uses a static field in a
  // thread-unsafe way
  ...
  protected void buildThreadList() // can only be run once if optionThreaded is true
      {
    if (optionThreaded) {
      if (self != null) return; // already running... do nothing
  ...
  public void run() {
    ...
    if (!(pta instanceof PAG)) {
      throw new RuntimeException(
          "You must use Spark for points-to analysis when computing MHP information!");
    }
    PAG pag = (PAG) pta;
    ...
    while (threadIt.hasNext()) {
      ...
      // Get a list of all possible unique Runnable.run methods for this thread start
      // statement
      AbstractRuntimeThread thread =
          new AbstractRuntimeThread(); // provides a list interface to the methods in a
      // thread's sub-call-graph
      ...
      // Get a list containing all methods in the call graph(s) rooted at the possible run
      // methods for this thread start statement
      // AKA a list of all methods that might be called by the thread started here
      int methodNum = 0;
      while (methodNum < thread.methodCount()) // iterate over all methods in threadMethods, even as
      // new methods are being added to it
      {
        ...
        while (succMethodsIt.hasNext()) {
          ...
          // (because it's a run method that won't be called as part of THIS thread) THIS
          // IS NOT OPTIMAL
          ...
          while (edgeInIt.hasNext()) {
            ...
            if (edge.kind() != Kind.THREAD
                ...
                && thread.containsMethod(edge.src())) // called directly by any of the thread
              // methods?
              ignoremethod = false;
          }
          if (!ignoremethod && !thread.containsMethod(method)) thread.addMethod(method);
        }
        methodNum++;
      }
      ...
      if (optionPrintDebug) System.out.println(thread.toString());
      ...
      {
        if (multiRunAllocNodes.contains(
            threadAllocNodes.iterator().next())) // but it gets run more than once
        {
          mayStartMultipleThreadObjects =
              true; // then "thread" in "thread.start()" could be more than one object
        }
      }

      if (mayStartMultipleThreadObjects) thread.setStartStmtHasMultipleReachingObjects();
      ...
      boolean mayBeRunMultipleTimes =
          multiCalledMethods.contains(startStmtMethod); // if method is called more than once...
      if (!mayBeRunMultipleTimes) {
        ...
        FlowSet multiRunStatements =
            finder.getMultiRunStatements(); // list of all units that may be run more
        // than once in this method
        if (multiRunStatements.contains(startStmt)) mayBeRunMultipleTimes = true;
      }
      ...
      // If a run-many thread.start() statement is (always) associated with a join statement
      // in the same method,
      // then it may be possible to treat it as run-once, if this method is non-reentrant and
      // called only
      ...
      if (mayBeRunMultipleTimes && startToJoin.containsKey(startStmt)) {
        ...
        // methods are being added to it
        {
          ...
          while (succMethodsIt.hasNext()) {
            ...
            if (!containingMethodCalls.contains(method)) containingMethodCalls.add(method);
          }
          methodNum++;
        }
        if (!mayBeRunMultipleTimes) { // There's still one thing that might cause this to be
          // run multiple times: if it can be run in parallel
          // with itself
    ...
    while (methodNum < mainThread.methodCount()) {
      ...
      while (succMethodsIt.hasNext()) {
        ...
        Iterator edgeInIt = callGraph.edgesInto(method);
        while (edgeInIt.hasNext()) {
          if (((Edge) edgeInIt.next()).kind() != Kind.THREAD) ignoremethod = false;
        }
        if (!ignoremethod && !mainThread.containsMethod(method)) mainThread.addMethod(method);
      }
      methodNum++;
    }
    if (optionPrintDebug) G.v().out.println(mainThread.toString());

    // Revisit the containing methods of start-join pairs that are non-reentrant but might be
    // called in parallel
    ...
    while (addedNew) {
      ...
      while (it.hasNext()) {
        ...
        if (mayHappenInParallelInternal(someStartMethod, someStartMethod)) {
          ...
          it.remove();
          if (optionPrintDebug) G.v().out.println(someThread.toString());
          addedNew = true;
        }
      }
    }
  ...
  public boolean mayHappenInParallel(SootMethod m1, Unit u1, SootMethod m2, Unit u2) {
    if (optionThreaded) {
      if (self == null) return true; // not started...
  ...
  public void printMhpSummary() {
    if (optionThreaded) {
      if (self == null) return; // not run... do nothing
  ...
  public List<SootClass> getThreadClassList() {
    if (optionThreaded) {
      if (self == null) return null; // not run... do nothing
    ...
    for (int i = 0; i < size; i++) {
      ...
      while (threadRunMethodIt.hasNext()) {
        SootClass threadClass =
            ((SootMethod) threadRunMethodIt.next()).getDeclaringClass(); // what about subclasses???
        if (!threadClasses.contains(threadClass)
