...
public class EncapsulatedMethodAnalysis // extends ForwardFlowAnalysis
 {
  ...
  public EncapsulatedMethodAnalysis(UnitGraph g) {
    ...
    // This is conservative... many "may-be-pure" functions can be proven pure if we track what
    // fields are object-pure
    // Also, if we do this, we should be able to track what fields refer to object-local
    // objects, which will allow
    // lock assignment to work when inner objects are read/written (because they can be ignored
    // for read/write sets).
  }
