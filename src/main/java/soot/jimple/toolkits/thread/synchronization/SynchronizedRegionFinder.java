  ...
  protected void flowThrough(
    ...
    while (outIt.hasNext()) {
      ...
      if (srfp.inside == true && (tn.nestLevel == nestLevel || optionOpenNesting == false)) {
        printed = true; // for debugging purposes, indicated that we'll print a debug output
        // for this statement
        ...
        if (stmt.containsInvokeExpr()) {
          ...
          if (!tn.invokes.contains(unit)) {
            // Mark this unit for later read/write set calculation (must be deferred
            // until all tns have been found)
            tn.invokes.add(unit);
    ...
    if (optionPrintDebug) {
      ...
      // If this unit is an invoke statement calling a library function and the R/W sets are
      // huge, print out the targets
      if (stmt.containsInvokeExpr()
    ...
    if (addSelf) {
      ...
      while (prepUnitsIt.hasNext()) {
        ...
        while (uses.hasNext()) {
          ...
          if (use.getUnit() == (Unit) unit) { // if this transaction's monitorenter statement is one
            // of the uses of this preparatory unit
