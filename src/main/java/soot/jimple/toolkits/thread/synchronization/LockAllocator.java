...
public class LockAllocator extends SceneTransformer {
  ...
  protected void internalTransform(String phaseName, Map<String, String> options) {
    ...
    //		optionOnFlyTLO = PhaseOptions.getBoolean( options, "on-fly-tlo" ); // not a real option
    // yet

    //		optionPrintMhpSummary = PhaseOptions.getBoolean( options, "print-mhp" ); // not a real
    // option yet
    ...
    //		optionIncludeEmptyPossibleEdges = PhaseOptions.getBoolean( options,
    // "include-empty-edges" ); // not a real option yet
    ...
    if (optionDoTLO) {
      ...
      if (mhp != null) tlo = new ThreadLocalObjectsAnalysis(mhp);
      else tlo = new ThreadLocalObjectsAnalysis(new SynchObliviousMhpAnalysis());
      if (!optionOnFlyTLO) {
        tlo.precompute();
        G.v()
            .out
            .println(
                "[wjtp.tn] TLO totals (#analyzed/#encountered): "
                    + SmartMethodInfoFlowAnalysis.counter
                    + "/"
        ...
        G.v()
            .out
            .println(
                "[wjtp.tn] TLO so far (#analyzed/#encountered): "
                    + SmartMethodInfoFlowAnalysis.counter
                    + "/"
                    + ClassInfoFlowAnalysis.methodCount);
    }
    ...
    Map<SootMethod, ExceptionalUnitGraph> methodToExcUnitGraph =
    ...
    tasea =
        new CriticalSectionAwareSideEffectAnalysis(
            pta, Scene.v().getCallGraph(), (optionOpenNesting ? criticalSections : null), tlo);
    ...
    while (tnIt.hasNext()) {
      ...
      for (Unit unit : tn.invokes) {
        ...
        if (stmtRead != null) tn.read.union(stmtRead);

        RWSet stmtWrite = tasea.writeSet(tn.method, stmt, tn, uses);
        if (stmtWrite != null) tn.write.union(stmtWrite);
      }
    }
    ...
    DeadlockDetector dd =
        new DeadlockDetector(optionPrintDebug, optionAvoidDeadlock, true, criticalSections);
    ...
    if (!optionStaticLocks) {
      ...
      for (CriticalSection tn : criticalSections) {
        ...
        for (CriticalSectionDataDependency tdd : tn.edges) tn.group.rwSet.union(tdd.rw);
      }
    }
            ...
            G.v()
                ...
                .println(
                    "[wjtp.tn] "
                        + tn.name
                        + " lockset: "
                        + locksetToLockNumString(tn.lockset, lockToLockNum)
    ...
    if (optionUseLocksets) // deadlock detection and lock ordering for lockset allocations
    {
      G.v()
          ...
          .println(
              "[wjtp.tn] *** Detect "
                  + (optionAvoidDeadlock ? "and Correct " : "")
                  + "the Possibility of Deadlock for Locksets *** "
                  + (new Date()));
      deadlockGraph = dd.detectLocksetDeadlock(lockToLockNum, lockPTSets);
      if (optionPrintDebug) ((HashMutableEdgeLabelledDirectedGraph) deadlockGraph).printGraph();
    ...
    if (optionPrintGraph) printGraph(criticalSections, ig, lockToLockNum);
    ...
    if (!optionLeaveOriginalLocks) {
      // Create an array of booleans to keep track of which global locks have been inserted
      // into the program
      ...
      for (int i = 1; i < ig.groupCount(); i++) {
        ...
        insertedGlobalLock[i] =
            (!optionOneGlobalLock) && (tnGroup.useDynamicLock || tnGroup.useLocksets);
      }
      ...
      for (SootClass appClass : Scene.v().getApplicationClasses()) {
        for (SootMethod method : appClass.getMethods()) {
          if (method.isConcrete()) {
            ...
            LockAllocationBodyTransformer.v()
                  .internalTransform(method.getActiveBody(), fs, ig.groups(), insertedGlobalLock);
          }
        }
      }
    }
  ...
  protected void findLockableReferences(
      List<CriticalSection> AllTransactions,
    ...
    while (tnIt9.hasNext()) {
      ...
      if (tn.group.useDynamicLock
          || tn.group.useLocksets) // if attempting to use a dynamic lock or locksets
      {
        ...
        LockableReferenceAnalysis la =
            new LockableReferenceAnalysis(new BriefUnitGraph(tn.method.retrieveActiveBody()));
        ...
        if (optionUseLocksets) {
          ...
          if (tn.lockset == null || tn.lockset.size() <= 0) {
            ...
            // Create a lockset containing a single placeholder static lock for each tn
            // in the group
            ...
            PointsToSetInternal dummyLockPT =
                new HashPointsToSet(
                    newStaticLock.getType(),
                    (PAG) pta); // KILLS CHA-BASED ANALYSIS (pointer exception)
            ...
            for (EquivalentValue lockEqVal : tn.lockset) {
              ...
              if (lock instanceof Local)
                lockPT = (PointsToSetInternal) pta.reachingObjects((Local) lock);
                  ...
                  instanceof StaticFieldRef) // needs special treatment: could be primitive
              lockPT = null;
                ...
                if (base instanceof FakeJimpleLocal)
                  lockPT =
                      (PointsToSetInternal)
                          pta.reachingObjects(
                              ((FakeJimpleLocal) base).getRealLocal(),
                              ((FieldRef) lock).getField());
                else
                  lockPT =
                      (PointsToSetInternal) pta.reachingObjects(base, ((FieldRef) lock).getField());
              } else if (lock
                  instanceof NewStaticLock) // placeholder for anything that needs a static
                // lock
                lockPT = null;
              else lockPT = null;
              ...
              if (lockPT != null) {
                ...
                for (int i = 0; i < lockPTSets.size(); i++) {
                  ...
                  if (lockPT.hasNonEmptyIntersection(
                      otherLockPT)) // will never happen for empty, negative
                  // numbered sets
                  {
                    G.v()
                        .out
                ...
                if (!foundLock) {
                  ...
                  PointsToSetInternal otherLockPT =
                      new HashPointsToSet(lockPT.getType(), (PAG) pta);
              ...
              } else // static field locks and pathological cases...
              {
                ...
                if (lockToLockNum.get(lockEqVal) != null) {
                  ...
                  Integer lockNum = new Integer(-lockPTSets.size()); // negative indicates a static
                  // lock
                  G.v()
                      .out
                      .println(
    ...
    if (optionUseLocksets) {
      ...
      for (int i = 0; i < lockPTSets.size(); i++) {
        ...
        if (pts.size() == 1
            && false) // isSingleton(pts)) // It's NOT easy to find a singleton: single
        // alloc node must be run just once
        {
  ...
  private void analyzeExistingLocks(
      List<CriticalSection> AllTransactions, CriticalSectionInterferenceGraph ig) {
    setFlagsForStaticAllocations(ig);

    // if for any lock there is any def to anything other than a static field, then it's a local
    // lock.
  ...
  public void assignNamesToTransactions(List<CriticalSection> AllTransactions) {
    ...
    while (tnIt5.hasNext()) {
      ...
      String mname = tn1.method.getSignature(); // tn1.method.getSignature() + "." +
      // tn1.method.getName();
      if (!methodNamesTemp.contains(mname)) methodNamesTemp.add(mname);
    }
    ...
    int identMatrix[][] =
        new int[methodNames.length][CriticalSection.nextIDNum - methodNames.length + 2];
    ...
    while (tnIt4.hasNext()) {
      ...
      tn1.name =
          "m"
              + (methodNum < 10 ? "00" : (methodNum < 100 ? "0" : ""))
              + methodNum
              + "n"
              + (tnNum < 10 ? "0" : "")
              + tnNum;
  ...
  public void printGraph(
      Collection<CriticalSection> AllTransactions,
      CriticalSectionInterferenceGraph ig,
      Map<Value, Integer> lockToLockNum) {
    final String[] colors = {
      "black",
    ...
    for (int group = 0; group < ig.groups().size(); group++) {
        ...
        if (tn.setNumber == group + 1) {
          if (!printedHeading) {
            ...
            if (tn.group.useDynamicLock && tn.group.lockObject != null) {
              ...
              //								typeString = ((RefType)
              // lockObject[group].getType()).getSootClass().getShortName();
              //							else
              //								typeString = lockObject[group].getType().toString();
              if (tn.group.lockObject.getType() instanceof RefType)
                typeString =
                    ((RefType) tn.group.lockObject.getType()).getSootClass().getShortName();
              else typeString = tn.group.lockObject.getType().toString();
              G.v()
                  .out
                  .println(
                      "[transaction-graph] subgraph cluster_"
                          + (group + 1)
                      ...
                      G.v()
                          ...
                          .println(
                              "[transaction-graph] "
                                  ...
                                  + color
                                  + " style="
                                  + (lockNum >= 0 ? "dashed" : "solid")
                                  + " exactsize=1 style=\"setlinewidth(3)\"];");
                    }
                  }
                }
              }
                ...
                G.v()
                    ...
                    .println(
                        "[transaction-graph] "
                            ...
                            + tnedge.name
                            + " [color="
                            + (edge.size > 0 ? "black" : "cadetblue1")
                            + " style="
                            + (tn.setNumber > 0 && tn.group.useDynamicLock ? "dashed" : "solid")
                            + " exactsize="
                            + edge.size
                            + " style=\"setlinewidth(3)\"];");
            }
      ...
      if (printedHeading) G.v().out.println("[transaction-graph] }");
    }
    ...
    // Print nodes with no group
    {
      ...
      while (tnIt.hasNext()) {
        ...
        if (tn.setNumber == -1) {
          if (!printedHeading) {
            // putting these nodes in a "source" ranked subgraph makes them appear above
            // all the clusters
            G.v()
                .out
                .println("[transaction-graph] subgraph lone {\n[transaction-graph] rank=source;");
            printedHeading = true;
          }
          if (Scene.v().getReachableMethods().contains(tn.method))
            G.v()
  ...
  public void printTable(Collection<CriticalSection> AllTransactions, MhpTester mhp) {
    ...
    while (tnIt7.hasNext()) {
      ...
      boolean mhpself = false;
      {
        ...
        if (mhp != null) mhpself = mhp.mayHappenInParallel(tn.method, tn.method);
      }
      G.v()
          .out
          .println(
      ...
      G.v()
          .out
          .print(
              "[transaction-table] End  : early:"
                  + tn.earlyEnds.toString()
                  + " exc:"
                  + tn.exceptionalEnd
                  + " through:"
                  + tn.end
      ...
      if (tn.read.size() < 100)
        ...
        G.v()
            .out
            .print("[transaction-table] Read : " + tn.read.size() + "  \n[transaction-table] ");
      ...
      // statement
        ...
        G.v()
            ...
            .print(
                "Write: "
                    + tn.write.size()
                    + "\n[transaction-table] "); // label provided by previous
      // print statement
      G.v()
          .out
          .print("Edges: (" + tn.edges.size() + ") "); // label provided by previous print statement
      Iterator<CriticalSectionDataDependency> tnedgeit = tn.edges.iterator();
      while (tnedgeit.hasNext()) G.v().out.print(tnedgeit.next().other.name + " ");
  ...
  public void printGroups(
      Collection<CriticalSection> AllTransactions, CriticalSectionInterferenceGraph ig) {
    ...
    for (int group = 0; group < ig.groupCount() - 1; group++) {
      ...
      if (tnGroup.size() > 0) {
        ...
        while (tnIt.hasNext()) {
          CriticalSection tn = tnIt.next();
          if (tn.setNumber == group + 1) G.v().out.print(tn.name + " ");
        }
        G.v()
            .out
            .print(
    ...
    while (tnIt.hasNext()) {
      CriticalSection tn = tnIt.next();
      if (tn.setNumber == -1) G.v().out.print(tn.name + " ");
    }
    G.v().out.println("\n[transaction-groups] ");
  }
