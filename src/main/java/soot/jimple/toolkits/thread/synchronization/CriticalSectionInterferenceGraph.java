...
public class CriticalSectionInterferenceGraph {
  ...
  public CriticalSectionInterferenceGraph(
      List<CriticalSection> criticalSections,
      MhpTester mhp,
    ...
    } else // calculate separate groups for transactions
    {
      ...
      while (tnIt1.hasNext()) {
        ...
        if (tn1.read.size() == 0 && tn1.write.size() == 0 && !optionLeaveOriginalLocks) {
          ...
          tn1.setNumber = -1; // AKA delete the transactional region (but don't really so long as
          // we are using
              ...
              if (size > 0) {
                ...
                if (tn1.setNumber > 0) {
                  ...
                  // if tn2 is already in a group
                    ...
                    if (tn1.setNumber != tn2.setNumber) // if they are equal, then they
