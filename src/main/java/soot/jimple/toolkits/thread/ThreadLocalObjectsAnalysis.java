...
public class ThreadLocalObjectsAnalysis extends LocalObjectsAnalysis
  ...
  public ThreadLocalObjectsAnalysis(MhpTester mhp) // must include main class
    ...
    this.primitiveDfa =
        new InfoFlowAnalysis(true, true, printDebug); // ref+primitive, with inner fields
  ...
  */
  ...
  /**
   * Returns a list of thread-shared sources and sinks. Returns empty list if not actually a shared
   * value.
   */
  public List escapesThrough(Value sharedValue, SootMethod containingMethod) {
    ...
    for (AbstractRuntimeThread thread : mhp.getThreads()) {
      // Each "abstract thread" from the MHP analysis actually represents a "Thread.start()"
      // statement that could
      // be starting one of several different kinds of threads.  We must consider each kind
      // separately.
      for (Object meth : thread.getRunMethods()) {
        SootMethod runMethod = (SootMethod) meth;
        ...
        if (runMethod.getDeclaringClass().isApplicationClass()
          ...
          ClassLocalObjectsAnalysis cloa =
              getClassLocalObjectsAnalysis(containingMethod.getDeclaringClass());
          ...
          for (EquivalentValue source : sources) {
            if (source.getValue() instanceof Ref) {
              ...
              {
                ret.add(source);
                //	                            System.out.println(sharedValue + " in
                // " + containingMethod + " escapes through " + source);
              }
            }
          }
        }
