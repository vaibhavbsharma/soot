 ...
 * A critical edge is an edge from Block A to block B, if B has more than one predecessor and A has
 * more the one successor.<br>
 * As an example: If we wanted a computation to be only on the path A-&gt;B this computation must be
 * directly on the edge. Otherwise it is either executed on the path through the second predecessor
 * of A or throught the second successor of B.<br>
 * Our critical edge-remover overcomes this problem by introducing synthetic nodes on this critical
 * edges.<br>
...
public class CriticalEdgeRemover extends BodyTransformer {
   ...
   * As a side-effect, after the transformation, the direct predecessor of a block/node with
   * multiple predecessors will will not fall through anymore. This simplifies the algorithm and is
