          ...
          if (!type.hasDescendant(child.type)) {
            if (DEBUG) {
              G.v()
                  .out
                  .println(
                      child.type + "(" + child + ") is not a child of " + type + "(" + this + ")");
            }
    ...
    for (BitSetIterator i = parents.iterator(); i.hasNext(); ) {
      ...
      if (var.depth() == depth) {
        ...
        if (var.type() == null) {
          ...
          if (!Options.v().j2me()) {
            var.addChild(
                resolver.typeVariable(ArrayType.v(RefType.v("java.lang.Cloneable"), var.depth())));
            var.addChild(
                resolver.typeVariable(ArrayType.v(RefType.v("java.io.Serializable"), var.depth())));
          }
        }
      }
    }
