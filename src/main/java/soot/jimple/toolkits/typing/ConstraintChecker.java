...
class ConstraintChecker extends AbstractStmtSwitch {
  ...
  private void handleInvokeExpr(InvokeExpr ie, Stmt invokestmt) {
    ...
    for (int i = 0; i < ie.getArgCount(); i++) {
      if (ie.getArg(i) instanceof Local) {
        ...
        if (!hierarchy
            ...
            .hasAncestorOrSelf(hierarchy.typeNode(method.parameterType(i)))) {
          if (fix) {
    ...
    if (ie instanceof InterfaceInvokeExpr) {
      ...
      if (base instanceof Local) {
        ...
        if (!hierarchy
            .typeNode(local.getType())
            .hasAncestorOrSelf(hierarchy.typeNode(method.declaringClass().getType()))) {
          if (fix) {
      ...
      for (int i = 0; i < die.getBootstrapArgCount(); i++) {
        if (die.getBootstrapArg(i) instanceof Local) {
          ...
          if (!hierarchy
              .typeNode(local.getType())
              .hasAncestorOrSelf(hierarchy.typeNode(bootstrapMethod.parameterType(i)))) {
            if (fix) {
  ...
  public void caseAssignStmt(AssignStmt stmt) {
    ...
    if (l instanceof ArrayRef) {
      ...
      if (!base.hasAncestorOrSelf(
          hierarchy.typeNode(ref.getField().getDeclaringClass().getType()))) {
        if (fix) {
          ref.setBase(
              insertCast(
                  (Local) ref.getBase(), ref.getField().getDeclaringClass().getType(), stmt));
        } else {
          error("Type Error(18)");
        }
      }
    ...
    if (r instanceof ArrayRef) {
        ...
        if (fix) {
          ...
          if (lefttype instanceof ArrayType) {
            ...
            ref.setBase(
                insertCast(
                    (Local) ref.getBase(),
                    ArrayType.v(atype.baseType, atype.numDimensions + 1),
                    stmt));
          } else {
      ...
      if (lv instanceof Local) {
        ...
        throw new RuntimeException(
            "Unhandled binary expression left operand type: " + lv.getClass());
      }
      ...
      if (rv instanceof Local) {
        ...
        throw new RuntimeException(
            "Unhandled binary expression right operand type: " + rv.getClass());
      }
      ...
      if (ne.getOp() instanceof Local) {
        ...
        throw new RuntimeException(
            "Unhandled neg expression operand type: " + ne.getOp().getClass());
      }
