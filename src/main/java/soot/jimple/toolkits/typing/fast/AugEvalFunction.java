...
public class AugEvalFunction implements IEvalFunction {
  ...
  public static Type eval_(Typing tg, Value expr, Stmt stmt, JimpleBody jb) {
    if (expr instanceof ThisRef) return expr.getType();
    else if (expr instanceof ParameterRef) return expr.getType();
    else if (expr instanceof Local) {
      Local ex = (Local) expr;
      // changed to prevent null pointer exception in case of phantom classes where a null
      // typing is encountered
      // syed
      ...
      if (expr instanceof CmpExpr || expr instanceof CmpgExpr || expr instanceof CmplExpr)
        return ByteType.v();
          ...
          || expr instanceof NeExpr) return BooleanType.v();
      ...
      if (t instanceof IntegerType) {
        ...
        if (t instanceof Integer1Type
            ...
            || t instanceof ByteType) return ByteType.v();
      ...
      for (RefType t : TrapManager.getExceptionTypesOf(stmt, jb)) {
        if (r == null) r = t;
        else if (t.getSootClass().isPhantom() || r.getSootClass().isPhantom()) r = throwableType;
        else
      ...
      if (at instanceof ArrayType) return ((ArrayType) at).getElementType();
        ...
        if (ref.getSootClass().getName().equals("java.lang.Object")
            ...
            || ref.getSootClass().getName().equals("java.lang.Cloneable")) return ref;
        else return BottomType.v();
      } else return BottomType.v();
    ...
    else if (expr instanceof FieldRef) return expr.getType();
