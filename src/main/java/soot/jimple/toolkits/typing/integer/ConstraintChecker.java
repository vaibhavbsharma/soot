...
class ConstraintChecker extends AbstractStmtSwitch {
  ...
  public void caseAssignStmt(AssignStmt stmt) {
    ...
    if (left != null && right != null) {
      if (!right.hasAncestor_1(left)) {
        if (fix) {
          stmt.setRightOp(
              insertCast(stmt.getRightOp(), getTypeForCast(right), getTypeForCast(left), stmt));
        } else {
          error("Type Error(15)");
        }
      }
        ...
        if (!right.hasAncestor_1(left)) {
          if (fix) {
            ((soot.jimple.internal.JIdentityStmt) stmt)
                .setLeftOp(
                    insertCastAfter((Local) l, getTypeForCast(left), getTypeForCast(right), stmt));
          } else {
            error("Type Error(16)");
          }
        }
    ...
    if (stmt.getOp() instanceof Local) {
      if (((Local) stmt.getOp()).getType() instanceof IntegerType) {
        if (!ClassHierarchy.v()
          ...
          if (fix) {
            stmt.setOp(
                insertCast((Local) stmt.getOp(), stmtBody.getMethod().getReturnType(), stmt));
          } else {
            error("Type Error(19)");
          }
        }
  ...
  private Local insertCastAfter(Local leftlocal, Type lefttype, Type righttype, Stmt stmt) {
    ...
    stmtBody
        .getUnits()
        .insertAfter(
            Jimple.v().newAssignStmt(leftlocal, Jimple.v().newCastExpr(newlocal, lefttype)), u);
    return newlocal;
  }
