...
public class TypeAssigner extends BodyTransformer {
  ...
  protected void internalTransform(Body b, String phaseName, Map<String, String> options) {
    ...
    if (Options.v().verbose()) {
      ...
      G.v()
          .out
          .println(
              "[TypeAssigner] typing system ended. It took "
                  + mins
                  + " mins and "
                  + secs
                  + " secs.");
  ...
  private void replaceNullType(Body b) {
    ...
    if (!opts.containsKey("enabled") || !opts.get("enabled").equals("true")) {
      G.v()
          .out
          .println(
              "Warning: Cannot run TypeAssigner.replaceNullType(Body). Try to enable jop.cfg.");
      return;
    }
    ...
    for (Unit u : unitToReplaceByException) {
      soot.dexpler.Util.addExceptionAfterUnit(
          b,
  ...
  private void compareTypeAssigners(Body b, boolean useOlderTypeAssigner) {
    ...
    G.v()
        .out
        .println("cmp;" + jb.getMethod() + ";" + size + ";" + oldTime + ";" + newTime + ";" + cmp);
  }
