...
public class ReflectiveCallsInliner extends SceneTransformer {
  ...
  protected void internalTransform(String phaseName, @SuppressWarnings("rawtypes") Map options) {
    if (!initialized) {
      ...
      Scene.v()
          .getSootClass(IUnexpectedReflectiveCallHandler.class.getName())
          .setApplicationClass();
      ...
      reflectiveCallsClass =
          new SootClass("soot.rtlib.tamiflex.ReflectiveCallsWrapper", Modifier.PUBLIC);
      Scene.v().addClass(reflectiveCallsClass);
      reflectiveCallsClass.setApplicationClass();

      UNINTERPRETED_METHOD =
          Scene.v()
              .makeMethodRef(
                  Scene.v().getSootClass("soot.rtlib.tamiflex.OpaquePredicate"),
                  "getFalse",
                  Collections.emptyList(),
                  BooleanType.v(),
                  true);

      if (useCaching) addCaching();
      ...
      }
      {
        ...
        if (!classNewInstanceClassNames.isEmpty()) {
          inlineRelectiveCalls(
              m, classNewInstanceClassNames, ReflectionTraceInfo.Kind.ClassNewInstance);
          if (Options.v().validate()) b.validate();
        }
      }
      {
        ...
        if (!constructorNewInstanceSignatures.isEmpty()) {
          inlineRelectiveCalls(
              m, constructorNewInstanceSignatures, ReflectionTraceInfo.Kind.ConstructorNewInstance);
          if (Options.v().validate()) b.validate();
        }
      }
      {
    ...
    for (SootMethod m : RTI.methodsContainingReflectiveCalls()) {
      {
        if (!RTI.classForNameClassNames(m).isEmpty()) {
          SootFieldRef fieldRef =
              Scene.v()
                  .makeFieldRef(reflCallsClass, "classForName", RefType.v("java.util.Set"), true);
            ...
            InterfaceInvokeExpr invokeExpr =
                Jimple.v()
                    .newInterfaceInvokeExpr(
                        setLocal, addMethodRef, StringConstant.v(callSiteId + className));
      ...
      }
      {
        if (!RTI.classNewInstanceClassNames(m).isEmpty()) {
          SootFieldRef fieldRef =
              Scene.v()
                  .makeFieldRef(
                      reflCallsClass, "classNewInstance", RefType.v("java.util.Set"), true);
          Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
          newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
      ...
      }
      {
        if (!RTI.constructorNewInstanceSignatures(m).isEmpty()) {
          SootFieldRef fieldRef =
              Scene.v()
                  .makeFieldRef(
                      reflCallsClass, "constructorNewInstance", RefType.v("java.util.Set"), true);
          Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
          newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
          for (String constrSig : RTI.constructorNewInstanceSignatures(m)) {
            InterfaceInvokeExpr invokeExpr =
                Jimple.v()
                    .newInterfaceInvokeExpr(
                        setLocal, addMethodRef, StringConstant.v(callSiteId + constrSig));
            newUnits.add(Jimple.v().newInvokeStmt(invokeExpr));
          }
          callSiteId++;
        }
      }
      {
        if (!RTI.methodInvokeSignatures(m).isEmpty()) {
          SootFieldRef fieldRef =
              Scene.v()
                  .makeFieldRef(reflCallsClass, "methodInvoke", RefType.v("java.util.Set"), true);
          Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
          newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
          for (String methodSig : RTI.methodInvokeSignatures(m)) {
            InterfaceInvokeExpr invokeExpr =
                Jimple.v()
                    .newInterfaceInvokeExpr(
                        setLocal, addMethodRef, StringConstant.v(callSiteId + methodSig));
            newUnits.add(Jimple.v().newInvokeStmt(invokeExpr));
          }
          callSiteId++;
        }
  ...
  private void addCaching(Kind kind) {
    ...
    InstanceFieldRef fieldRef =
        Jimple.v()
            .newInstanceFieldRef(
                body.getParameterLocal(m.getParameterCount() - 1),
    ...
    newUnits.add(
        Jimple.v()
            .newIfStmt(Jimple.v().newEqExpr(alreadyCheckedLocal, IntConstant.v(0)), jumpTarget));
    ...
    while (iter.hasNext()) {
      ...
      if (s.containsInvokeExpr()) {
        ...
        for (String target : targets) {
          ...
          newUnits.add(
              Jimple.v().newIfStmt(Jimple.v().newEqExpr(IntConstant.v(0), predLocal), jumpTarget));
          ...
          StaticInvokeExpr methodInvokeExpr =
              Jimple.v().newStaticInvokeExpr(newMethod.makeRef(), args);
  ...
  private SootMethod createNewMethod(Kind callKind, String target, Type fieldSetGetType) {
    ...
    if (!returnType.equals(VoidType.v())) newUnits.add(Jimple.v().newReturnStmt(retLocal));
  ...
  private void unboxParameter(
    ...
    if (paramType instanceof PrimType) {
      ...
      AssignStmt cast =
          Jimple.v().newAssignStmt(castedLocal, Jimple.v().newCastExpr(boxedLocal, boxedType));
      newUnits.add(cast);
      ...
      AssignStmt cast =
          Jimple.v().newAssignStmt(castedLocal, Jimple.v().newCastExpr(boxedLocal, paramType));
      newUnits.add(cast);
