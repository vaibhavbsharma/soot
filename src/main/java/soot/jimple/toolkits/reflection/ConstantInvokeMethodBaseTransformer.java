 ...
 * Method.invoke call. Therefore, {@link
 * soot.jimple.toolkits.callgraph.OnFlyCallGraphBuilder.TypeBasedReflectionModel} can handle such
 * cases and extend the call graph for edges to the specific java.lang.String method invoked by the
 * reflective call.
 *
          ...
          if (u.containsInvokeExpr()) {
            ...
            if (invokeExpr.getMethod().getSignature().equals(INVOKE_SIG)) {
              if (invokeExpr.getArg(0) instanceof StringConstant) {
                ...
                Local newLocal =
                    Jimple.v().newLocal("sc" + body.getLocalCount(), constant.getType());
