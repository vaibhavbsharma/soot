...
public enum NewValidator implements BodyValidator {
  ...
  /**
   * Checks whether all pathes from start to the end of the method have a call to the &lt;init&gt;
   * method in between. <code>
   * $r0 = new X;<br>
   * ...<br>
        ...
        if (!creatingAlias) {
          for (ValueBox box : curStmt.getUseBoxes()) {
            ...
            if (aliasingLocals.contains(used)) {
              ...
              exception.add(
                  new ValidationException(newStmt, String.format(errorMsg, newStmt, curStmt)));
              return false;
            }
          }
        }
