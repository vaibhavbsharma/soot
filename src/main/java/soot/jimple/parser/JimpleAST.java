...
public class JimpleAST {
  ...
  public JimpleAST(InputStream aJIS) throws ParserException, LexerException, IOException {
    Parser p =
        new Parser(
            new Lexer(new PushbackReader(new BufferedReader(new InputStreamReader(aJIS)), 1024)));
    mTree = p.parse();
  }
   ...
   * the reference types it contains. The given SootClass instance will be filled to contain a class
   * skeleton: that is no Body instances will be created for the class' methods.
   *
  ...
  /**
   * Returns a body corresponding to the parsed jimple for m. If necessary, applies the
   * BodyExtractorWalker to initialize the bodies map.
   *
