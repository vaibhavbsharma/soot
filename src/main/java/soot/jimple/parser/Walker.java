...
public class Walker extends DepthFirstAdapter {
  ...
  public void outAMethodMember(AMethodMember node) {
    ...
    if (node.getThrowsClause() != null) throwsClause = (List<SootClass>) mProductions.removeLast();
    ...
    else method = Scene.v().makeSootMethod(name, parameterList, type, modifier);
  ...
  public void outAIntegerConstant(AIntegerConstant node) {
    ...
    } else if (s.equals("2147483648")) mProductions.addLast(IntConstant.v(Integer.MIN_VALUE));
  ...
  public void outADynamicInvokeExpr(ADynamicInvokeExpr node) {
    List<Value> bsmArgs;
    if (node.getStaticargs() != null) bsmArgs = (List) mProductions.removeLast();
    else bsmArgs = Collections.emptyList();

    SootMethodRef bsmMethodRef = (SootMethodRef) mProductions.removeLast();

    List<Value> dynArgs;
    if (node.getDynargs() != null) dynArgs = (List) mProductions.removeLast();
    else dynArgs = Collections.emptyList();

    SootMethodRef dynMethodRef = (SootMethodRef) mProductions.removeLast();

    mProductions.addLast(
        Jimple.v().newDynamicInvokeExpr(bsmMethodRef, bsmArgs, dynMethodRef, dynArgs));
  }
  ...
  public void outAMultiNewExpr(AMultiNewExpr node) {
    ...
    while (it.hasNext()) {
      ...
      if (o.getImmediate() != null) sizes.add(0, mProductions.removeLast());
      else break;
    }
  ...
  protected int processModifiers(List l) {
    ...
    while (it.hasNext()) {
      ...
      else if (t instanceof AStrictfpModifier) modifier |= Modifier.STRICTFP;
