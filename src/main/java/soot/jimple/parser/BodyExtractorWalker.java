 ...
 * associated with this walker (see constructor). note: Contrary to the plain "Walker", this walker
 * does not create a SootClass, or interact with the scene. It merely adds method bodies for each of
 * the methods of the SootClass it was initialized with.
 */
...
public class BodyExtractorWalker extends Walker {
  ...
  /**
   * Constructs a walker, and attaches it to the given SootClass, sending bodies to the given
   * methodToParsedBodyMap.
   */
  public BodyExtractorWalker(
      SootClass sc, SootResolver resolver, Map<SootMethod, JimpleBody> methodToParsedBodyMap) {
    super(sc, resolver);
  ...
  public void outAMethodMember(AMethodMember node) {
    ...
    if (sm != null) {
      ...
      G.v()
          .out
          .println(
              "[!!! Couldn't parse !!] " + SootMethod.getSubSignature(name, parameterList, type));
