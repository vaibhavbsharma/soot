...
public class LoadStoreOptimizer extends BodyTransformer {
  ...
  class Instance {
        ...
        while (hasChanged) {
          ...
          while (unitIt.hasNext()) {
            ...
            if (uses.size() < 3) {
              ...
              // Check for stack independance (automatic reordering may be performed
              // by stackIndependent() fcnt)
              {
                Block block;
                switch (uses.size()) {
                  case 0: /*
                  ...
                  case 2:
                    if (PhaseOptions.getBoolean(gOptions, "sll")) {
                      if (!mPass2 || PhaseOptions.getBoolean(gOptions, "sll2")) {
                        ...
                        int result =
                            stackIndependent(unit, firstLoad, block, STORE_LOAD_ELIMINATION);
                        if (result == SUCCESS) {

                          // move the first load just after its defining
                          // store.
                          ...
                          int res =
                              stackIndependent(
                                  unit, secondLoad, block, STORE_LOAD_LOAD_ELIMINATION);
                          if (res == MAKE_DUP) {
                            ...
                            unitIt.remove(); // remove store from
                            // store list
                        ...
                        } else if (result == SPECIAL_SUCCESS
                            || result == HAS_CHANGED
                            || result == SPECIAL_SUCCESS2) {
                          if (!hasChangedFlag) {
                            hasChangedFlag = true;
                            hasChanged = true;
                          }
    ...
    /**
     * Checks if the units occurring between [from, to] consume . stack items not produced by these
     * interval units. (ie if the stack height ever goes negative between from and to, assuming the
    ...
    private int pushStoreToLoad(Unit from, Unit to, Block block) {
      ...
      if (storePred != null) {
        if (((Inst) storePred).getOutCount() == 1) {
          ...
          // if we get here it means we can move all the units in the set pass the units
          // in between [to, from]
    ...
    private int stackIndependent(Unit from, Unit to, Block block, int aContext) {
      if (debug) {
        G.v()
            .out
            .println("Trying: " + from + "/" + to + " in block  " + block.getIndexInMethod() + ":");
        G.v()
            .out
            .println(
                "context:"
      ...
      // note: now stackHeight contains the delta height of the stack after executing the
      // units contained in
        ...
        while (u != to) {
          ...
          if (((Inst) u).getNetCount() == 1) {
            ...
            if (u instanceof LoadInst
                || u instanceof PushInst
                || u instanceof NewInst
                || u instanceof StaticGetInst
                || u instanceof Dup1Inst) {

              // verify that unitToMove is not required by following units (until the
              // 'to' unit)
              ...
              if (debug) {
                G.v()
                    .out
                    .println(
                        "1003:(LoadStoreOptimizer@stackIndependent): found unknown unit w/ getNetCount == 1"
                            + u);
              }
            }
          }
        ...
        if (h < 0) {
          if (debug) {
            G.v()
                .out
                .println(
                    "1006:(LoadStoreOptimizer@stackIndependent): Stack went negative while trying to reorder code.");
          }
          ...
          if (flag == 1) {
            if (current instanceof DupInst) {
              ...
              //                        block.insertAfter(  new BSwapInst(  )
              // ,unitToMove);
            }
          }
          return false;
          ...
          if (!isRequiredByFollowingUnits(unitToMove, to)) {
            if (debug) {
              G.v()
                  .out
                  .println(
                      "10077:(LoadStoreOptimizer@stackIndependent): reordering bytecode move: "
                          + unitToMove
                          + "before: "
                          + current);
            }
      ...
      if (reorderingOccurred) {
        ...
        if (debug) {
          G.v()
              .out
              .println(
                  "1008:(LoadStoreOptimizer@stackIndependent):failled to find a new slot for unit to move");
        }
        return false;
      }
    }
    ...
    /**
     * Replace 1 or 2 units by a third unit in a block. Both units to replace should be in the same
    ...
    /**
     * Performs 2 simple inter-block optimizations in order to keep some variables on the stack
     * between blocks. Both are intended to catch 'if' like constructs where the control flow
     * branches temporarily into two paths that join up at a latter point.
     */
        ...
        while (it.hasNext()) {
          ...
          if (u instanceof LoadInst) {
                  ...
                  if (uses.size() == 1) {
                    if (allSuccesorsOfAreThePredecessorsOf(defBlock, loadBlock)) {
                      if (getDeltaStackHeightFromTo(
                        ...
                        if (debug) {
                          G.v().out.println(defBlock.toString() + loadBlock.toString());
                        }
            ...
            // second optimization
              ...
              if (defBlock0 != loadBlock
                  && defBlock1 != loadBlock
                  && defBlock0 != defBlock1
                  && !(isExceptionHandlerBlock(loadBlock))) {
                if (mLocalUses.getUsesOf(def0).size() == 1
                    && mLocalUses.getUsesOf(def1).size() == 1) {
