    ...
    if (!isZipFile()) {
      try {
        ...
        throw new RuntimeException(
            "Error: Failed to open a InputStream for the file at path '" + file.getPath() + "'.",
            e);
      }
    } else {
      if (zipFile == null) {
        try {
          ...
          silentClose();
          throw new RuntimeException(
              "Error: Failed to open the archive file at path '"
                  + file.getPath()
                  + "' for entry '"
                  + entryName
                  + "'.",
      ...
      try {
        ...
        throw new RuntimeException(
            "Error: Failed to open a InputStream for the entry '"
                + zipEntry.getName()
                + "' of the archive at path '"
                + zipFile.getName()
                + "'.",
            e);
      } finally {
