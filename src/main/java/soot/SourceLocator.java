...
public class SourceLocator {
  ...
  /**
   * Set containing all dex files that were appended to the classpath later on. The classes from
   * these files are not yet loaded and are still missing from dexClassIndex.
   */
  private Set<String> dexClassPathExtensions;
  /**
   * The index that maps classes to the files they are defined in. This is necessary because a dex
   * file can hold multiple classes.
   */
    ...
    for (String originalDir : classPath.split(regex)) {
      try {
        ...
        throw new CompilationDeathException(
            "Couldn't resolve classpath entry " + originalDir + ": " + e);
      }
    }
    return ret;
  }
  ...
  private List<String> getClassesUnder(String aPath, String prefix) {
    ...
    if (cst == ClassSourceType.apk || cst == ClassSourceType.dex) {
      try {
        for (DexFileProvider.DexContainer dex :
            DexFileProvider.v().getDexFromSource(new File(aPath))) {
      ...
      try {
        for (DexFileProvider.DexContainer container :
          ...
          CompilationDeathException
              e) { // There might be cases where there is no dex file within a JAR or
        // ZIP file...
      } catch (IOException e) {
  ...
  /**
   * Returns the output directory given by {@link Options} or a default if not set. Also ensures
  ...
  /**
   * If {@link Options#v()#output_jar()} is set, returns the name of the jar file to which the
   * output will be written. The name of the jar file can be given with the -output-dir option or a
   * default will be used. Also ensures that all directories in the path exist.
   *
  ...
  protected FoundFile lookupInArchive(String archivePath, String fileName) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Error: Failed to retrieve the archive entries list for the archive at path '"
              + archivePath
              + "'.",
          e);
    }
  ...
  /**
   * Gets all files that were added to the classpath later on and that have not yet been processed
   * for the dexClassIndex mapping
   *
