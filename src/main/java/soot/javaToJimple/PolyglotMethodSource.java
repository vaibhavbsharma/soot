...
public class PolyglotMethodSource implements MethodSource {
  ...
  public void addAssertInits(soot.Body body) {
    ...
    HashMap<SootClass, InnerClassInfo> innerMap =
        soot.javaToJimple.InitialResolver.v().getInnerClassInfoMap();
    ...
    soot.SootFieldRef field =
        soot.Scene.v()
            .makeFieldRef(assertStatusClass, fieldName, soot.RefType.v("java.lang.Class"), true);

    soot.Local fieldLocal =
        soot.jimple.Jimple.v().newLocal("$r0", soot.RefType.v("java.lang.Class"));
    ...
    soot.jimple.AssignStmt fieldAssignStmt =
        soot.jimple.Jimple.v().newAssignStmt(fieldLocal, fieldRef);

    body.getUnits().add(fieldAssignStmt);

    // if field not null
    soot.jimple.ConditionExpr cond =
    ...
    soot.SootMethodRef methodToInvoke =
        soot.Scene.v()
            .makeMethodRef(
                assertStatusClass, "class$", paramTypes, soot.RefType.v("java.lang.Class"), true);

    ArrayList params = new ArrayList();
    params.add(soot.jimple.StringConstant.v(paramName));
    soot.jimple.StaticInvokeExpr invoke =
    ...
    // field ref assign
    soot.jimple.AssignStmt fieldRefAssign =
        soot.jimple.Jimple.v().newAssignStmt(fieldRef, invokeLocal);
    ...
    soot.jimple.AssignStmt fieldRefAssign2 =
        soot.jimple.Jimple.v().newAssignStmt(invokeLocal, fieldRef);
    ...
    // virtual invoke
    soot.SootMethodRef vMethodToInvoke =
        Scene.v()
            .makeMethodRef(
    ...
    // if
    soot.jimple.ConditionExpr cond2 =
        soot.jimple.Jimple.v().newNeExpr(boolLocal1, soot.jimple.IntConstant.v(0));
    ...
    // alternative
    soot.jimple.AssignStmt altAssign =
        soot.jimple.Jimple.v().newAssignStmt(boolLocal2, soot.jimple.IntConstant.v(1));
    ...
    soot.jimple.AssignStmt conAssign =
        soot.jimple.Jimple.v().newAssignStmt(boolLocal2, soot.jimple.IntConstant.v(0));
