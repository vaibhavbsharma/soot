...
public class AnonInitBodyBuilder extends JimpleBodyBuilder {
  ...
  public soot.jimple.JimpleBody createBody(soot.SootMethod sootMethod) {
    ...
    boolean hasQualifier =
        ((AnonClassInitMethodSource) body.getMethod().getSource()).hasQualifier();
    ...
    SootMethodRef callMethod =
        Scene.v()
            .makeMethodRef(
                ...
                false);
    if ((!hasQualifier)
        && (needsOuterClassRef(polyglotType))) { // && (needsRef.contains(superClass.getType()))){
      if (isSubType) {
        invokeList.add(0, outerLocal);
      } else {
        invokeList.add(
            0,
            Util.getThisGivenOuter(
                superOuterType, new HashMap(), body, new LocalGenerator(body), outerLocal));
      }
    }
    soot.jimple.InvokeExpr invoke =
        soot.jimple.Jimple.v().newSpecialInvokeExpr(specialThisLocal, callMethod, invokeList);

    soot.jimple.Stmt invokeStmt = soot.jimple.Jimple.v().newInvokeStmt(invoke);
    body.getUnits().add(invokeStmt);

    // System.out.println("polyglotType: "+polyglotType+" needs ref:
    // "+needsOuterClassRef(polyglotType));

    // field assign
    if (!inStaticMethod && needsOuterClassRef(anonType)) {
      soot.SootFieldRef field =
          Scene.v().makeFieldRef(sootMethod.getDeclaringClass(), "this$0", outerClassType, false);
      soot.jimple.InstanceFieldRef ref =
    ...
    if (fields != null) {
      ...
      while (finalsIt.hasNext() && fieldsIt.hasNext()) {
        ...
        soot.jimple.FieldRef pRef =
            soot.jimple.Jimple.v().newInstanceFieldRef(specialThisLocal, pField.makeRef());
    ...
    ArrayList<Block> staticBlocks =
        ((AnonClassInitMethodSource) body.getMethod().getSource()).getInitializerBlocks();
