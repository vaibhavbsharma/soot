...
public class JavaToJimple {
  ...
  public static final polyglot.frontend.Pass.ID CAST_INSERTION =
      new polyglot.frontend.Pass.ID("cast-insertion");
  public static final polyglot.frontend.Pass.ID STRICTFP_PROP =
      new polyglot.frontend.Pass.ID("strictfp-prop");
  public static final polyglot.frontend.Pass.ID ANON_CONSTR_FINDER =
      new polyglot.frontend.Pass.ID("anon-constr-finder");
    ...
    ExtensionInfo extInfo =
        new soot.javaToJimple.jj.ExtensionInfo() {
          ...
          public List passes(Job job) {
            ...
            // VisitorPass(polyglot.frontend.Pass.FOLD, job, new
            // polyglot.visit.ConstantFolder(ts, nf)));
            beforePass(
                passes,
                Pass.EXIT_CHECK,
                new VisitorPass(CAST_INSERTION, job, new CastInsertionVisitor(job, ts, nf)));
            beforePass(
                passes,
                Pass.EXIT_CHECK,
                new VisitorPass(STRICTFP_PROP, job, new StrictFPPropagator(false)));
            beforePass(
                passes,
                Pass.EXIT_CHECK,
    ...
    try {
      ...
      if (compiler.sourceExtension() instanceof soot.javaToJimple.jj.ExtensionInfo) {
        soot.javaToJimple.jj.ExtensionInfo jjInfo =
            (soot.javaToJimple.jj.ExtensionInfo) compiler.sourceExtension();
