...
public class JimpleBodyBuilder extends AbstractJimpleBodyBuilder {
  ...
  public soot.jimple.JimpleBody createJimpleBody(
    ...
    if ((outerIndex != -1) && (sootMethod.getName().equals("<init>"))) {
      ...
      if (this0Field != null) {
        ...
        soot.jimple.ParameterRef paramRef =
            soot.jimple.Jimple.v().newParameterRef(outerClass.getType(), formalsCounter);
        paramRefCount++;
        ...
        ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource())
            .setOuterClassThisInit(outerLocal);
    ...
    if (finalsList != null) {
      ...
      while (finalsIt.hasNext()) {
        ...
        soot.jimple.ParameterRef paramRef =
            soot.jimple.Jimple.v().newParameterRef(sf.getType(), formalsCounter);
        paramRefCount++;
        soot.jimple.Stmt stmt =
            soot.jimple.Jimple.v().newIdentityStmt(lg.generateLocal(sf.getType()), paramRef);
        body.getUnits().add(stmt);
        formalsCounter++;
      }
    }
  ...
  private void handleFieldInits(soot.SootMethod sootMethod) {
    ...
    ArrayList<FieldDecl> fieldInits =
        ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource()).getFieldInits();
    if (fieldInits != null) {
  ...
  protected void handleFieldInits(ArrayList<FieldDecl> fieldInits) {
    ...
    while (fieldInitsIt.hasNext()) {
      ...
      soot.SootFieldRef sootField =
          soot.Scene.v()
              .makeFieldRef(
                  currentClass,
                  fieldName,
                  Util.getSootType(field.type().type()),
  ...
  private void handleOuterClassThisInit(soot.SootMethod sootMethod) {
    ...
    if (this0Field != null) {
      soot.jimple.FieldRef fieldRef =
          soot.jimple.Jimple.v().newInstanceFieldRef(specialThisLocal, this0Field.makeRef());
      soot.jimple.AssignStmt stmt =
          soot.jimple.Jimple.v().newAssignStmt(fieldRef, outerClassParamLocal);
      body.getUnits().add(stmt);
    }
  }
  ...
  private soot.Local getLocal(polyglot.types.LocalInstance li) {
    ...
    if (localsMap.containsKey(new polyglot.util.IdentityKey(li))) {
      ...
      return sootLocal;
    } else if (body.getMethod()
        .getDeclaringClass()
        .declaresField("val$" + li.name(), Util.getSootType(li.type()))) {
      ...
      soot.SootFieldRef field =
          soot.Scene.v()
              .makeFieldRef(
                  ...
                  false);
      soot.jimple.FieldRef fieldRef =
          soot.jimple.Jimple.v().newInstanceFieldRef(specialThisLocal, field);
     ...
     */
      ...
      while (!fieldFound) {
        if (!currentClass.declaresFieldByName("this$0")) {
          throw new RuntimeException(
              "Trying to get field val$"
                  + li.name()
                  + " from some outer class but can't access the outer class of: "
                  + currentClass.getName()
                  + "!"
                  + " current class contains fields: "
                  + currentClass.getFields());
        }
        soot.SootClass outerClass =
            ((soot.RefType) currentClass.getFieldByName("this$0").getType()).getSootClass();
      ...
      soot.Local res =
          Util.getPrivateAccessFieldInvoke(methToInvoke.makeRef(), methParams, body, lg);
      return res;
    }
  }

  private soot.SootMethod makeLiFieldAccessMethod(
      soot.SootClass classToInvoke, polyglot.types.LocalInstance li) {
    String name =
        "access$" + soot.javaToJimple.InitialResolver.v().getNextPrivateAccessCounter() + "00";
    ArrayList paramTypes = new ArrayList();
    paramTypes.add(classToInvoke.getType());

    soot.SootMethod meth =
        Scene.v()
            .makeSootMethod(name, paramTypes, Util.getSootType(li.type()), soot.Modifier.STATIC);

    classToInvoke.addMethod(meth);
    PrivateFieldAccMethodSource src =
        new PrivateFieldAccMethodSource(
            Util.getSootType(li.type()), "val$" + li.name(), false, classToInvoke);
  ...
  /**
   * If Stmts Creation - only add line-number tags to if (the other stmts needing tags are created
   * elsewhere
   */
  /*
  ...
  private void createLocalDecl(polyglot.ast.LocalDecl localDecl) {
    ...
    if (expr != null) {
      ...
      if (localDecl.position() != null) {
        Util.addLnPosTags(
            stmt.getLeftOpBox(),
            localDecl.position().line(),
            localDecl.position().endLine(),
            localDecl.position().endColumn() - name.length(),
            localDecl.position().endColumn());
        if (expr != null) {
          Util.addLnPosTags(
              stmt,
              localDecl.position().line(),
              expr.position().endLine(),
              localDecl.position().column(),
  ...
  private void createSwitch(polyglot.ast.Switch switchStmt) {
    ...
    if (isLookupSwitch(switchStmt)) {
      ...
      soot.jimple.TableSwitchStmt tableStmt =
          soot.jimple.Jimple.v()
              .newTableSwitchStmt(
                  sootValue, (int) lowVal, (int) highVal, sortedTargets, defaultTarget);
  ...
  /**
   * Determine if switch should be lookup or table - this doesn't always get the same result as
   * javac lookup: non-table table: sequential (no gaps)
   */
  ...
  private void createBranch(polyglot.ast.Branch branchStmt) {
    ...
    if (branchStmt.kind() == polyglot.ast.Branch.BREAK) {
      if (branchStmt.label() == null) {
        ...
        soot.jimple.Stmt gotoLabel =
            soot.jimple.Jimple.v().newGotoStmt(labelBreakMap.get(branchStmt.label()));
      ...
      if (branchStmt.label() == null) {
        ...
        soot.jimple.Stmt gotoLabel =
            soot.jimple.Jimple.v().newGotoStmt(labelContinueMap.get(branchStmt.label()));
        body.getUnits().add(gotoLabel);
        Util.addLnPosTags(gotoLabel, branchStmt.position());
      }
    }
  ...
  private void createAssert(polyglot.ast.Assert assertStmt) {
    ...
    soot.jimple.AssignStmt fieldAssign =
        soot.jimple.Jimple.v().newAssignStmt(testLocal, assertFieldRef);
    body.getUnits().add(fieldAssign);

    soot.jimple.NopStmt nop1 = soot.jimple.Jimple.v().newNopStmt();
    soot.jimple.ConditionExpr cond1 =
        soot.jimple.Jimple.v().newNeExpr(testLocal, soot.jimple.IntConstant.v(0));
    ...
    soot.jimple.NewExpr newExpr =
        soot.jimple.Jimple.v().newNewExpr(soot.RefType.v("java.lang.AssertionError"));
    ...
    methToInvoke =
        soot.Scene.v()
            .makeMethodRef(
                soot.Scene.v().getSootClass("java.lang.AssertionError"),
                "<init>",
                paramTypes,
                soot.VoidType.v(),
                false);
  ...
  private void createTryCatchFinally(polyglot.ast.Try tryStmt) {
    ...
    soot.jimple.Stmt catchAllAssign =
        soot.jimple.Jimple.v().newAssignStmt(catchAllAssignLocal, formalLocal);
    ...
    addToExceptionList(
        beforeCatchAllAssignNoop,
        catchAllBeforeThrowNoop,
        catchAllBeforeNoop,
        soot.Scene.v().getSootClass("java.lang.Throwable"));
  ...
  private void addToExceptionList(
      soot.jimple.Stmt from,
      soot.jimple.Stmt to,
  ...
  protected soot.Value createAggressiveExpr(
    ...
    if (expr.isConstant()
        ...
        && expr.type() != null
        && !(expr instanceof polyglot.ast.Binary
            && expr.type().toString().equals("java.lang.String"))) {
  ...
  protected soot.Local handlePrivateFieldUnarySet(polyglot.ast.Unary unary) {
    ...
    if (unary.operator() == polyglot.ast.Unary.PRE_INC
        || unary.operator() == polyglot.ast.Unary.POST_INC) {
    ...
    if (unary.operator() == polyglot.ast.Unary.PRE_INC
        || unary.operator() == polyglot.ast.Unary.PRE_DEC) {
  ...
  protected soot.Local handlePrivateFieldSet(
    ...
    soot.SootClass containClass =
        ((soot.RefType) Util.getSootType(fLeft.target().type())).getSootClass();
    ...
    soot.jimple.InvokeExpr invoke =
        soot.jimple.Jimple.v().newStaticInvokeExpr(methToUse.makeRef(), params);
  ...
  private soot.SootMethod addSetAccessMeth(
      soot.SootClass conClass, polyglot.ast.Field field, soot.Value param) {
     ...
     */
    soot.SootMethod meth =
        Scene.v().makeSootMethod(name, paramTypes, retType, soot.Modifier.STATIC);
    PrivateFieldSetMethodSource pfsms =
        new PrivateFieldSetMethodSource(
            Util.getSootType(field.type()), field.name(), field.flags().isStatic());
  ...
  private soot.SootMethod addGetFieldAccessMeth(soot.SootClass conClass, polyglot.ast.Field field) {
    ...
    soot.SootMethod meth =
        Scene.v()
            .makeSootMethod(name, paramTypes, Util.getSootType(field.type()), soot.Modifier.STATIC);
    PrivateFieldAccMethodSource pfams =
        new PrivateFieldAccMethodSource(
            Util.getSootType(field.type()), field.name(), field.flags().isStatic(), conClass);
    ...
    soot.SootMethod meth =
        Scene.v()
            .makeSootMethod(
                name,
                paramTypes,
                Util.getSootType(call.methodInstance().returnType()),
  ...
  private soot.Value getFieldLocal(polyglot.ast.Field field) {
    ...
    polyglot.ast.Receiver receiver = field.target();
    soot.javaToJimple.PolyglotMethodSource ms =
        (soot.javaToJimple.PolyglotMethodSource) body.getMethod().getSource();
    ...
    if ((field.target() instanceof polyglot.ast.Special)
      ...
      soot.jimple.AssignStmt fieldAssignStmt =
          soot.jimple.Jimple.v().newAssignStmt(baseLocal, fieldRef);
  ...
  /**
   * needs accessors: when field or meth is private and in some other class when field or meth is
   * protected and in
   */
  protected boolean needsAccessor(polyglot.types.MemberInstance inst) {
    if (inst.flags().isPrivate()) {
      if (!Util.getSootType(inst.container())
          .equals(body.getMethod().getDeclaringClass().getType())) {
        return true;
      }
      ...
      while (currentClass.hasOuterClass()) {
        ...
        if (Util.getSootType(inst.container()).equals(currentClass.getType())) {
          return false;
        } else if (Util.getSootType(inst.container())
            .equals(currentClass.getSuperclass().getType())) {
          return true;
        }
      }
      return false;
  ...
  /**
   * needs a protected access method if field is protected and in a super class of the outer class
   * of the innerclass trying to access the field (ie not in self or in outer of self)
   */
  /*
  ...
  private soot.jimple.Constant getConstant(Object constVal, polyglot.types.Type type) {
    ...
    if (constVal instanceof String) {
      ...
      if (num instanceof Long) {
        ...
        return soot.jimple.LongConstant.v(num.longValue());
      } else if (num instanceof Double) {
        return soot.jimple.DoubleConstant.v(num.doubleValue());
      } else if (num instanceof Float) {
        return soot.jimple.FloatConstant.v(num.floatValue());
      } else if (num instanceof Byte) {
        return soot.jimple.IntConstant.v(num.byteValue());
      } else if (num instanceof Short) {
        return soot.jimple.IntConstant.v(num.shortValue());
      } else {
        return soot.jimple.IntConstant.v(num.intValue());
      }
    }
  }
  ...
  protected soot.jimple.FieldRef getFieldRef(polyglot.ast.Field field) {
    ...
    soot.SootClass receiverClass =
        ((soot.RefType) Util.getSootType(field.target().type())).getSootClass();
    ...
    if (field.target() instanceof polyglot.ast.Local
      ...
      Util.addLnPosTags(
          ((soot.jimple.InstanceFieldRef) fieldRef).getBaseBox(), field.target().position());
    }
    return fieldRef;
  }
    ...
    if (field.fieldInstance().flags().isPrivate()) {
      toInvoke =
          addGetFieldAccessMeth(
              ((soot.RefType) Util.getSootType(field.fieldInstance().container())).getSootClass(),
              field);
      invokeClass =
          ((soot.RefType) Util.getSootType(field.fieldInstance().container())).getSootClass();
    } else {
      ...
      if (body.getMethod().getDeclaringClass().hasOuterClass()) {
        ...
        while (!InitialResolver.v()
            .hierarchy()
            .canStoreType(containingClass.getType(), addToClass.getType())) {
    ...
    if (lit.typeNode().type().isPrimitive()) {
      ...
      if (primType.isBoolean()) {
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Boolean"),
                    "TYPE",
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Byte"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Character"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Double"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Float"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Integer"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Long"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Short"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
        ...
        primField =
            soot.Scene.v()
                .makeFieldRef(
                    soot.Scene.v().getSootClass("java.lang.Void"),
                    "TYPE",
                    soot.RefType.v("java.lang.Class"),
                    true);
      }
      ...
      soot.jimple.Stmt noop1 = soot.jimple.Jimple.v().newNopStmt();
      soot.jimple.Expr neExpr =
          soot.jimple.Jimple.v().newNeExpr(fieldLocal, soot.jimple.NullConstant.v());
      ...
      if (thisClass.isInterface()) {
        ...
        if ((specialAnonMap != null) && (specialAnonMap.containsKey(thisClass))) {
          ...
          invokeMeth =
              soot.Scene.v()
                  .makeMethodRef(
                      specialClass, "class$", paramTypes, soot.RefType.v("java.lang.Class"), true);
        } else {
          throw new RuntimeException(
              "Class is interface so it must have an anon class to handle class lits but its anon class cannot be found.");
        }
        ...
        invokeMeth =
            soot.Scene.v()
                .makeMethodRef(
                    thisClass, "class$", paramTypes, soot.RefType.v("java.lang.Class"), true);
      }
      ArrayList params = new ArrayList();
  ...
  private soot.Local getSpecialArrayLengthLocal(polyglot.ast.Field field) {
    ...
    if (receiver instanceof polyglot.ast.Local) {
      ...
      localField =
          (soot.Local) base().createAggressiveExpr((polyglot.ast.Expr) receiver, false, false);
    } else {
  ...
  private boolean isComparisonBinary(polyglot.ast.Binary.Operator op) {
    return (op == polyglot.ast.Binary.EQ)
        || (op == polyglot.ast.Binary.NE)
        || (op == polyglot.ast.Binary.GE)
        || (op == polyglot.ast.Binary.GT)
        || (op == polyglot.ast.Binary.LE)
        || (op == polyglot.ast.Binary.LT);
  }

  /** Creates a binary expression that is not a comparison */
  private soot.Value getBinaryExpr(
      soot.Value lVal, soot.Value rVal, polyglot.ast.Binary.Operator operator) {
  ...
  private soot.Value getBinaryComparisonExpr(
      soot.Value lVal, soot.Value rVal, polyglot.ast.Binary.Operator operator) {
  ...
  private soot.Value handleDFLCond(soot.jimple.ConditionExpr cond) {
    ...
    if (isDouble(cond.getOp1())
        || isDouble(cond.getOp2())
        || isFloat(cond.getOp1())
        || isFloat(cond.getOp2())) {
      // use cmpg and cmpl
  ...
  private boolean isDouble(soot.Value val) {
    return val.getType() instanceof soot.DoubleType;
  }

  private boolean isFloat(soot.Value val) {
    return val.getType() instanceof soot.FloatType;
  }

  private boolean isLong(soot.Value val) {
    return val.getType() instanceof soot.LongType;
  }
  ...
  private soot.Value createCondAnd(polyglot.ast.Binary binary) {
    ...
    soot.jimple.Stmt assign1 =
        soot.jimple.Jimple.v().newAssignStmt(retLocal, soot.jimple.IntConstant.v(1));
    ...
    soot.jimple.Stmt assign2 =
        soot.jimple.Jimple.v().newAssignStmt(retLocal, soot.jimple.IntConstant.v(0));
  ...
  private soot.Local handleCondBinExpr(soot.jimple.ConditionExpr condExpr) {
    ...
    body.getUnits()
        .add(soot.jimple.Jimple.v().newAssignStmt(boolLocal, soot.jimple.IntConstant.v(1)));
    ...
    body.getUnits()
        .add(soot.jimple.Jimple.v().newAssignStmt(boolLocal, soot.jimple.IntConstant.v(0)));
  ...
  private soot.Local createStringBuffer(polyglot.ast.Expr expr) {
    ...
    soot.jimple.NewExpr newExpr =
        soot.jimple.Jimple.v().newNewExpr(soot.RefType.v("java.lang.StringBuffer"));
    ...
    soot.SootMethodRef methodToInvoke1 =
        soot.Scene.v()
            .makeMethodRef(classToInvoke1, "<init>", new ArrayList(), soot.VoidType.v(), false);

    soot.jimple.SpecialInvokeExpr invoke =
        soot.jimple.Jimple.v().newSpecialInvokeExpr(local, methodToInvoke1);
  ...
  private soot.Local createToString(soot.Local sb, polyglot.ast.Expr expr) {
    ...
    soot.SootMethodRef methodToInvoke2 =
        soot.Scene.v()
            .makeMethodRef(
                classToInvoke2,
                "toString",
                new ArrayList(),
                soot.RefType.v("java.lang.String"),
                false);

    soot.jimple.VirtualInvokeExpr toStringInvoke =
        soot.jimple.Jimple.v().newVirtualInvokeExpr(sb, methodToInvoke2);
      ...
      soot.SootMethodRef methodToInvoke =
          soot.Scene.v()
              .makeMethodRef(
                  ...
                  "append",
                  paramsTypes,
                  soot.RefType.v("java.lang.StringBuffer"),
                  false);
  ...
  private soot.Value getUnaryLocal(polyglot.ast.Unary unary) {
    ...
    if (op == polyglot.ast.Unary.POST_INC
        || op == polyglot.ast.Unary.PRE_INC
        || op == polyglot.ast.Unary.POST_DEC
      ...
      if (unary.operator() == polyglot.ast.Unary.POST_DEC
          || unary.operator() == polyglot.ast.Unary.POST_INC) {
        return tmp;
      } else {
        return tmp2;
     ...
     */
      ...
      soot.jimple.XorExpr xor =
          soot.jimple.Jimple.v().newXorExpr(sootExpr, base().getConstant(sootExpr.getType(), -1));
      ...
      soot.jimple.NeExpr neExpr =
          soot.jimple.Jimple.v().newNeExpr(local, base().getConstant(local.getType(), 0));
      ...
      soot.jimple.Stmt assign1 =
          soot.jimple.Jimple.v().newAssignStmt(retLocal, base().getConstant(retLocal.getType(), 1));
      ...
      soot.jimple.Stmt assign2 =
          soot.jimple.Jimple.v().newAssignStmt(retLocal, base().getConstant(retLocal.getType(), 0));
  ...
  private soot.SootMethodRef getMethodFromClass(
      soot.SootClass sootClass,
      String name,
      ArrayList paramTypes,
      soot.Type returnType,
      boolean isStatic) {
    soot.SootMethodRef ref =
        soot.Scene.v().makeMethodRef(sootClass, name, paramTypes, returnType, isStatic);
    return ref;
  }

  /** Adds extra params */
  private void handleFinalLocalParams(
      ArrayList sootParams, ArrayList sootParamTypes, polyglot.types.ClassType keyType) {

    HashMap<IdentityKey, AnonLocalClassInfo> finalLocalInfo =
        soot.javaToJimple.InitialResolver.v().finalLocalInfo();
        ...
        if (finalLocals != null) {
          ...
          while (it.hasNext()) {
            ...
            polyglot.types.LocalInstance li =
                (polyglot.types.LocalInstance) ((polyglot.util.IdentityKey) next).object();
  ...
  protected boolean needsOuterClassRef(polyglot.types.ClassType typeToInvoke) {
    ...
    // other nested
    else if (typeToInvoke.isNested()
        && !typeToInvoke.flags().isStatic()
        && !typeToInvoke.isAnonymous()
  ...
  private void handleOuterClassParams(
      ArrayList sootParams,
      soot.Value qVal,
      ArrayList sootParamsTypes,
    ...
    if (addRef) {
      ...
      soot.SootClass outerClass =
          ((soot.RefType) Util.getSootType(typeToInvoke.outer())).getSootClass();
  ...
  private void createConstructorCall(polyglot.ast.ConstructorCall cCall) {
    ...
    while (invokeParamsIt.hasNext()) {
      Util.addLnPosTags(
          specialInvokeExpr.getArgBox(numParams),
    ...
    if (body.getMethod().getName().equals("<init>")
        && (cCall.kind() == polyglot.ast.ConstructorCall.SUPER)) {
  ...
  private void handleFinalLocalInits() {
    ...
    while (it.hasNext()) {
      ...
      soot.jimple.FieldRef fieldRef =
          soot.jimple.Jimple.v().newInstanceFieldRef(specialThisLocal, sf.makeRef());
      soot.jimple.AssignStmt stmt =
          soot.jimple.Jimple.v().newAssignStmt(fieldRef, body.getParameterLocal(paramCount));
      body.getUnits().add(stmt);
      paramCount++;
    }
  }
  ...
  protected soot.SootMethodRef getSootMethodRef(polyglot.ast.Call call) {
    ...
    if (Util.getSootType(call.methodInstance().container())
  ...
  private soot.Local getCallLocal(polyglot.ast.Call call) {
    ...
    if (needsAccessor(call)) {
      ...
      soot.SootClass containingClass =
          ((soot.RefType) Util.getSootType(call.methodInstance().container())).getSootClass();
      ...
      if (!call.methodInstance().flags().isStatic()) {
        ...
        if (call.target() instanceof polyglot.ast.Expr) {
          ...
          sootParams.add(
              0, getThis(Util.getSootType(call.methodInstance().container()))); // baseLocal);
        } else {
          sootParams.add(0, baseLocal);
        }
      }
    ...
    while (callParamsIt.hasNext()) {
      Util.addLnPosTags(
          invokeExpr.getArgBox(numParams), ((polyglot.ast.Expr) callParamsIt.next()).position());
      numParams++;
    }

    if (invokeExpr instanceof soot.jimple.InstanceInvokeExpr) {
      Util.addLnPosTags(
          ((soot.jimple.InstanceInvokeExpr) invokeExpr).getBaseBox(), call.target().position());
    }
  ...
  private soot.Local getNewArrayLocal(polyglot.ast.NewArray newArrExpr) {
    ...
    if (newArrExpr.numDims() == 1) {
      ...
      if (newArrExpr.additionalDims() == 1) {
        ...
        dimLocal =
            base().createAggressiveExpr((polyglot.ast.Expr) newArrExpr.dims().get(0), false, false);
      }
      ...
      if (newArrExpr.additionalDims() != 1) {
        Util.addLnPosTags(
            newArrayExpr.getSizeBox(), ((polyglot.ast.Expr) newArrExpr.dims().get(0)).position());
      }
    } else {
      ...
      while (sizeBoxIt.hasNext()) {
        Util.addLnPosTags(
            newMultiArrayExpr.getSizeBox(counter),
  ...
  private soot.Local getArrayInitLocal(
      polyglot.ast.ArrayInit arrInit, polyglot.types.Type lhsType) {
    ...
    soot.jimple.NewArrayExpr arrExpr =
        soot.jimple.Jimple.v()
            .newNewArrayExpr(
                ((soot.ArrayType) local.getType()).getElementType(),
    ...
    while (it.hasNext()) {
      ...
      if (elemExpr instanceof polyglot.ast.ArrayInit) {
        ...
        if (elemExpr.type() instanceof polyglot.types.NullType) {
          if (lhsType instanceof polyglot.types.ArrayType) {
            ...
            elem =
                getArrayInitLocal(
                    (polyglot.ast.ArrayInit) elemExpr, ((polyglot.types.ArrayType) lhsType).base());
          } else {
      ...
      soot.jimple.ArrayRef arrRef =
          soot.jimple.Jimple.v().newArrayRef(local, soot.jimple.IntConstant.v(index));
  ...
  private soot.Local getSpecialSuperQualifierLocal(polyglot.ast.Expr expr) {
    ...
    if (expr instanceof polyglot.ast.Call) {
      ...
      throw new RuntimeException(
          "Trying to create special super qualifier for: "
              + expr
              + " which is not a field or call");
    }
    ...
    soot.jimple.InvokeExpr invokeExpr =
        soot.jimple.Jimple.v().newStaticInvokeExpr(methToInvoke.makeRef(), methodParams);
  ...
  private soot.SootMethod makeSuperAccessMethod(
      soot.SootClass classToInvoke, Object memberToAccess) {
    String name =
        "access$" + soot.javaToJimple.InitialResolver.v().getNextPrivateAccessCounter() + "00";
    ArrayList paramTypes = new ArrayList();
    paramTypes.add(classToInvoke.getType());
    ...
    if (memberToAccess instanceof polyglot.ast.Field) {
      ...
      meth =
          Scene.v()
              .makeSootMethod(
                  name, paramTypes, Util.getSootType(fieldToAccess.type()), soot.Modifier.STATIC);
      PrivateFieldAccMethodSource fSrc =
          new PrivateFieldAccMethodSource(
      ...
      meth =
          Scene.v()
              .makeSootMethod(
                  name,
                  paramTypes,
                  Util.getSootType(methToAccess.methodInstance().returnType()),
                  soot.Modifier.STATIC);
      PrivateMethodAccMethodSource mSrc =
          new PrivateMethodAccMethodSource(methToAccess.methodInstance());
      src = mSrc;
    } else {
  ...
  private soot.Local getInstanceOfLocal(polyglot.ast.Instanceof instExpr) {
    ...
    soot.jimple.InstanceOfExpr instOfExpr =
        soot.jimple.Jimple.v().newInstanceOfExpr(local, sootType);
  ...
  private soot.Local getConditionalLocal(polyglot.ast.Conditional condExpr) {
    ...
    soot.jimple.AssignStmt conseqAssignStmt =
        soot.jimple.Jimple.v().newAssignStmt(retLocal, conseqVal);
