...
public class ClassResolver {
  ...
  private void createClassDecl(polyglot.ast.ClassDecl cDecl) {
    ...
    if (!cDecl.type().isTopLevel()) {
      ...
      InitialResolver.v()
          ...
          .put(sootClass, new InnerClassInfo(outerClass, cDecl.name(), InnerClassInfo.NESTED));
    ...
    if (cDecl.superClass() == null) {
      ...
      sootClass.setSuperclass(
          ((soot.RefType) Util.getSootType(cDecl.superClass().type())).getSootClass());
    ...
    if ((staticFieldInits != null) || (staticInitializerBlocks != null)) {
      ...
      ((PolyglotMethodSource) clinitMethod.getSource())
          .setStaticInitializerBlocks(staticInitializerBlocks);
    }

    // add final locals to local inner classes inits
    if (cDecl.type().isLocal()) {
      AnonLocalClassInfo info =
          InitialResolver.v().finalLocalInfo().get(new polyglot.util.IdentityKey(cDecl.type()));
      ArrayList<SootField> finalsList =
          addFinalLocals(cDecl.body(), info.finalLocalsAvail(), cDecl.type(), info);
  ...
  private void createClassBody(polyglot.ast.ClassBody classBody) {
    ...
    while (it.hasNext()) {
      ...
      if (next instanceof polyglot.ast.MethodDecl) {
        ...
        G.v()
            .out
            .println("Class Body Member not implemented for type " + next.getClass().getName());
      }
    }
  ...
  private void addOuterClassThisRefField(polyglot.types.Type outerType) {
    ...
    soot.SootField field =
        Scene.v()
            .makeSootField("this$0", outerSootType, soot.Modifier.PRIVATE | soot.Modifier.FINAL);
  ...
  private ArrayList<SootField> addFinalLocals(
      polyglot.ast.ClassBody cBody,
      ArrayList<IdentityKey> finalLocalsAvail,
      polyglot.types.ClassType nodeKeyType,
      AnonLocalClassInfo info) {
    ...
    while (fieldsNeededIt.hasNext()) {
      ...
      polyglot.types.LocalInstance li =
          (polyglot.types.LocalInstance) fieldsNeededIt.next().object();
    ...
    while (newsIt.hasNext()) {
      ...
      if (InitialResolver.v()
          .finalLocalInfo()
          .containsKey(new polyglot.util.IdentityKey(tempNewType))) {
        AnonLocalClassInfo lInfo =
            InitialResolver.v().finalLocalInfo().get(new polyglot.util.IdentityKey(tempNewType));
    ...
    while (!Util.getSootType(superType)
        .equals(soot.Scene.v().getSootClass("java.lang.Object").getType())) {
      if (InitialResolver.v()
          .finalLocalInfo()
          .containsKey(new polyglot.util.IdentityKey(superType))) {
        AnonLocalClassInfo lInfo =
            InitialResolver.v().finalLocalInfo().get(new polyglot.util.IdentityKey(superType));
        Iterator<IdentityKey> it = lInfo.finalLocalsAvail().iterator();
        while (it.hasNext()) {
          polyglot.types.LocalInstance li2 = (polyglot.types.LocalInstance) it.next().object();
          if (!sootClass.declaresField("val$" + li2.name(), Util.getSootType(li2.type()))) {
  ...
  /**
   * creates the Jimple for an anon class - in the AST there is no class decl for anon classes - the
   * revelant fields and methods are created
   */
  private void createAnonClassDecl(polyglot.ast.New aNew) {

    SootClass outerClass =
        ((soot.RefType) Util.getSootType(aNew.anonType().outer())).getSootClass();
    if (InitialResolver.v().getInnerClassInfoMap() == null) {
      InitialResolver.v().setInnerClassInfoMap(new HashMap<SootClass, InnerClassInfo>());
    }
    InitialResolver.v()
        .getInnerClassInfoMap()
        .put(sootClass, new InnerClassInfo(outerClass, "0", InnerClassInfo.ANON));
    sootClass.setOuterClass(outerClass);

    soot.SootClass typeClass =
        ((soot.RefType) Util.getSootType(aNew.objectType().type())).getSootClass();
    ...
    AnonLocalClassInfo info =
        InitialResolver.v().finalLocalInfo().get(new polyglot.util.IdentityKey(aNew.anonType()));
    ...
    if (info != null) {
      src.setFinalsList(
          addFinalLocals(aNew.body(), info.finalLocalsAvail(), aNew.anonType(), info));
    }
    src.outerClassType(Util.getSootType(aNew.anonType().outer()));
    if (((polyglot.types.ClassType) aNew.objectType().type()).isNested()) {
      src.superOuterType(
          Util.getSootType(((polyglot.types.ClassType) aNew.objectType().type()).outer()));
      src.isSubType(
          Util.isSubType(
              aNew.anonType().outer(),
    ...
    Util.addLnPosTags(
        sootClass,
        aNew.position().line(),
        aNew.body().position().endLine(),
        aNew.position().column(),
  ...
  private soot.SootClass getSpecialInterfaceAnonClass(soot.SootClass addToClass) {
    ...
    if ((InitialResolver.v().specialAnonMap() != null)
      ...
      Util.addInnerClassTag(
          addToClass, specialClass.getName(), addToClass.getName(), null, soot.Modifier.STATIC);
      Util.addInnerClassTag(
          specialClass, specialClass.getName(), addToClass.getName(), null, soot.Modifier.STATIC);
  ...
  private void handleAssert(polyglot.ast.ClassBody cBody) {
    ...
    if (!addToClass.declaresField(fieldName, fieldType)) {
      soot.SootField classField =
          Scene.v().makeSootField(fieldName, fieldType, soot.Modifier.STATIC);
    ...
    soot.SootMethod sootMethod =
        Scene.v().makeSootMethod(methodName, paramTypes, methodRetType, soot.Modifier.STATIC);
  ...
  private void createConstructorDecl(polyglot.ast.ConstructorDecl constructor) {
    ...
    soot.SootMethod sootMethod =
        createSootConstructor(name, constructor.flags(), parameters, exceptions);
  ...
  private void createMethodDecl(polyglot.ast.MethodDecl method) {
    ...
    soot.SootMethod sootMethod =
        createSootMethod(name, method.flags(), method.returnType().type(), parameters, exceptions);
    ...
    if ((fieldInits != null) || (initializerBlocks != null)) {
      ...
      while (methodsIt.hasNext()) {
        ...
        if (next.getName().equals("<init>")) {
          ...
          soot.javaToJimple.PolyglotMethodSource src =
              (soot.javaToJimple.PolyglotMethodSource) next.getSource();
    ...
    if (!found) {
      ...
      if (!found) {
        ...
        if ((InitialResolver.v().getAnonClassMap() != null)
          ...
          polyglot.ast.New aNew =
              (polyglot.ast.New) InitialResolver.v().getAnonClassMap().getKey(simpleName);
          if (aNew == null) throw new RuntimeException("Could resolve class: " + simpleName);
  ...
  private void addConstValTag(polyglot.ast.FieldDecl field, soot.SootField sootField) {
    ...
    if (field.fieldInstance().constantValue() instanceof Integer) {
      sootField.addTag(
          new soot.tagkit.IntegerConstantValueTag(
      ...
      sootField.addTag(
          new soot.tagkit.LongConstantValueTag(
              ((Long) field.fieldInstance().constantValue()).longValue()));
    } else if (field.fieldInstance().constantValue() instanceof Double) {
      // System.out.println("const val:
      // "+field.fieldInstance().constantValue());
      sootField.addTag(
          new soot.tagkit.DoubleConstantValueTag(
      ...
      sootField.addTag(
          new soot.tagkit.FloatConstantValueTag(
              ((Float) field.fieldInstance().constantValue()).floatValue()));
    } else if (field.fieldInstance().constantValue() instanceof String) {
      sootField.addTag(
          new soot.tagkit.StringConstantValueTag((String) field.fieldInstance().constantValue()));
    } else {
      throw new RuntimeException(
          "Expecting static final field to have a constant value! For field: "
              + field
              + " of type: "
  ...
  private soot.SootMethod createSootMethod(
      String name,
    ...
    soot.SootMethod method =
        Scene.v().makeSootMethod(name, parameters, sootReturnType, modifier, exceptions);
    return method;
  }
  ...
  private soot.SootMethod createSootConstructor(
      String name,
      polyglot.types.Flags flags,
      ArrayList parameters,
      ArrayList<SootClass> exceptions) {
    int modifier = Util.getModifier(flags);
    soot.SootMethod method =
        Scene.v().makeSootMethod(name, parameters, soot.VoidType.v(), modifier, exceptions);
    return method;
  }
}
