...
public class InitialResolver implements IInitialResolver {
  ...
  private HashMap<IdentityKey, AnonLocalClassInfo>
  ...
  private void createClassToSourceMap(polyglot.ast.SourceFile src) {
    ...
    if (src.package_() != null) {
      ...
      srcFileName =
          srcName.substring(srcName.lastIndexOf(System.getProperty("file.separator")) + 1);
    }
  ...
  private void createLocalAndAnonClassNames(
      ArrayList<Node> anonBodyList, ArrayList<Node> localClassDeclList) {
      ...
      if (!localTypeMap.isEmpty()) {
        ...
        while (matchIt.hasNext()) {
          ...
          if (outerMatch.equals(outerToMatch)) {
            int numFound =
                getLocalClassNum(
                    localTypeMap.get(new polyglot.util.IdentityKey(pType)), lcd.decl().name());
            if (numFound >= nextAvailNum) {
              nextAvailNum = numFound + 1;
            }
          }
  ...
  /**
   * ClassToSourceMap is for classes whos names don't match the source file name - ex: multiple top
   * level classes in a single file
   */
