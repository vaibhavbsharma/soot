...
public class PhaseOptions {
   ...
   * its constructor needs PhaseOptions, which also needs a PackManager; OK because we store the
   * PackManager being initialized in a field. b. PhaseOptions used first. Then getPM() calls
   * PackManager.v(), which calls the constr, which sets the .pm field here, uses PhaseOptions
  ...
  public boolean setPhaseOption(HasPhaseOptions phase, String option) {
    ...
    if (optionMap == null) {
      G.v()
          .out
          .println(
              "Option " + option + " given for nonexistent" + " phase " + phase.getPhaseName());
      return false;
    }
