...
public class SootMethod extends AbstractHost
  ...
  /**
   * Nomair A. Naeem , January 14th 2006 Need it for the decompiler to create a new SootMethod The
   * SootMethod can be created fine but when one tries to create a SootMethodRef there is an error
   * because there is no declaring class set. Dava cannot add the method to the class until after it
   * has ended decompiling the remaining method (new method added is added in the PackManager) It
   * would make sense to setDeclared to true within this method too. However later when the
   * sootMethod is added it checks that the method is not set to declared (isDeclared).
   */
  ...
  /**
   * Adds the given exception to the list of exceptions thrown by this method unless the exception
   ...
   * [Ljava/lang/Object instead of java.lang.Object[]).
   */
  ...
  public SootMethodRef makeRef() {
    return Scene.v()
        .makeMethodRef(
            declaringClass,
            name,
            parameterTypes == null ? null : Arrays.asList(parameterTypes),
            returnType,
            isStatic());
  }
