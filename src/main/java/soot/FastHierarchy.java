...
public class FastHierarchy {
  ...
  protected int dfsVisit(int start, SootClass c) {
    ...
    if (!classToInterval.containsKey(c)) classToInterval.put(c, r);
    return start;
  }
  ...
  public boolean canStoreType(Type child, Type parent) {
    ...
    if (child instanceof RefType) {
      if (parent.equals(sc.getObjectType())) return true;
      ...
      if (!(parent instanceof RefLikeType)) {
        ...
        return base.equals(sc.getObjectType())
    ...
    for (final Type t : concreteTypes) {
      if (t instanceof AnySubType) {
        ...
        SootMethod concreteM =
            resolveConcreteDispatch(RefType.v("java.lang.Object").getSootClass(), m);
        if (concreteM != null) ret.add(concreteM);
      } else throw new RuntimeException("Unrecognized reaching type " + t);
    }
    return ret;
      ...
      if (!concreteType.isAbstract()) {
        while (true) {
          ...
          if (!concreteType.hasSuperclass()) {
            if (concreteType.isPhantom()) break;
            else
              throw new RuntimeException(
