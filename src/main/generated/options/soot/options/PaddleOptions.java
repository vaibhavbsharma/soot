...
public class PaddleOptions {
   ...
   * Verbose --
   *
   * <p>Print detailed information about the execution of Paddle.
   *
   * <p>When this option is set to true, Paddle prints detailed information about its execution.
   */
   ...
   * Use BDDs --
   *
   * <p>Use BDD version of Paddle.
   *
   * <p>Causes Paddle to use BDD versions of its components
   */
  public boolean bdd() {
   ...
   * <p>.
   *
   * <p>Allows the BDD package to perform dynamic variable ordering.
   */
   ...
   * Profile --
   *
   * <p>Profile BDDs using JeddProfiler.
   *
   * <p>Turns on JeddProfiler for profiling BDD operations.
   */
   ...
   * Verbose GC --
   *
   * <p>Print memory usage at each BDD garbage collection..
   *
   * <p>Print memory usage at each BDD garbage collection.
   */
   ...
   *
   * <p>Make Paddle completely ignore declared types of variables.
   *
   * <p>When this option is set to true, all parts of Paddle completely ignore declared types of
   * variables and casts.
   */
   ...
   * Pre Jimplify --
   *
   * <p>Jimplify all methods before starting Paddle.
   *
   * <p>When this option is set to true, Paddle converts all available methods to Jimple before
   ...
   *
   * <p>Treat allocation sites context-sensitively.
   *
   * <p>When this option is set to true, the context-sensitivity level that is set for the
   * context-sensitive call graph and for pointer variables is also used to model heap locations
   * context-sensitively. When this option is false, heap locations are modelled
   * context-insensitively regardless of the context-sensitivity level.
   */
   ...
   *
   * <p>Setting RTA to true sets types-for-sites to true, and causes Paddle to use a single
   ...
   * objects are grouped together as a single allocation site. Allocation sites creating a
   * java.lang.StringBuilder object are also grouped together as a single allocation site.
   */
   ...
   *
   * <p>When this option is set to false, Paddle only distinguishes string constants that may be the
   ...
   *
   * <p>Use global node to model variables in simulations of native methods.
   *
   * <p>The simulations of native methods such as System.arraycopy() use temporary local variable
   * nodes. Setting this switch to true causes them to use global variable nodes instead, reducing
   * precision. The switch exists only to make it possible to measure this effect on precision;
   ...
   *
   * <p>Use pointer assignment edges to model this parameters.
   *
   * <p>When constructing a call graph on-the-fly during points-to analysis, Paddle normally
   * propagates only those receivers that cause a method to be invoked to the this pointer of the
   * method. When this option is set to true, however, Paddle instead models flow of receivers as an
   * assignnment edge from the receiver at the call site to the this pointer of the method, reducing
   * precision.
   */
   ...
   * Precise newInstance --
   *
   * <p>Make newInstance only allocate objects of dynamic classes.
   *
   * <p>Normally, newInstance() calls are treated as if they may return an object of any type.
   * Setting this option to true causes them to be treated as if they return only objects of the
   * type of some dynamic class.
   */
   ...
   *
   * <p>Print number of contexts for each method.
   *
   * <p>Causes Paddle to print the number of contexts for each method and call edge, and the number
   * of equivalence classes of contexts for each variable node.
   */
   ...
   *
   * <p>Print total number of contexts.
   *
   * <p>Causes Paddle to print the number of contexts and number of context equivalence classes.
   */
   ...
   *
   * <p>Causes Paddle to print the number of contexts and number of context equivalence classes
   * split out by method. Requires total-context-counts to also be turned on.
   */
   ...
   *
   * <p>When this option is set to true, Paddle computes and prints various cryptic statistics about
   ...
   * Number nodes --
   *
   * <p>Print node numbers in dumps.
   *
   * <p>When printing debug information about nodes, this option causes the node number of each node
   * to be printed.
   */
   ...
   * <p>.
   *
   * <p>Selects one of the BDD variable orderings hard-coded in Paddle.
   */
  public int order() {
   ...
   * BDD Nodes --
   *
   * <p>Number of BDD nodes to allocate (0=unlimited).
   *
   * <p>This option specifies the number of BDD nodes to be used by the BDD backend. A value of 0
   * causes the backend to start with one million nodes, and allocate more as required. A value
   * other than zero causes the backend to start with the specified size, and prevents it from ever
   * allocating any more nodes.
   */
   ...
   * <p>.
   *
   * <p>The maximum length of call string or receiver object string used as context.
   */
  public int k() {
   ...
   * Configuration --
   *
   * <p>Select Paddle configuration.
   *
   * <p>Selects the configuration of points-to analysis and call graph construction to be used in
   * Paddle.
   */
  public int conf() {
    ...
    if (s.equalsIgnoreCase("cha")) return conf_cha;

    if (s.equalsIgnoreCase("cha-aot")) return conf_cha_aot;

    if (s.equalsIgnoreCase("ofcg-aot")) return conf_ofcg_aot;

    if (s.equalsIgnoreCase("cha-context-aot")) return conf_cha_context_aot;

    if (s.equalsIgnoreCase("ofcg-context-aot")) return conf_ofcg_context_aot;

    if (s.equalsIgnoreCase("cha-context")) return conf_cha_context;

    if (s.equalsIgnoreCase("ofcg-context")) return conf_ofcg_context;
   ...
   *
   * <p>Select queue implementation.
   *
   * <p>Select the implementation of worklists to be used in Paddle.
   */
  public int q() {
    String s = soot.PhaseOptions.getString(options, "q");

    if (s.equalsIgnoreCase("auto")) return q_auto;

    if (s.equalsIgnoreCase("trad")) return q_trad;

    if (s.equalsIgnoreCase("bdd")) return q_bdd;

    if (s.equalsIgnoreCase("debug")) return q_debug;

    if (s.equalsIgnoreCase("trace")) return q_trace;

    if (s.equalsIgnoreCase("numtrace")) return q_numtrace;
   ...
   * Backend --
   *
   * <p>Select BDD backend.
   *
   * <p>This option tells Paddle which implementation of BDDs to use.
   */
  public int backend() {
    String s = soot.PhaseOptions.getString(options, "backend");

    if (s.equalsIgnoreCase("auto")) return backend_auto;

    if (s.equalsIgnoreCase("buddy")) return backend_buddy;

    if (s.equalsIgnoreCase("cudd")) return backend_cudd;

    if (s.equalsIgnoreCase("sable")) return backend_sable;

    if (s.equalsIgnoreCase("javabdd")) return backend_javabdd;

    if (s.equalsIgnoreCase("none")) return backend_none;
   ...
   * Context abstraction --
   *
   * <p>Select context-sensitivity level.
   *
   * <p>This option tells Paddle which level of context-sensitivity to use in constructing the call
   * graph.
   */
  public int context() {
    String s = soot.PhaseOptions.getString(options, "context");

    if (s.equalsIgnoreCase("insens")) return context_insens;

    if (s.equalsIgnoreCase("1cfa")) return context_1cfa;

    if (s.equalsIgnoreCase("kcfa")) return context_kcfa;

    if (s.equalsIgnoreCase("objsens")) return context_objsens;

    if (s.equalsIgnoreCase("kobjsens")) return context_kobjsens;

    if (s.equalsIgnoreCase("uniqkobjsens")) return context_uniqkobjsens;

    if (s.equalsIgnoreCase("threadkobjsens")) return context_threadkobjsens;
   ...
   *
   * <p>This option tells Paddle which propagation algorithm to use.
   */
   ...
   *
   * <p>Select an implementation of points-to sets for Paddle to use.
   */
  public int set_impl() {
