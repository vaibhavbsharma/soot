...
/**
 * @production EnumConstant : {@link FieldDeclaration} ::= <span class="component">{@link
 *     Modifiers}</span> <span class="component">&lt;ID:String&gt;</span> <span
 *     class="component">Arg:{@link Expr}*</span> <span class="component">[Init:{@link Expr}]</span>
...
public class EnumConstant extends FieldDeclaration implements Cloneable {
  ...
  private static Opt<TypeDecl> createOptAnonymousDecl(List<BodyDecl> bds) {
    if (bds.getNumChildNoTransform() == 0) return new Opt<TypeDecl>();
  ...
  public int getNumBodyDecl() {
    ...
    if (!init.hasTypeDecl()) return 0;
    for (BodyDecl bd : init.getTypeDecl().getBodyDecls())
      if (!(bd instanceof ConstructorDecl)) ++cnt;
    return cnt;
  }
  /**
   * @ast method
  ...
  public BodyDecl getBodyDecl(int i) {
    ...
    if (init.hasTypeDecl())
      for (BodyDecl bd : init.getTypeDecl().getBodyDecls())
        if (!(bd instanceof ConstructorDecl)) if (i-- == 0) return bd;
