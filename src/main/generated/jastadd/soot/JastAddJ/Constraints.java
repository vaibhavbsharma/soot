...
public class Constraints extends java.lang.Object {
  ...
  public boolean unresolvedTypeArguments() {
    for (Iterator iter = typeVariables.iterator(); iter.hasNext(); ) {
      ...
      if (set.typeArgument == null) return true;
    }
    return false;
  }
  ...
  public void resolveEqualityConstraints() {
    for (Iterator iter = typeVariables.iterator(); iter.hasNext(); ) {
      ...
      if (set.typeArgument == null
          ...
          && set.equaltypeConstraints.contains(T)) set.typeArgument = T;
    }
  }
  ...
  */
  ...
  public static HashSet directSupertypes(TypeDecl t) {
    if (t instanceof ClassDecl) {
      ...
      HashSet set = new HashSet();
      if (type.hasSuperclass()) set.add(type.superclass());
      for (int i = 0; i < type.getNumImplements(); i++) set.add(type.getImplements(i).type());
      return set;
      ...
      HashSet set = new HashSet();
      for (int i = 0; i < type.getNumTypeBound(); i++) set.add(type.getTypeBound(i).type());
      return set;
    } else
      throw new Error(
  ...
  public static void addParameterizedSupertypes(TypeDecl t, HashSet processed, HashSet result) {
    if (!processed.contains(t)) {
      processed.add(t);
      if (t.isParameterizedType() /*&& !t.isRawType()*/) result.add(t);
  ...
  public void convertibleTo(TypeDecl A, TypeDecl F) {
    // System.out.println("Convertible to called with " + A.fullName() + "(" +
    // A.getClass().getName() + ")" + " and " + F.fullName() + "(" + F.getClass().getName() +
    // ")");
    // If F does not involve a type parameter Tj then con constraint is implied on Tj
    if (!F.involvesTypeParameters()) return;
    ...
    // If F = Tj, then the constrint Tj :> A is implied
      ...
      if (typeVariables.contains(F)) addSupertypeConstraint(F, A);
    }
  ...
  public void convertibleFrom(TypeDecl A, TypeDecl F) {
    ...
    } else if (F instanceof ParTypeDecl
        && !F.isRawType()
              ...
              if (H != G) {
                for (Iterator iter = parameterizedSupertypes(F).iterator(); iter.hasNext(); ) {
                  ...
                  if (!V.isRawType() && ((ParTypeDecl) V).genericDecl() == H) {
                    if (F.instanceOf(V)) convertibleFrom(A, V);
                  }
                }
    ...
    } else if (F.isRawType()) rawAccess = true;
  }
