...
public class PathPart extends java.lang.Object {
  ...
  private static PathPart createPathPart(String s) {
    try {
      File f = new File(s);
      if (f.isDirectory()) return new FolderPart(f);
      else if (f.isFile()) return new ZipFilePart(f);
    } catch (IOException e) {
      // error in path
    }
    return null;
  ...
  public CompilationUnit getCompilationUnit() {
    ...
    try {
      if (!isSource) {
        if (program.options().verbose()) System.out.print("Loading .class file: " + fullName + " ");
        ...
        if (program.options().verbose())
          System.out.println(
              "from " + pathName + " in " + (System.currentTimeMillis() - startTime) + " ms");
        return u;
      } else {
        if (program.options().verbose()) System.out.print("Loading .java file: " + fullName + " ");
