...
public class MethodInfo extends java.lang.Object {
  ...
  public MethodInfo(BytecodeParser parser) {
    p = parser;
    flags = p.u2();
    if (BytecodeParser.VERBOSE) p.print("  Flags: " + Integer.toBinaryString(flags));
    int name_index = p.u2();
    ...
    if (info == null || !(info instanceof CONSTANT_Utf8_Info)) {
      ...
      //  System.err.println(" found CONSTANT_Class_Info: " +
      // ((CONSTANT_Class_Info)info).name());
  ...
  public BodyDecl bodyDecl() {
    ...
    List exceptionList =
        (s != null && s.hasExceptionList()) ? s.exceptionList() : attributes.exceptionList();
    ...
    if (isConstructor()) {
      ...
      b =
          new AnnotationMethodDecl(
              ...
              exceptionList,
              new Opt(new Block()),
              new Opt(attributes.elementValue()));
