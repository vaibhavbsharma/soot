...
public class FolderPart extends PathPart {
  ...
  /**
   * Maps package names to a collection of the names of files in the corresponding package
   * directory.
   */
   ...
   * @return <code>true</code> if the given package exists in this source folder
   */
  ...
  /**
   * We need to use getCanonicalFile in order to get the case-sensitive package name on
   * case-insensitive file systems or we might incorrectly report a package name conflict.
   *
    ...
    if (!packageMap.containsKey(packageName)) {
      ...
      try {
        ...
        if (canonical.isDirectory()
            && (packageName.isEmpty() || canonical.getName().equals(name))) {
