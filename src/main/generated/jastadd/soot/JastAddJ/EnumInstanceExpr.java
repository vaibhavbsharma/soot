...
/**
 * @production EnumInstanceExpr : {@link ClassInstanceExpr} ::= <span class="component">{@link
 *     Access}</span> <span class="component">Arg:{@link Expr}*</span> <span
 *     class="component">[{@link TypeDecl}]</span>;
 * @ast node
...
public class EnumInstanceExpr extends ClassInstanceExpr implements Cloneable {
  ...
  * @attribute syn nta
  ...
  private List<Expr> getArgList_compute() {
    ...
    if (idx == -1) throw new Error("internal: cannot determine numeric value of enum constant");
    List<Expr> argList = new List<Expr>();
    argList.add(Literal.buildStringLiteral(ec.name()));
    argList.add(Literal.buildIntegerLiteral(idx));
    for (Expr arg : ec.getArgs()) argList.add((Expr) arg.fullCopy());
    return argList;
  }
  /** @apilevel internal */
