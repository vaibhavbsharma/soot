...
public class Attributes extends java.lang.Object {
  ...
  protected void attributes() {
    ...
    if (BytecodeParser.VERBOSE) p.println("    " + attributes_count + " attributes:");
  ...
  protected ElementValue readElementValue() {
    ...
    switch (c) {
      ...
      case '[':
        int index = p.u2();
        List list = new List();
        for (int i = 0; i < index; i++) list.add(readElementValue());
  ...
  public static class FieldAttributes extends Attributes {
    ...
    protected void processAttribute(String attribute_name, int attribute_length) {
      if (attribute_name.equals("ConstantValue") && attribute_length == 2) {
        ...
        if (annotations == null) annotations = new java.util.ArrayList();
        for (int j = 0; j < num_annotations; j++) annotations.add(readAnnotation());
  ...
  public static class MethodAttributes extends Attributes {
    ...
    protected void processAttribute(String attribute_name, int attribute_length) {
      if (attribute_name.equals("Exceptions")) {
        ...
        for (int i = 0; i < num_parameters; i++) {
          if (parameterAnnotations[i] == null) parameterAnnotations[i] = new java.util.ArrayList();
          int num_annotations = p.u2();
          for (int j = 0; j < num_annotations; j++) parameterAnnotations[i].add(readAnnotation());
        }
    ...
    private void parseExceptions() {
      ...
      if (BytecodeParser.VERBOSE) p.println("      " + number_of_exceptions + " exceptions:");
      for (int i = 0; i < number_of_exceptions; i++) {
        CONSTANT_Class_Info exception = p.getCONSTANT_Class_Info(p.u2());
        if (BytecodeParser.VERBOSE) p.println("        exception " + exception.name());
  ...
  public static class TypeAttributes extends Attributes {
    ...
    public TypeAttributes(
        BytecodeParser p, TypeDecl typeDecl, TypeDecl outerTypeDecl, Program classPath) {
      super(p);
    ...
    protected void innerClasses() {
      ...
      if (BytecodeParser.VERBOSE) p.println("    Number of classes: " + number_of_classes);
      for (int i = 0; i < number_of_classes; i++) {
        if (BytecodeParser.VERBOSE) p.print("      " + i + "(" + number_of_classes + ")" + ":");
        ...
        if (inner_class_info_index > 0) {
          CONSTANT_Class_Info inner_class_info =
              this.p.getCONSTANT_Class_Info(inner_class_info_index);
          ...
          if (outer_class_info_index > 0) {
            CONSTANT_Class_Info outer_class_info =
                this.p.getCONSTANT_Class_Info(outer_class_info_index);
            ...
            // anonymous inner class; need to infer outer_class_name from
            // inner_class_name
          ...
          if (outer_class_name.equals(this.p.classInfo.name())) {
            if (BytecodeParser.VERBOSE)
              p.println(
                  "      Class "
                      + this.p.classInfo.name()
                      + " has inner class: "
                      + inner_class_name);
            if (BytecodeParser.VERBOSE) p.println("Begin processing: " + inner_class_name);
            try {
              ...
              try {
                ...
                if (is != null) {
                  ...
                  p.println("Error: ClassFile " + inner_class_name + " not found");
                }
              } catch (Error e) {
            ...
            if (BytecodeParser.VERBOSE) p.println("End processing: " + inner_class_name);
          }
        }
      }
      if (BytecodeParser.VERBOSE) p.println("    end");
    }
  }
}
