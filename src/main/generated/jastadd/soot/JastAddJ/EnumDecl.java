...
/**
 * @production EnumDecl : {@link ClassDecl} ::= <span class="component">{@link Modifiers}</span>
 *     <span class="component">&lt;ID:String&gt;</span> <span
...
public class EnumDecl extends ClassDecl implements Cloneable {
                                ...
                                new Opt(
                                    new CastExpr(
                                        ...
                                        values
                                            ...
                                            .qualifiesAccess(
                                                new MethodAccess("clone", new List()))))))))));
    // public static Test valueOf(String s) { return (Test)java.lang.Enum.valueOf(Test.class,
    // s); }
    addBodyDecl(
        new MethodDecl(
            new Modifiers(
            ...
            new List()
                .add(
                    new ParameterDeclaration(
                        new Modifiers(new List()), typeString().createQualifiedAccess(), "s")),
            new List(),
            new Opt(
                new Block(
                    new List()
  ...
  protected void checkEnum(EnumDecl enumDecl) {
    for (int i = 0; i < getNumBodyDecl(); i++) {
      if (getBodyDecl(i) instanceof ConstructorDecl) getBodyDecl(i).checkEnum(enumDecl);
      else if (getBodyDecl(i) instanceof InstanceInitializer) getBodyDecl(i).checkEnum(enumDecl);
      else if (getBodyDecl(i) instanceof FieldDeclaration) {
        FieldDeclaration f = (FieldDeclaration) getBodyDecl(i);
        if (!f.isStatic() && f.hasInit()) f.checkEnum(enumDecl);
      }
    }
  }
  /**
  ...
  public void checkModifiers() {
    ...
    if (!unimplementedMethods().isEmpty()) {
      ...
      s.append(
          ""
              + name()
              + " lacks implementations in one or more "
  ...
  * @attribute syn nta
  ...
  private Opt getSuperClassAccessOpt_compute() {
    return new Opt(
        new ParTypeAccess(
            new TypeAccess("java.lang", "Enum"), new List().add(createQualifiedAccess())));
  }
  /**
   * @attribute syn
   * @aspect Modifiers
  ...
  private ArrayList enumConstants_compute() {
    ...
    for (int i = 0; i < getNumBodyDecl(); i++)
      if (getBodyDecl(i).isEnumConstant()) list.add(getBodyDecl(i));
    return list;
  }
  /**
   * @attribute syn
   ...
   *
   * <p>It is a compile-time error for an enum type E to have an abstract method m as a member
   * unless E has one or more enum constants, and all of E's enum constants have class bodies that
  ...
  private Collection unimplementedMethods_compute() {
    ...
    for (Iterator iter = interfacesMethodsIterator(); iter.hasNext(); ) {
      ...
      if (!implemented) methods.add(method);
    }

    for (Iterator iter = localMethodsIterator(); iter.hasNext(); ) {
      MethodDecl method = (MethodDecl) iter.next();
      if (method.isAbstract()) methods.add(method);
    }
    ...
    for (MethodDecl method : methods) {
      ...
      if (missing) unimplemented.add(method);
    }
  ...
  public int sootTypeModifiers() {
    ASTNode$State state = state();
    try {
      return super.sootTypeModifiers() | Modifiers.ACC_ENUM;
    } finally {
    }
  }
  /**
  ...
  public ASTNode rewriteTo() {
    // Declared in
    // /Users/eric/Documents/workspaces/clara-soot/JastAddJ/Java1.5Frontend/Enums.jrag at line
    // 88
    if (!done()) {
      state().duringEnums++;
  ...
  private EnumDecl rewriteRule0() {
    {
      if (noConstructor()) {
        ...
        addBodyDecl(
            new ConstructorDecl(
                new Modifiers(
                    new List().add(new Modifier("private")).add(new Modifier("synthetic"))),
                name(),
                parameterList,
                new List(),
                new Opt(
                    new ExprStmt(
                        new SuperConstructorAccess(
                            "super",
                            new List().add(new VarAccess("p0")).add(new VarAccess("p1"))))),
                new Block(new List())));
      } else {
