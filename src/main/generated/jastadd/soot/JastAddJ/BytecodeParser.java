...
public class BytecodeParser extends java.lang.Object implements Flags, BytecodeReader {
  ...
  public CompilationUnit parse(TypeDecl outerTypeDecl, String outerClassName, Program program)
    ...
    if (BytecodeParser.VERBOSE) println("Parsing byte codes in " + name);
  ...
  public void parseMagic() {
    if (next() != 0xca || next() != 0xfe || next() != 0xba || next() != 0xbe) error("magic error");
  }

  public void parseMinor() {
    int low = u1();
    int high = u1();
    if (BytecodeParser.VERBOSE) println("Minor: " + high + "." + low);
  }

  public void parseMajor() {
    int low = u1();
    int high = u1();
    if (BytecodeParser.VERBOSE) println("Major: " + high + "." + low);
  }
  ...
  public TypeDecl parseTypeDecl() {
    ...
    if ((flags & (ACC_INTERFACE | ACC_ENUM)) == ACC_ENUM) {
      ...
      decl.setSuperInterfaceIdList(
          parseInterfaces(superClass == null ? new List() : new List().add(superClass)));
      return decl;
    } else {
  ...
  public Access fromClassName(String s) {
    ...
    if (index != -1) packageName = s.substring(0, index).replace('/', '.');
  ...
  public static Modifiers modifiers(int flags) {
    ...
    if ((flags & 0x0001) != 0) m.addModifier(new Modifier("public"));
    if ((flags & 0x0002) != 0) m.addModifier(new Modifier("private"));
    if ((flags & 0x0004) != 0) m.addModifier(new Modifier("protected"));
    if ((flags & 0x0008) != 0) m.addModifier(new Modifier("static"));
    if ((flags & 0x0010) != 0) m.addModifier(new Modifier("final"));
    if ((flags & 0x0020) != 0) m.addModifier(new Modifier("synchronized"));
    if ((flags & 0x0040) != 0) m.addModifier(new Modifier("volatile"));
    if ((flags & 0x0080) != 0) m.addModifier(new Modifier("transient"));
    if ((flags & 0x0100) != 0) m.addModifier(new Modifier("native"));
    if ((flags & 0x0400) != 0) m.addModifier(new Modifier("abstract"));
    if ((flags & 0x0800) != 0) m.addModifier(new Modifier("strictfp"));
    return m;
  }

  public void parseFields(TypeDecl typeDecl) {
    int count = u2();
    if (BytecodeParser.VERBOSE) println("Fields (" + count + "):");
    for (int i = 0; i < count; i++) {
      if (BytecodeParser.VERBOSE) print(" Field nbr " + i + " ");
      FieldInfo fieldInfo = new FieldInfo(this);
      if (!fieldInfo.isSynthetic()) typeDecl.addBodyDecl(fieldInfo.bodyDecl());
    }
  }

  public void parseMethods(TypeDecl typeDecl) {
    int count = u2();
    if (BytecodeParser.VERBOSE) println("Methods (" + count + "):");
    for (int i = 0; i < count; i++) {
      if (BytecodeParser.VERBOSE) print("  Method nbr " + i + " ");
  ...
  public CONSTANT_Utf8_Info getCONSTANT_Utf8_Info(int index) {
    ...
    if (!(info instanceof CONSTANT_Utf8_Info))
      throw new Error(
          "Expected CONSTANT_Utf8_info at "
              + index
              + " in constant pool but found "
  ...
  public CONSTANT_Class_Info getCONSTANT_Class_Info(int index) {
    ...
    if (!(info instanceof CONSTANT_Class_Info))
      throw new Error(
          "Expected CONSTANT_Class_info at "
              + index
              + " in constant pool but found "
              + info.getClass().getName());
  ...
  public void parseConstantPool() {
    int count = u2();
    if (BytecodeParser.VERBOSE) println("constant_pool_count: " + count);
    constantPool = new CONSTANT_Info[count + 1];
    for (int i = 1; i < count; i++) {
      parseEntry(i);
      if (constantPool[i] instanceof CONSTANT_Long_Info
          || constantPool[i] instanceof CONSTANT_Double_Info) i++;
    }
