...
public class Unicode extends FilterReader {
  ...
  public int read() throws java.io.IOException {
    ...
    numConsecutiveBackSlash = 0;
    // UnicodeEscape found
    while (lookahead == 'u') next();
    // The next four characters must be hexadecimal digits or else a compile-time error is
    // thrown
    int result = 0;
    for (int i = 0; i < 4; i++) {
      int c = next();
      int value = Character.digit((char) c, 16);
      if (value == -1) throw new Error("Invalid Unicode Escape");
      result <<= 4;
      result += value;
    }
    return result;
