...
public class GLBTypeFactory extends java.lang.Object {
  ...
  private static void addInterfaces(
      ArrayList intersectInterfaceList, ArrayList allInterfaceList, TypeDecl typeDecl) {
   ...
   *
   * <p>For each <i>T<sub>i</sub></i>, 1 &le; i &le; n, let <i>C<sub>i</sub></i> be the most
   * specific class or array type such that <i>T<sub>i</sub></i> &lt;: <i>C<sub>i</sub></i> Then
   * there must be some <i>T<sub>k</sub></i> &lt;: <i>C<sub>k</sub></i> such that
   * <i>C<sub>k</sub></i> &lt;: <i>C<sub>i</sub></i> for any <i>i</i>, 1 &le; i &le; n, or a
   * compile-time error occurs.
   *
   * @param types
   * @return the most specific class that all elements in <i>types</i> are a subtype of. Or null if
  ...
  private static final TypeDecl mostSpecificSuperClass(final TypeDecl t) {
    ...
    else return t.typeObject();
  }

  /**
   * Add the superclasses (<i>C<sub>i</sub></i>) of <i>t</i> to the set <i>result</i>.
   *
   * <ul>
   *   <li>If <i>t</i> is a class, then <i>C<sub>i</sub></i> is t itself.
   *   <li>If <i>t</i> is a type variable, then <i>C<sub>i</sub></i> is the first class encountered
   *       in it bounds
   *   <li>It <i>t</i> is an intersection type, then <i>C<sub>i</sub></i> is class that is a member
   *       of the intersection, otherwise it's Object
   * </ul>
   *
   * @param t
   * @param result
  ...
  private static final void addSuperClasses(final TypeDecl t, final HashSet result) {
    ...
    // interface
    else if (t.isInterfaceDecl()) result.add(t.typeObject());
  }
      ...
      for (Iterator iter1 = superISet_i.iterator(); iter1.hasNext(); ) {
        ...
        if (superIface_i instanceof ParInterfaceDecl) {
          ...
          for (int j = i + 1; j < ifaceList.size(); j++) {
            ...
            for (Iterator iter2 = superISet_j.iterator(); iter2.hasNext(); ) {
  ...
  public static final void greatestLowerBounds(ArrayList types) {
    for (int i = 0; i < types.size(); i++) {
      ...
      for (int j = i + 1; j < types.size(); j++) {
        ...
        if (U.instanceOf(V)) types.set(j, null);
        else if (V.instanceOf(U)) types.set(i, null);
      }
    }
    // filter null's
