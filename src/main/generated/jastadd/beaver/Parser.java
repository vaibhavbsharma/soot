...
public abstract class Parser {
   ...
   *
   * <p>If a source that is being parsed does not have syntax errors this wrapper only adds one
   * indirection while it delivers the next token. However when parser needs to recover from a
   * syntax error this wrapper accumulates tokens shifted by a forward parsing simulation and later
   * feeds them to the recovered parser.
   */
  public class TokenStream {
    ...
    public Symbol nextToken() throws IOException {
      if (buffer != null) {
        if (n_read < n_written) return buffer[n_read++];
     ...
     * @throws IOException as thrown by a scanner
     */
   ...
   * successfully. If during simulation this parser encounters an error it drops the first token it
   * tried to use and restarts the simulated parsing.
   *
   * <p>Note: Without a special "error" rule present in a grammar, which a parser will try to shift
   * at the beginning of an error recovery, simulation continues without removing anything from the
   * original states stack. This often will lead to cases when no parsing ahead will recover the
  ...
  public class Simulator {
      ...
      do {
        ...
        while (true) {
          ...
          if (act > 0) {
            ...
            if (act > 0) shift(act);
            else return act == accept_action_id;
    ...
    private void shift(short state) {
      if (++top == states.length) increaseStackCapacity();
      states[top] = state;
    }
  ...
  /**
   * Parses a source and returns a semantic value of the accepted nonterminal. Before parsing starts
   * injects alternative goal marker into the source to indicate that an alternative goal should be
   * matched.
   *
   * @param source of tokens - a Scanner
   ...
   * @return semantic value of the accepted nonterminal
   */
  public Object parse(Scanner source, short alt_goal_marker_id)
    ...
    while (true) {
      ...
      while (true) {
        ...
        } else // act == 0, i.e. this is an error
        {
          ...
          break; // because error recovery altered token stream - parser needs to refetch
          // the next token
        }
      }
    }
  ...
  /**
   * Invoke actual reduce action routine. Method must be implemented by a generated parser
   *
  ...
  private void init() {
    ...
    _symbols[top] = new Symbol("none"); // need a symbol here for a default reduce on the very first
    // erroneous token
  ...
  /**
   * Implements parsing error recovery. Tries several simple approaches first, like deleting "bad"
   * token or replacing the latter with one of the expected in his state (if possible). If simple
   * methods did not work tries to do error phrase recovery.
   *
   * <p>It is expected that normally descendant parsers do not need to alter this method. In same
   * cases though they may want to override it if they need a different error recovery strategy.
   *
  ...
  protected void recoverFromError(Symbol token, TokenStream in)
    ...
    if (!tables.compressed) // then we can try "insert missing" and "replace unexpected"
    // recoveries
    {
      ...
      if (first_term_id >= 0) {
        ...
        for (short term_id = (short) (first_term_id + 1); term_id < tables.n_term; term_id++) {
          ...
          if (index >= tables.lookaheads.length) break;
    ...
    while ((goto_state = tables.findNextState(states[top], tables.error_symbol_id)) <= 0) {
      ...
      if (--top < 0) throw new Parser.Exception("Cannot recover from the syntax error");
    }
    Symbol error =
        new Symbol(tables.error_symbol_id, first_sym.start, last_sym.end); // the end is temporary
