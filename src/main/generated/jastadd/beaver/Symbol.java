...
public class Symbol implements Cloneable {
   ...
   *
   * <p>Used by classes descending from Symbol and which instances are returned by reduce actions.
   * In this case ID and symbol position will be assigned by the parser when reduce action code
   * returns this symbol.
   */
  protected Symbol() {
    this.value = this;
   ...
   *
   * <p>This ID typically is, depending on a symbol type, either a terminal ID if a Symbol is a
   * token created and returned by a Scanner, or a nonterminal ID if a symbol was created by parser
   * based on that nonterminal definition. In the former case the ID is one of IDs generated by
   * Beaver for terminal symbols. The latter keeps IDs of nonterminal symbols.
   *
   * @return symbol's ID
   */
  public short getId() {
