...
public final class ParsingTables {
  ...
  /**
   * A table containing the lookahead for each entry in "actions" table. Used to detect
   * "collisions".
   */
  final short[] lookaheads;

  /**
   * For each state, the offset into "actions" table that is used to find action for a terminal that
   * has been fetched from the scanner.
   */
  ...
  /**
   * Scans lookaheads expected in a given state for a terminal symbol. Used in error recovery when
   * an unexpected terminal is replaced with one that is expected.
   *
  ...
  final short findFirstTerminal(int state) {
    ...
    for (short term_id = offset < 0 ? (short) -offset : 0; term_id < n_term; term_id++) {
      ...
      if (index >= lookaheads.length) break;
      if (lookaheads[index] == term_id) return term_id;
    }
    return -1;
  }
  ...
  /**
   * Find the appropriate action for a parser in a given state with a specified nonterminal
   * look-ahead. In this case the only possible outcomes are either a state to shift to or an accept
   * action.
   *
   * @param state of a parser
   * @param lookahead
  ...
  static byte[] decode(String spec) {
    ...
    if (chars.length % 4 != 0) throw new IllegalArgumentException("corrupted encoding");
    int len = chars.length / 4 * 3;
    byte[] bytes =
        new byte
            [chars[chars.length - 1] == '='
                ? chars[chars.length - 2] == '=' ? len - 2 : len - 1
                : len];
  ...
  static InputStream getSpecAsResourceStream(Class impl_class) {
    ...
    if (spec_stream == null) throw new IllegalStateException("parser specification not found");
    return spec_stream;
  }
}
