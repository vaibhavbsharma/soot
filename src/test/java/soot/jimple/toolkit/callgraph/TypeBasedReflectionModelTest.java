 ...
 *
 * @author Manuel Benz created on 01.08.17
 */
...
public class TypeBasedReflectionModelTest {
  ...
  private void genericLocalVsStringConstantTest(boolean useConstantBase) {
    ...
    for (Edge edge : callGraph) {
      if (edge.getSrc().method().getSignature().contains("main")
          && edge.getTgt().method().getSignature().contains("concat")) {
        found = true;
        break;
      }
    }
    Assert.assertTrue(
        "There should be an edge from main to String.concat after resolution of the target of Method.invoke, but none found",
        found);
  }
  ...
  private SootClass setUpTestClass(boolean useConstantBase) {
    ...
    // get class
    StaticInvokeExpr getClass =
        Jimple.v()
            .newStaticInvokeExpr(
                Scene.v()
                    .getMethod("<java.lang.Class: java.lang.Class forName(java.lang.String)>")
                    .makeRef(),
    ...
    if (useConstantBase) base = StringConstant.v(baseString);
      ...
      base = l;
      b.getLocals().add(l);
      b.getUnits().add(Jimple.v().newAssignStmt(l, StringConstant.v(baseString)));
    }
    ...
    // prepare arg array
    Local argArray =
        Jimple.v().newLocal("argArray", ArrayType.v(Scene.v().getType("java.lang.Object"), 1));
        ...
        .add(
            Jimple.v()
                .newAssignStmt(
                    argArray,
                    Jimple.v()
                        .newNewArrayExpr(Scene.v().getType("java.lang.Object"), IntConstant.v(1))));
    b.getUnits()
        .add(Jimple.v().newAssignStmt(Jimple.v().newArrayRef(argArray, IntConstant.v(0)), l));
