/**
 * This class is a piece of test infrastructure. It compares various varieties of control flow
 * graphs.
 */
...
public class GraphComparer {
  ...
  /**
   * Utility interface for keeping track of graph nodes which are considered to represent equivalent
   * nodes in the two graphs being compared.
   */
  ...
  /**
   * {@link EquivalenceRegistry} for comparing two {@link UnitGraph}s Since the same {@link Unit}s
   * are stored as nodes in both graphs, equivalence is the same as object equality.
   */
  ...
  /**
   * {@link EquivalenceRegistry} for comparing two {@link BlockGraph}s. Two blocks are considered
   * equivalent if they contain exactly the same list of {@link Unit}s, in the same order.
   */
  static class EquivalentBlockRegistry implements EquivalenceRegistry {
    private Map equivalenceMap = new HashMap();

    /**
     * Create an {@link EquivalentBlockRegistry} which records the equivalent blocks in two graphs
     * whose nodes are blocks. To allow the use of graphs that are loaded from alternate class
     * paths, the parameters are not required to be instances of {@link BlockGraph}. They just have
     * to be {@link DirectedGraph}s whose nodes are instances of some class that has an
     * <tt>iterator()</tt> method that iterates over the {@link Unit}s in that block.
     *
     * @param g1 The first graph to register.
     * @param g2 The second graph to register.
     * @throws IllegalArgumentException if a given {@link Unit} appears in more than one block of
    ...
    /**
     * Return a map from a {@link Unit} in the body represented by a {@link BlockGraph} to the graph
     * node representing the block containing that {@link Unit}.
     *
     * @param g a graph whose nodes represent lists of {@link Unit}s. The nodes must have an
     *     <tt>iterator()</tt> method which will iterate over the {@link Unit}s represented by the
     *     node.
     * @return a {@link Map} from {@link Unit}s to {@link Object}s that are the graph nodes
     *     containing those {@link Unit}s.
     * @throws IllegalArgumentException should any node of <tt>g</tt> lack an <tt>iterator()</tt>
     *     method or should any {@link Unit} appear in more than one node of the graph.
     */
    private static Map blockGraphToUnitMap(DirectedGraph g) throws IllegalArgumentException {
        ...
        for (Iterator unitIt = units.iterator(); unitIt.hasNext(); ) {
          ...
          if (result.containsKey(unit)) {
            throw new IllegalArgumentException(
                "blockGraphToUnitMap(): adding " + unit.toString() + " twice");
          }
    ...
    /**
     * Return the {@link List} of {@link Unit}s represented by an object which has an
     * <tt>iterator()</tt> method which iterates over {@link Unit}s.
     *
    ...
    private static List getUnits(Object block) {
      ...
      try {
        ...
        for (Iterator it = (Iterator) iterMethod.invoke(block, new Object[0]); it.hasNext(); ) {
          Unit unit = (Unit) it.next();
          result.add(unit);
        }
      } catch (NoSuchMethodException e) {
        throw new IllegalArgumentException(
            "GraphComparer.getUnits(): node lacks iterator() method.");
      } catch (IllegalAccessException e) {
        throw new IllegalArgumentException(
            "GraphComparer.getUnits(): inaccessible iterator() method.");
      } catch (java.lang.reflect.InvocationTargetException e) {
        throw new IllegalArgumentException(
            "GraphComparer.getUnits(): failed iterator() invocation.");
      }
      return result;
    }
  }
  ...
  /**
   * Utility interface for checking whether two graphs of particular types differ only in the ways
   * we would expect from two graphs of those types that represent the same {@link Body}.
   */
  interface TypedGraphComparer {
    /** @return true if the two graphs represented by this comparer differ only in expected ways. */
  ...
  /**
   * Return a class that implements <tt>TypedGraphComparer</tt> for the two graphs being compared by
   * this <tt>GraphComparer</tt>, or <tt>null</tt>, if there is no comparer available to compare
   * their types.
   */
  TypedGraphComparer TypedGraphComparerFactory() {
    class TypeAssignments {
      ...
      void add(DirectedGraph g) {
        if (g instanceof CompleteUnitGraph) {
          ...
          this.add((ExceptionalUnitGraph) g, Options.v().omit_excepting_unit_edges());
          ...
          throw new IllegalStateException(
              "GraphComparer.add(): don't know how to add(" + g.getClass().getName() + ')');
        }
      }
    }
    TypeAssignments subtypes = new TypeAssignments(g1, g2);

    if (subtypes.exceptionalUnitGraph != null && subtypes.classicCompleteUnitGraph != null) {
    ...
    if (subtypes.exceptionalUnitGraph != null && subtypes.trapUnitGraph != null) {
      return new ExceptionalToTrapUnitGraphComparer(
          subtypes.exceptionalUnitGraph, subtypes.trapUnitGraph, subtypes.omitExceptingUnitEdges);
    }

    if (subtypes.classicCompleteBlockGraph != null && subtypes.altCompleteBlockGraph != null) {
      return new BlockToAltBlockGraphComparer(
          subtypes.classicCompleteBlockGraph, subtypes.altCompleteBlockGraph);
    }

    if (subtypes.briefBlockGraph != null && subtypes.altBriefBlockGraph != null) {
      return new BlockToAltBlockGraphComparer(
          subtypes.briefBlockGraph, subtypes.altBriefBlockGraph);
    }

    if (subtypes.arrayRefBlockGraph != null && subtypes.altArrayRefBlockGraph != null) {
      return new BlockToAltBlockGraphComparer(
          subtypes.arrayRefBlockGraph, subtypes.altArrayRefBlockGraph);
    }

    if (subtypes.zonedBlockGraph != null && subtypes.altZonedBlockGraph != null) {
      return new BlockToAltBlockGraphComparer(
          subtypes.zonedBlockGraph, subtypes.altZonedBlockGraph);
    }
    return null;
  }
  ...
  public GraphComparer(DirectedGraph g1, DirectedGraph g2) {
    ...
    if (g1ClassName.endsWith("BlockGraph") && g2ClassName.endsWith("BlockGraph")) {
  ...
  /**
   * Checks that the two graphs in the GraphComparer differ only in ways that the GraphComparer
   * expects that they should differ, given the types of graphs they are, assuming that the two
   ...
   *
   * @param g the {@link DirectedGraph} whose edge set is to be checked.
   * @return <tt>true</tt> if <tt>g</tt>'s edge set is consistent (meaning that <tt>x</tt> is
   *     recorded as a predecessor of <tt>y</tt> if and only if <tt>y</tt> is recorded as a
   *     successor of <tt>x</tt>).
   */
  public static boolean consistentGraph(DirectedGraph g) {
    for (Iterator nodeIt = g.iterator(); nodeIt.hasNext(); ) {
      ...
      for (Iterator predIt = g.getPredsOf(node).iterator(); predIt.hasNext(); ) {
        Object pred = predIt.next();
        if (!g.getSuccsOf(pred).contains(node)) {
          return false;
        }
      }
      for (Iterator succIt = g.getSuccsOf(node).iterator(); succIt.hasNext(); ) {
   ...
   *
   * @return <tt>true</tt> if the two graphs have the same nodes, the same lists of heads and tails,
   *     and the same sets of edges, <tt>false</tt> otherwise.
   */
  public boolean equal() {
    if ((!consistentGraph(g1)) || (!consistentGraph(g2))) {
      throw new CompilationDeathException(
          CompilationDeathException.COMPILATION_ABORTED, "Cannot compare inconsistent graphs");
    }
  ...
  /**
   * Compares the predecessors and successors of corresponding nodes of this {@link GraphComparer}'s
   * two {@link DirectedGraph}s. This is factored out for sharing by {@link #equal()} and {@link
   * BlockToAltBlockGraphComparer#onlyExpectedDiffs}.
   *
   * @return <tt>true</tt> if the predecessors and successors of each node in one graph are
   *     equivalent to the predecessors and successors of the equivalent node in the other graph.
   */
  protected boolean equivPredsAndSuccs() {
    ...
    for (Iterator g1it = g1.iterator(); g1it.hasNext(); ) {
      ...
      try {
        if (!equivLists(g1.getSuccsOf(g1node), g2.getSuccsOf(equivalences.getEquiv(g1node)))) {
          return false;
        }
        if (!equivLists(g1.getPredsOf(g1node), g2.getPredsOf(equivalences.getEquiv(g1node)))) {
          return false;
        }
   ...
   *
   * @param graph1Label a string to be used to identify the first graph (<tt>g1</tt> in the call to
   *     {@link GraphComparer#GraphComparer()}) in the report of differences.
   * @param graph2Label a string to be used to identify the second graph (<tt>g2</tt> in the call to
   *     {@link GraphComparer@GraphComparer()}) in the report of differences .
   * @return a {@link String} summarizing the differences between the two graphs.
   */
  public String diff(String graph1Label, String graph2Label) {
    ...
    diffList(report, printer1, printer2, "HEADS", g1.getHeads(), g2.getHeads());
    diffList(report, printer1, printer2, "TAILS", g1.getTails(), g2.getTails());
    ...
    for (Iterator g1it = g1.iterator(); g1it.hasNext(); ) {
      ...
      diffList(report, printer1, printer2, g1string + " PREDS", g1preds, g2preds);
      ...
      diffList(report, printer1, printer2, g1string + " SUCCS", g1succs, g2succs);
    }
    ...
    for (Iterator g2it = g2.iterator(); g2it.hasNext(); ) {
      ...
      try {
        ...
        if (e.getMessage() != null && e.getMessage().startsWith("Invalid unit ")) {
          diffList(report, printer1, printer2, g2string + " PREDS", null, g2preds);
        } else {
          throw e;
        }
      }
      ...
      try {
        ...
        if (e.getMessage() != null && e.getMessage().startsWith("Invalid unit ")) {
          diffList(report, printer1, printer2, g2string + " SUCCS", null, g2succs);
        } else {
          throw e;
        }
      }
  ...
  /** Class for comparing a {@link TrapUnitGraph} to an {@link ExceptionalUnitGraph}. */
  class ExceptionalToTrapUnitGraphComparer implements TypedGraphComparer {
    ...
    /**
     * Indicates whether {@link #predOfTrappedThrower()} should return false for edges from head to
     * tail when the only one of head's successors which throws an exception caught by tail is the
     * first unit trapped by tail (this distinguishes ClassicCompleteUnitGraph from TrapUnitGraph).
     */
    protected boolean predOfTrappedThrowerScreensFirstTrappedUnit;

    ExceptionalToTrapUnitGraphComparer(
        ExceptionalUnitGraph exceptional, TrapUnitGraph cOrT, boolean omitExceptingUnitEdges) {
      ...
      if (exceptional.size() != cOrT.size()) {
        if (Options.v().verbose())
          G.v()
              .out
              .println(
                  "ExceptionalToTrapUnitComparer.onlyExpectedDiffs(): sizes differ"
                      + exceptional.size()
                      + " "
                      + cOrT.size());
        return false;
              ...
              if (Options.v().verbose())
                G.v()
                    ...
                    .println(
                        "ExceptionalToTrapUnitComparer.onlyExpectedDiffs(): "
                            + cOrTSucc.toString()
                            + " is not exceptional successor of "
                            + node.toString()
                            + " even though "
                            + node.toString()
                            + " can throw to it");
            ...
            if ((!cOrTSuccs.contains(exceptionalSucc))
              ...
              if (Options.v().verbose())
                G.v()
                    ...
                    .println(
                        "ExceptionalToTrapUnitComparer.onlyExpectedDiffs(): "
                            + exceptionalSucc.toString()
                            + " is not TrapUnitGraph successor of "
                            + node.toString()
                            + " even though "
                            + node.toString()
              ...
              if (Options.v().verbose())
                G.v()
                    ...
                    .println(
                        "ExceptionalToTrapUnitComparer.onlyExpectedDiffs(): "
                            + cOrTPred.toString()
                            + " is not ExceptionalUnitGraph predecessor of "
                            + node.toString()
                            + " even though "
                            + cOrTPred.toString()
                            + " can throw to "
                ...
                G.v()
                    ...
                    .println(
                        "ExceptionalToTrapUnitComparer.onlyExpectedDiffs(): "
                            ...
                            + " is not COrTUnitGraph predecessor of "
                            + node.toString()
                            + " even though "
                            + exceptionalPred.toString()
    ...
    /**
     * A utility method for confirming that a node which is considered a tail by a {@link
     * ExceptionalUnitGraph} but not by the corresponding {@link TrapUnitGraph} or {@link
     * ClassicCompleteUnitGraph} is a return instruction or throw instruction with a {@link Trap}
     * handler as its successor in the {@link TrapUnitGraph} or {@link ClassicCompleteUnitGraph}.
     *
     * @param node the node which is considered a tail by <tt>exceptional</tt> but not by
     *     <tt>cOrT</tt>.
     * @return <tt>true</tt> if <tt>node</tt> is a return instruction which has a trap handler as
     *     its successor in <tt>cOrT</tt>.\
     */
    ...
    /**
     * A utility method for confirming that an edge which is in a {@link TrapUnitGraph} or {@link
     * ClassicCompleteUnitGraph} but not the corresponding {@link ExceptionalUnitGraph} satisfies
     * the criterion we would expect of such an edge: that it leads from a unit to a handler, that
     * the unit might be expected to trap to the handler based solely on the range of Units trapped
     * by the handler, but that there is no way for the particular exception that the handler
     * catches to be thrown at this point.
     *
     * @param head the graph node that the edge leaves.
     * @param tail the graph node that the edge leads to.
     * @return true if <tt>head</tt> cannot really be associated with an exception that
     *     <tt>tail</tt> catches, false otherwise.
     */
    ...
    /**
     * A utility method for determining if a {@link Unit} is among those protected by a particular
     * {@link Trap}..
     *
     * @param unit the {@link Unit} being asked about.
     * @param trap the {@link Trap} being asked about.
     * @return <tt>true</tt> if <tt>unit</tt> is protected by <tt>trap</tt>, false otherwise.
     */
    ...
    /**
     * A utility method for confirming that an edge which is in a {@link CompleteUnitGraph} but not
     * the corresponding {@link TrapUnitGraph} satisfies the criteria we would expect of such an
     * edge: that the tail of the edge is a {@link Trap} handler, and that the head of the edge is
     * not itself in the <tt>Trap</tt>'s protected area, but is the predecessor of a {@link Unit},
     * call it <tt>u</tt>, such that <tt>u</tt> is a <tt>Unit</tt> in the <tt>Trap</tt>'s protected
     * area and <tt>u</tt> might throw an exception that the <tt>Trap</tt> catches.
     *
     * @param head the graph node that the edge leaves.
     * @param tail the graph node that the edge leads to.
     * @return a {@link List} of {@link Trap}s such that <tt>head</tt> is a predecessor of a {@link
     *     Unit} that might throw an exception caught by {@link Trap}.
     */
    protected boolean predOfTrappedThrower(Unit head, Unit tail) {
      ...
      for (Iterator throwerIt = possibleThrowers.iterator(); throwerIt.hasNext(); ) {
          ...
          if (tailsTraps.contains(trap)) {
            if (headsCatchers.contains(trap)) {
              throw new RuntimeException(
                  "trapsReachedViaEdge(): somehow there is no TrapUnitGraph edge from "
                      + head
                      + " to "
                      + tail
                      + " even though the former throws an exception caught by the latter!");
    ...
    /**
     * Utility method that returns all the {@link Trap}s whose handlers begin with a particular
     * {@link Unit}.
     *
     * @param handler the handler {@link Unit} whose {@link Trap}s are to be returned.
     * @return a {@link List} of {@link Trap}s with <code>handler</code> as their handler's first
     *     {@link Unit}. The list may be empty.
     */
  ...
  /**
   * Class for comparing an {@link ExceptionalUnitGraph} to a {@link ClassicCompleteUnitGraph} .
   * Since {@link ClassicCompleteUnitGraph} is a subclass of {@link TrapUnitGraph}, this class makes
   * only minor adjustments to its parent.
   */
  class ExceptionalToClassicCompleteUnitGraphComparer extends ExceptionalToTrapUnitGraphComparer {

    ExceptionalToClassicCompleteUnitGraphComparer(
        ExceptionalUnitGraph exceptional,
  ...
  /**
   * Class for Comparing a {@link BriefBlockGraph}, {@link ArrayRefBlockGraph}, or {@link
   * ZonedBlockGraph} to an {@link AltBriefBlockGraph}, {@link AltArrayRefBlockGraph}, or {@link
   * AltZonedBlockGraph}, respectively. The two should differ only in that the <tt>Alt</tt> variants
   * have empty tail lists.
   */
  ...
  /**
   * A utility method for determining if a {@link Collection} of {@link
   * CompleteUnitGraph#ExceptionDest} contains one which leads to a specified {@link Trap}.
   *
   * @param dests the {@link Collection} of {@link CompleteUnitGraph#ExceptionDest}s to search.
   * @param trap the {@link Trap} to search for.
   * @return <tt>true</tt> if <tt>dests</tt> contains <tt>trap</tt> as a destination, false
   *     otherwise.
   */
  ...
  /**
   * Utility method to return the {@link Body} associated with a {@link DirectedGraph}, if there is
   * one.
   *
   * @param g the graph for which to return a {@link Body}.
   * @return the {@link Body} represented by <tt>g</tt>, if <tt>g</tt> is a control flow graph, or
  ...
  /**
   * Utility method that returns a {@link LabeledUnitPrinter} for printing the {@link Unit}s in
   * graph.
   *
   * @param g the graph for which to return a {@link LabeledUnitPrinter}.
   * @return A {@link LabeledUnitPrinter} for printing the {@link Unit}s in <tt>g</tt> if <tt>g</tt>
   *     is a control flow graph. Returns <tt>null</tt> if <tt>g</tt> is not a control flow graph.
   */
  ...
  /**
   * Utility method to return a {@link String} representation of a graph node.
   *
   * @param node an {@link Object} representing a node in a {@link DirectedGraph}.
   * @param printer either a {@link LabeledUnitPrinter} for printing the {@link Unit}s in the graph
   *     represented by <tt>node</tt>'s graph, if node is part of a control flow graph, or
   *     <tt>null</tt>, if <tt>node</tt> is not part of a control flow graph.
  ...
  /**
   * A utility method for reporting the differences between two lists of graph nodes. The lists are
   * usually the result of calling <tt>getHeads()</tt>, <tt>getTails()</tt>, <tt>getSuccsOf()</tt>
   * or <tt>getPredsOf()</tt> on each of two graphs being compared.
   *
   * @param buffer a {@link StringBuffer} to which to append the description of differences.
   * @param printer1 a {@link LabeledUnitPrinter} to be used to format any {@link Unit}s found in
   *     <tt>list1</tt>.
   * @param printer2 a {@link LabeledUnitPrinter} to be used to format any {@link Unit}s found in
   *     <tt>list2</tt>.
   * @param label a string characterizing these lists.
   * @param list1 the list from the first graph, or <tt>null</tt> if this list is missing in the
   *     first graph.
   * @param list2 the list from the second graph, or <tt>null</tt> if this list is missing in the
   *     second graph.
   */
    ...
    if (!equivLists(list1, list2)) {
      ...
      for (Iterator it = list2.iterator(); it.hasNext(); ) {
        ...
        if (!list1.contains(list1Node)) {
          buffer.append("+     ").append(nodeToString(list2Node, printer2)).append("\n");
        }
      }
      buffer.append("---------\n");
    }
   ...
   * @return <tt>true</tt> if the equivalent of each node in <tt>list1</tt> is found in
   *     <tt>list2</tt>, and vice versa.
   */
