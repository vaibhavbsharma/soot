...
public class ExceptionTestUtility {
  ...
  ExceptionTestUtility() {
    ...
    Scene.v().forceResolve("java.lang.StringIndexOutOfBoundsException", SootClass.BODIES);
    ...
    CLASS_CIRCULARITY_ERROR = Scene.v().getRefType("java.lang.ClassCircularityError");

    CLASS_FORMAT_ERROR = Scene.v().getRefType("java.lang.ClassFormatError");

    Scene.v().forceResolve("java.lang.UnsupportedClassVersionError", SootClass.BODIES);
    UNSUPPORTED_CLASS_VERSION_ERROR =
        Scene.v().getRefType("java.lang.UnsupportedClassVersionError");

    EXCEPTION_IN_INITIALIZER_ERROR = Scene.v().getRefType("java.lang.ExceptionInInitializerError");

    INCOMPATIBLE_CLASS_CHANGE_ERROR =
        Scene.v().getRefType("java.lang.IncompatibleClassChangeError");

    ABSTRACT_METHOD_ERROR = Scene.v().getRefType("java.lang.AbstractMethodError");

    ILLEGAL_ACCESS_ERROR = Scene.v().getRefType("java.lang.IllegalAccessError");

    INSTANTIATION_ERROR = Scene.v().getRefType("java.lang.InstantiationError");

    NO_SUCH_FIELD_ERROR = Scene.v().getRefType("java.lang.NoSuchFieldError");

    NO_SUCH_METHOD_ERROR = Scene.v().getRefType("java.lang.NoSuchMethodError");

    NO_CLASS_DEF_FOUND_ERROR = Scene.v().getRefType("java.lang.NoClassDefFoundError");
    ...
    OUT_OF_MEMORY_ERROR = Scene.v().getRefType("java.lang.OutOfMemoryError");

    STACK_OVERFLOW_ERROR = Scene.v().getRefType("java.lang.StackOverflowError");

    UNKNOWN_ERROR = Scene.v().getRefType("java.lang.UnknownError");

    THREAD_DEATH = Scene.v().getRefType("java.lang.ThreadDeath");

    Scene.v().forceResolve("java.lang.VirtualMachineError", SootClass.BODIES);
    ...
    Scene.v().forceResolve("java.lang.reflect.UndeclaredThrowableException", SootClass.BODIES);
    UNDECLARED_THROWABLE_EXCEPTION =
        Scene.v().getRefType("java.lang.reflect.UndeclaredThrowableException");

    Scene.v().forceResolve("javax.swing.UnsupportedLookAndFeelException", SootClass.BODIES);
    ...
    Scene.v().forceResolve("de.ecspride.NonExistingExceptionToTestPhantoms1", SootClass.BODIES);
    PHANTOM_EXCEPTION1 = Scene.v().getRefType("de.ecspride.NonExistingExceptionToTestPhantoms1");

    Scene.v().forceResolve("de.ecspride.NonExistingExceptionToTestPhantoms2", SootClass.BODIES);
    PHANTOM_EXCEPTION2 = Scene.v().getRefType("de.ecspride.NonExistingExceptionToTestPhantoms2");
    ...
    Set<RefLikeType> temp = new ExceptionHashSet<>(VM_ERRORS);
    ...
    Set<RefLikeType> tempForRep = new ExceptionHashSet<>(temp);
    ...
    temp = new ExceptionHashSet<>(VM_AND_RESOLVE_FIELD_ERRORS);
    temp.add(VIRTUAL_MACHINE_ERROR);
    temp.add(ERROR);
    temp.add(THROWABLE);
    ...
    temp = new ExceptionHashSet<>(VM_AND_RESOLVE_METHOD_ERRORS_PLUS_SUPERTYPES);
  ...
  /**
   * Verifies that the argument <code>set</code> is catchable as any of the exceptions in <code>
   * members</code>.
   *
   * @param set <code>ThrowableSet</code> whose membership is being checked.
   * @param members A {@link List} of {@link RefType} objects representing Throwable classes.
   */
  ...
  /**
   * Verifies that the argument <code>set</code> is not catchable as any of the exceptions in <code>
   * members</code>.
   *
   * @param set <code>ThrowableSet</code> whose membership is being checked.
   * @param members A {@link List} of {@link RefType} objects representing Throwable classes.
  ...
  /**
   * Verifies that the argument <code>set</code> is catchable as any of the exceptions in <code>
   * members</code>, but not as any other of the exceptions ALL_TEST_THROWABLES.
   *
   * @param set <code>ThrowableSet</code> whose membership is being checked.
   * @param members A {@link List} of {@link RefType} objects representing Throwable classes.
   */
  ...
  /**
   * Returns a Set representation of the subset of ALL_TEST_THROWABLES which are catchable by the
   * argument <code>ThrowableSet</code> (for use in assertions about the catchable exceptions.
   *
   * @param thrownSet <code>ThrowableSet</code> representing some set of possible exceptions.
   */
  public Set<RefType> catchableSubset(ThrowableSet thrownSet) {
    Set<RefType> result = new ExceptionHashSet<>(ALL_TEST_THROWABLES.size());
    for (RefType e : ALL_TEST_THROWABLES) {
  ...
  /**
   * Checks that the internal representation of the exceptions in an argument {@link ThrowableSet}
   * matches expectations.
   *
   * @param thrownSet {@link ThrowableSet} whose contents are being checked.
   * @param expectedIncludes contains the collection of {@link RefType} and {@link AnySubType}
   *     objects that are expected to be included in <code>thrownSet</code>.
   * @param expectedExcludes contains the collection of {@link RefType} and {@link AnySubType}
   *     objects that are expected to be excluded from <code>thrownSet</code>.
   * @return <code>true</code> if <code>thrownSet</code> and <code>expected</code> have the same
   *     members.
   */
  ...
  public static class ExceptionHashSet<T extends RefLikeType> extends HashSet<T> {
    ...
    public String toString() {
      ...
      Comparator<RefLikeType> comparator =
          new Comparator<RefLikeType>() {
            @Override
      ...
      result.append("\nExceptionHashSet<");
      for (RefLikeType content : contents) {
        result.append("\n\t");
        result.append(content.toString());
        result.append("<");
        result.append(Integer.toHexString(content.hashCode()));
        result.append(">");
      }
      result.append("\n>ExceptionHashSet");
